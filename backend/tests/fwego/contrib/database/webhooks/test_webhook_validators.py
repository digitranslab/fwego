...
from fwego.contrib.database.webhooks.validators import url_validator
...
@httpretty.activate(verbose=True, allow_net_connect=False)
@override_settings(FWEGO_WEBHOOKS_IP_WHITELIST=[ip_network("127.0.0.1/32")])
@patch("socket.getaddrinfo", wraps=stub_getaddrinfo)
...
@httpretty.activate(verbose=True, allow_net_connect=False)
@override_settings(FWEGO_WEBHOOKS_IP_BLACKLIST=[ip_network("1.1.1.1/32")])
@patch("socket.getaddrinfo", wraps=stub_getaddrinfo)
...
@httpretty.activate(verbose=True, allow_net_connect=False)
@override_settings(
    FWEGO_WEBHOOKS_URL_REGEX_BLACKLIST=[re.compile(r"(?:www\.?)?google.com")]
)
@patch("socket.getaddrinfo", wraps=stub_getaddrinfo)
...
@httpretty.activate(verbose=True, allow_net_connect=False)
@override_settings(
    FWEGO_WEBHOOKS_URL_REGEX_BLACKLIST=[URL_BLACKLIST_ONLY_ALLOWING_GOOGLE_WEBHOOKS]
)
@patch("socket.getaddrinfo", wraps=stub_getaddrinfo)
...
@httpretty.activate(verbose=True, allow_net_connect=False)
@override_settings(
    FWEGO_WEBHOOKS_IP_BLACKLIST=[ip_network("1.0.0.0/8")],
    FWEGO_WEBHOOKS_IP_WHITELIST=[ip_network("1.1.1.1/32")],
)
