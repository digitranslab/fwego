...
from fwego.contrib.database.webhooks.exceptions import (
    TableWebhookDoesNotExist,
    TableWebhookMaxAllowedCountExceeded,
)
from fwego.contrib.database.webhooks.handler import WebhookHandler
from fwego.contrib.database.webhooks.models import TableWebhook, TableWebhookCall
...
def test_get_all_table_webhooks(data_fixture, django_assert_num_queries):
    ...
    webhook_1 = data_fixture.create_table_webhook(
        ...
        headers={"Fwego-test-2": "Value 2"},
        include_all_events=False,
    )
    webhook_2 = data_fixture.create_table_webhook(
        table=table, headers={"Fwego-test-1": "Value 1"}, include_all_events=True
    )
    data_fixture.create_table_webhook()
...
@override_settings(FWEGO_WEBHOOKS_MAX_PER_TABLE=4)
def test_create_webhook(data_fixture):
    ...
    # new url
    webhook_data["url"] = "https://fwego.io/endpoint-2"
    ...
    webhook_data["url"] = "https://fwego.io/endpoint-3"
    headers = {"Fwego-test-1": "Value 1", "Fwego-header-2": "Value 2"}
    webhook = webhook_handler.create_table_webhook(
    ...
    assert webhook_headers[0].name == "Fwego-test-1"
    assert webhook_headers[0].value == "Value 1"
    assert webhook_headers[1].name == "Fwego-header-2"
    ...
    webhook_data["url"] = "https://fwego.io/endpoint-4"
...
def test_update_webhook(data_fixture):
    ...
    webhook = handler.update_table_webhook(
        user=user,
        webhook=webhook,
        headers={"Fwego-test-1": "Updated 1", "Fwego-test-3": "Updated 3"},
    )
    headers = webhook.headers.all()
    assert len(headers) == 2
    assert headers[0].id == old_headers[0].id
    assert headers[0].name == "Fwego-test-1"
    assert headers[0].value == "Updated 1"
    assert headers[1].name == "Fwego-test-3"
...
def test_trigger_test_call(data_fixture):
    ...
    assert response.request.headers["Fwego-add-1"] == "Value 1"
    assert response.request.headers["Content-type"] == "application/json"
    assert response.request.headers["X-Fwego-Event"] == "rows.created"
    assert "X-Fwego-Delivery" in request.headers
...
@pytest.mark.django_db
@override_settings(FWEGO_WEBHOOKS_MAX_CALL_LOG_ENTRIES=2)
