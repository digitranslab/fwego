...
from fwego.contrib.database.webhooks.tasks import call_webhook
from fwego.core.redis import RedisQueue
from fwego.test_utils.helpers import stub_getaddrinfo
...
@responses.activate
@override_settings(
    FWEGO_WEBHOOKS_MAX_RETRIES_PER_CALL=1,
    FWEGO_WEBHOOKS_MAX_CONSECUTIVE_TRIGGER_FAILURES=1,
)
@patch("fwego.contrib.database.webhooks.tasks.RedisQueue", MemoryQueue)
@patch("fwego.contrib.database.webhooks.tasks.cache", MagicMock())
@patch("fwego.contrib.database.webhooks.tasks.clear_webhook_queue")
def test_call_webhook_webhook_does_not_exist(mock_clear_queue):
    ...
    call_webhook.run(
        ...
        method="POST",
        url="http://localhost/",
        headers={"Fwego-header-1": "Value 1"},
...
def test_call_webhook_concurrent_task_moved_to_queue(data_fixture):
    from fwego.contrib.database.webhooks.tasks import get_queue
...
@patch("fwego.contrib.database.webhooks.tasks.RedisQueue", MemoryQueue)
@patch("fwego.contrib.database.webhooks.tasks.cache", MagicMock())
@patch("fwego.contrib.database.webhooks.tasks.schedule_next_task_in_queue")
...
@override_settings(
    ...
    FWEGO_WEBHOOKS_MAX_RETRIES_PER_CALL=0,
    FWEGO_WEBHOOKS_MAX_CONSECUTIVE_TRIGGER_FAILURES=0,
)
...
@responses.activate
@override_settings(
    FWEGO_WEBHOOKS_ALLOW_PRIVATE_ADDRESS=True,
    FWEGO_WEBHOOKS_MAX_RETRIES_PER_CALL=0,
    FWEGO_WEBHOOKS_MAX_CONSECUTIVE_TRIGGER_FAILURES=0,
)
@patch("fwego.contrib.database.webhooks.tasks.RedisQueue", MemoryQueue)
