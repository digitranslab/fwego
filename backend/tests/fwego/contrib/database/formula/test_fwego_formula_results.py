...
from fwego.contrib.database.fields.models import Field, FormulaField
...
from fwego.contrib.database.formula.ast.function_defs import (
    Fwego2dArrayAgg,
    FwegoAggJoin,
    FwegoArrayAggNoNesting,
    FwegoMultipleSelectCount,
    FwegoMultipleSelectOptionsAgg,
    FwegoStringAggMultipleSelectValues,
)
...
from fwego.contrib.database.formula.types.type_checker import MustBeManyExprChecker
from fwego.contrib.database.management.commands.fill_table_rows import fill_table_rows
...
VALID_FORMULA_TESTS = [
    ...
    ("tourl('fwego.io')", "fwego.io"),
    ("tourl('fwego.io/subpage')", "fwego.io/subpage"),
    ("tourl('fwego.io/subpage/?query=true')", "fwego.io/subpage/?query=true"),
    (
        "tourl(concat('fwego.io', '/subpage/?query=true'))",
        "fwego.io/subpage/?query=true",
    ),
    ("tourl(lower('FWEGO.io'))", "fwego.io"),
    ("tourl(replace('base-ow.io', '-', 'r'))", "fwego.io"),
    ("tourl('localhost')", "localhost"),
    ("tourl('skype://call-me')", "skype://call-me"),
    ("tourl('https://fwego.io:3000')", "https://fwego.io:3000"),
    ("tourl('ftp://fwego.io/some/path')", "ftp://fwego.io/some/path"),
    ("tourl('https://fwego.io:3000invalid')", ""),
    ("tourl('https://fwego.iohttps://fwego.io')", ""),
    (
        "tourl('https://user:password@www.fwego.io:8080/path/to/resource?search=query&filter=active#section2')",
        "https://user:password@www.fwego.io:8080/path/to/resource?search=query&filter=active#section2",
    ),
]
...
def test_aggregate_functions_never_allow_non_many_inputs(data_fixture, api_client):
    ...
    function_exceptions = {
        ...
        FwegoMultipleSelectOptionsAgg.type,
        FwegoMultipleSelectCount.type,
        FwegoStringAggMultipleSelectValues.type,
    }
    custom_cases = {
        FwegoAggJoin.type: [
            [literal("x"), literal("y")],
            [literal("x"), FwegoFieldReference[UnTyped](link_field.name, None, None)],
        ]
    }
    for formula_func in formula_function_registry.get_all():
        ...
        for arg_set in fake_args:
            formula = str(FwegoFunctionCall[UnTyped](formula_func, arg_set, None))
            try:
...
def construct_some_literal_args(formula_func):
    ...
    for a in args:
        ...
        if arg_checker in (FwegoFormulaValidType, FwegoFormulaMultipleSelectType):
            r = ""
        elif arg_checker == FwegoFormulaBooleanType:
            r = True
        elif arg_checker == FwegoFormulaTextType:
            r = "literal"
        elif arg_checker == FwegoFormulaNumberType:
            r = Decimal(1.2345)
        elif arg_checker == FwegoFormulaArrayType:
...
def get_field_name_from_arg_types(
    ...
    for a in args:
        ...
        if isinstance(arg_checker, MustBeManyExprChecker):
            arg_checker = arg_checker.formula_types[0]
        if arg_checker == FwegoFormulaValidType:
            r = text_field.name
        elif arg_checker == FwegoFormulaBooleanType:
            r = bool_field.name
        elif arg_checker == FwegoFormulaTextType:
        ...
        elif arg_checker == FwegoFormulaMultipleSelectType:
        ...
        field_refs.append(FwegoFieldReference[UnTyped](through_field.name, r, None))
    return field_refs
...
def test_formulas_with_lookup_url_field_type(data_fixture):
        ...
        [
            {
                ...
                linked_table_url_field.db_column: "https://fwego.io/1",
            },
            {
                linked_table_primary_field.db_column: "URL #2",
                linked_table_url_field.db_column: "https://fwego.io/2",
            },
        ],
    )
    ...
    assert rows == [
        [
            "Row #1",
            [{"id": 1, "value": "https://fwego.io/1"}],
            "https://fwego.io/1",
            "https://fwego.io/1",
            [{"id": 1, "value": "https://fwego.io/1"}],
            [{"id": 1, "value": "https://fwego.io/1some-text"}],
            "https://fwego.io/1",
            "https://fwego.io/1",
            Decimal("1"),
            [{"id": 1, "value": "https://fwego.io/1"}],
        ...
        [
            ...
            [
                {"id": 1, "value": "https://fwego.io/1"},
                {"id": 2, "value": "https://fwego.io/2"},
            ],
            "https://fwego.io/1https://fwego.io/2",
            "https://fwego.io/1some-texthttps://fwego.io/2",
            [
                {"id": 1, "value": "https://fwego.io/1"},
                {"id": 2, "value": "https://fwego.io/2"},
            ],
            [
                {"id": 1, "value": "https://fwego.io/1some-text"},
                {"id": 2, "value": "https://fwego.io/2some-text"},
            ],
            "https://fwego.io/2",
            "https://fwego.io/1",
            Decimal("2"),
