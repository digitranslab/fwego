...
from fwego.contrib.database.webhooks.models import TableWebhook, TableWebhookEvent
from fwego.core.utils import truncate_middle
...
def test_list_webhooks(api_client, data_fixture):
    ...
    webhook_1 = data_fixture.create_table_webhook(
        table=table, headers={"Fwego-add-1": "Value 1"}
    )
    ...
    assert response_json[0]["headers"] == {"Fwego-add-1": "Value 1"}
...
def test_create_webhooks(api_client, data_fixture):
    ...
    assert response_json["headers"] == {"Fwego-add-1": "Value 1"}
    assert response_json["calls"] == []
...
def test_trigger_test_call(api_client, data_fixture):
    ...
    responses.add(responses.POST, "http://fwego.io", json={}, status=200)
