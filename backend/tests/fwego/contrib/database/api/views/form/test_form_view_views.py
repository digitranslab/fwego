...
from fwego.test_utils.helpers import (
    AnyInt,
    is_dict_subset,
...
def test_create_form_view_with_webhooks(api_client, data_fixture):
    ...
    File "/fwego/backend/src/fwego/contrib/database/api/views/form/serializers.py",
    ...
    # here.
    with patch("fwego.contrib.database.webhooks.registries.call_webhook.delay") as m:
        response = api_client.post(
            reverse("api:database:views:list", kwargs={"table_id": table.id}),
            {
                "name": "Test Form",
...
def test_submit_form_with_link_row_field(api_client, data_fixture):
    ...
    form = data_fixture.create_form_view(
        ...
        submit_action_redirect_url="https://fwego.io",
    )
...
def test_submit_password_protected_form_view_requires_authorization(
    ...
    url = reverse("api:database:views:form:submit", kwargs={"slug": form_view.slug})
    response = api_client.get(
        url, format="json", HTTP_FWEGO_VIEW_AUTHORIZATION=f"JWT {public_view_token}"
    )
    assert response.status_code == HTTP_200_OK
    ...
    response = api_client.post(
        ...
        format="json",
        HTTP_FWEGO_VIEW_AUTHORIZATION=f"JWT {public_view_token}",
    )
    assert response.status_code == HTTP_200_OK
    response_json = response.json()
    assert response_json == {
...
def test_upload_file_view(api_client, data_fixture, tmpdir):
    ...
    with patch(
        "fwego.core.user_files.handler.get_default_storage"
    ) as get_storage_mock:
