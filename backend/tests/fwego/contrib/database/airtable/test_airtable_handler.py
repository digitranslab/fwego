...
from fwego.contrib.database.airtable.exceptions import AirtableShareIsNotABase
from fwego.contrib.database.airtable.handler import AirtableHandler
from fwego.contrib.database.airtable.job_types import AirtableImportJobType
from fwego.contrib.database.airtable.models import AirtableImportJob
from fwego.contrib.database.fields.models import TextField
from fwego.core.exceptions import UserNotInWorkspace
from fwego.core.jobs.constants import JOB_PENDING
from fwego.core.jobs.exceptions import JobDoesNotExist, MaxJobCountExceeded
from fwego.core.jobs.handler import JobHandler
from fwego.core.user_files.models import UserFile
from fwego.core.utils import Progress
...
@pytest.mark.django_db
@responses.activate
def test_to_fwego_database_export():
    base_path = os.path.join(
        settings.BASE_DIR, "../../../tests/airtable_responses/basic"
    )
    path = os.path.join(base_path, "airtable_base.html")
    ...
    schema, tables = AirtableHandler.extract_schema([user_table_json, data_table_json])
    fwego_database_export, files_buffer = AirtableHandler.to_fwego_database_export(
    ...
    assert fwego_database_export["id"] == 1
    assert fwego_database_export["name"] == "Test"
    assert fwego_database_export["order"] == 1
    assert fwego_database_export["type"] == "database"
    assert len(fwego_database_export["tables"]) == 2

    assert fwego_database_export["tables"][0]["id"] == "tblRpq315qnnIcg5IjI"
    assert fwego_database_export["tables"][0]["name"] == "Users"
    assert fwego_database_export["tables"][0]["order"] == 0
    assert len(fwego_database_export["tables"][0]["fields"]) == 4

    assert fwego_database_export["tables"][1]["id"] == "tbl7glLIGtH8C8zGCzb"
    assert fwego_database_export["tables"][1]["name"] == "Data"
    assert fwego_database_export["tables"][1]["order"] == 1
    assert len(fwego_database_export["tables"][1]["fields"]) == 25

    # We don't have to check all the fields and rows, just a single one, because we have
    # separate tests for mapping the Airtable fields and values to Fwego.
    assert (
        fwego_database_export["tables"][0]["fields"][0]["id"] == "fldG9y88Zw7q7u4Z7i4"
    )
    assert fwego_database_export["tables"][0]["fields"][0] == {
        "type": "text",
    ...
    assert fwego_database_export["tables"][0]["fields"][1] == {
        "type": "email",
    ...
    assert len(fwego_database_export["tables"][0]["rows"]) == 3
    assert fwego_database_export["tables"][0]["rows"][0] == {
        "id": 1,
    ...
    assert fwego_database_export["tables"][0]["rows"][1] == {
        "id": 2,
    ...
    assert fwego_database_export["tables"][0]["rows"][2] == {
        "id": 3,
    ...
    assert (
        fwego_database_export["tables"][1]["rows"][0]["field_fldEB5dp0mNjVZu0VJI"]
        == "2022-01-21T00:00:00+00:00"
    )
    assert fwego_database_export["tables"][0]["views"] == [
        {
            "id": 1,
            "type": "grid",
            "name": "Grid",
...
@pytest.mark.django_db
@responses.activate
def test_to_fwego_database_export_without_primary_value():
    base_path = os.path.join(
        settings.BASE_DIR, "../../../tests/airtable_responses/basic"
    )
    path = os.path.join(base_path, "airtable_base.html")
    ...
    assert fwego_database_export["tables"][0]["fields"][0]["primary"] is True
    ...
    assert fwego_database_export["tables"][0]["fields"] == [
        {
            "type": "text",
            "id": "primary_field",
...
@responses.activate
@patch("fwego.core.jobs.handler.run_async_job")
