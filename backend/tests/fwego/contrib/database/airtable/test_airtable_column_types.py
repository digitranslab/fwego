...
from fwego.contrib.database.airtable.airtable_column_types import (
...
from fwego.contrib.database.airtable.registry import airtable_column_type_registry
...
def test_airtable_import_text_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, TextField)
...
def test_airtable_import_checkbox_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, BooleanField)
...
def test_airtable_import_created_on_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, CreatedOnField)
    assert isinstance(airtable_column_type, FormulaAirtableColumnType)
    assert fwego_field.date_format == "ISO"
    assert fwego_field.date_include_time is False
    assert fwego_field.date_time_format == "24"
    assert fwego_field.date_force_timezone is None
    ...
    assert isinstance(fwego_field, CreatedOnField)
    assert isinstance(airtable_column_type, FormulaAirtableColumnType)
    assert fwego_field.date_format == "EU"
    assert fwego_field.date_include_time is True
    assert fwego_field.date_time_format == "12"
    assert fwego_field.date_force_timezone == "Europe/Amsterdam"

    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "2022-01-03T14:51:00.000Z", {}
        )
        is None
    )
...
def test_airtable_import_date_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, DateField)
    assert isinstance(airtable_column_type, DateAirtableColumnType)
    assert fwego_field.date_format == "US"
    assert fwego_field.date_include_time is False
    assert fwego_field.date_time_format == "24"
    ...
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "0999-02-04T14:51:00.000Z", {}
        )
        == "0999-02-04"
    )
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, None, {}
        )
        is None
    )
...
def test_airtable_import_european_date_column(data_fixture, api_client):
    ...
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "2020-08-27T21:10:24.828Z", {}
        )
        == "2020-08-27"
    )
    assert (
...
def test_airtable_import_datetime_with_default_timezone_column(
    ...
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "2022-01-03T23:51:00.000Z", {}
        )
...
def test_airtable_import_datetime_edge_case_1(data_fixture, api_client):
    ...
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "+020222-03-28T00:00:00.000Z", {}
        )
        is None
    )
...
def test_airtable_import_email_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, EmailField)
    assert isinstance(airtable_column_type, TextAirtableColumnType)

    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "NOT_EMAIL", {}
        )
        == ""
    )
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "test@test.nl", {}
        )
        == "test@test.nl"
    )
...
def test_airtable_import_multiple_attachment_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, FileField)
    assert isinstance(airtable_column_type, MultipleAttachmentAirtableColumnType)

    files_to_download = {}
    assert airtable_column_type.to_fwego_export_serialized_value(
        {},
        airtable_field,
        fwego_field,
        [
...
def test_airtable_import_last_modified_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, LastModifiedField)
    assert isinstance(airtable_column_type, FormulaAirtableColumnType)
    assert fwego_field.date_format == "ISO"
    assert fwego_field.date_include_time is False
    assert fwego_field.date_time_format == "24"
...
def test_airtable_import_foreign_key_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, LinkRowField)
    assert isinstance(airtable_column_type, ForeignKeyAirtableColumnType)
    assert fwego_field.link_row_table_id == "tblRpq315qnnIcg5IjI"
    assert fwego_field.link_row_related_field_id == "fldFh5wIL430N62LN6t"
    ...
    assert isinstance(airtable_column_type, ForeignKeyAirtableColumnType)
    assert fwego_field.link_row_table_id == "tblRpq315qnnIcg5IjI"
    assert fwego_field.link_row_related_field_id == "fldQcEaGEe7xuhUEuPL"
...
def test_airtable_import_multiline_text_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, LongTextField)
    assert isinstance(airtable_column_type, MultilineTextAirtableColumnType)

    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "test", {}
        )
        == "test"
    )
...
def test_airtable_import_rich_text_column(data_fixture, api_client):
    ...
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, content, {}
        )
...
def test_airtable_import_multi_select_column(
    ...
    assert isinstance(fwego_field, MultipleSelectField)
    assert isinstance(airtable_column_type, MultiSelectAirtableColumnType)

    fwego_field.table_id = table.id
    fwego_field.order = 999
    fwego_field.save()

    with django_assert_num_queries(0):
        select_options = list(fwego_field.select_options.all())
...
def test_airtable_import_number_integer_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, NumberField)
    assert isinstance(airtable_column_type, NumberAirtableColumnType)
    assert fwego_field.number_decimal_places == 0
    assert fwego_field.number_negative is False

    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "10", {}
        )
        == "10"
    )
    assert (
...
def test_airtable_import_number_decimal_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, NumberField)
    assert isinstance(airtable_column_type, NumberAirtableColumnType)
    assert fwego_field.number_decimal_places == 1
    assert fwego_field.number_negative is False
    ...
    assert isinstance(fwego_field, NumberField)
    assert isinstance(airtable_column_type, NumberAirtableColumnType)
    assert fwego_field.number_decimal_places == 2
    assert fwego_field.number_negative is True

    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "10.22", {}
        )
        == "10.22"
    )
    assert (
    ...
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "-10.555", {}
        )
        == "-10.555"
    )
    assert (
    ...
    assert isinstance(fwego_field, NumberField)
    assert isinstance(airtable_column_type, NumberAirtableColumnType)
    assert fwego_field.number_decimal_places == 10
    assert fwego_field.number_negative is True
...
def test_airtable_import_phone_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, PhoneNumberField)
    assert isinstance(airtable_column_type, PhoneAirtableColumnType)

    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "NOT_PHONE", {}
        )
        == ""
    )
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "1234", {}
        )
        == "1234"
    )
...
def test_airtable_import_rating_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, RatingField)
    assert isinstance(airtable_column_type, RatingAirtableColumnType)
    assert fwego_field.max_value == 5
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, 5, {}
        )
        == 5
    )
...
def test_airtable_import_select_column(
    ...
    assert isinstance(fwego_field, SingleSelectField)
...
def test_airtable_import_url_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, URLField)
    assert isinstance(airtable_column_type, TextAirtableColumnType)

    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "NOT_URL", {}
        )
        == ""
    )
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "https://test.nl", {}
        )
        == "https://test.nl"
    )
...
def test_airtable_import_count_column(data_fixture, api_client):
    ...
    assert isinstance(fwego_field, CountField)
    assert isinstance(airtable_column_type, CountAirtableColumnType)

    assert fwego_field.through_field_id == "fldABC88Zw7q7u4Z7i4"
    assert (
        airtable_column_type.to_fwego_export_serialized_value(
            {}, airtable_field, fwego_field, "1", {}
        )
        is None
    )
