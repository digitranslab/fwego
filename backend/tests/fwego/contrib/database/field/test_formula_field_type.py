...
from fwego.contrib.database.fields.field_types import FormulaFieldType
from fwego.contrib.database.fields.fields import FwegoExpressionField
...
from fwego.contrib.database.formula.ast.tree import FwegoFunctionDefinition
...
from fwego.contrib.database.table.cache import generated_models_cache
...
def test_can_change_formula_type_breaking_other_fields(data_fixture):
    ...
    assert second_formula_field.formula_type == FwegoFormulaInvalidType.type
...
def test_all_functions_are_registered():
    ...
    assert len(names) == len(get_all_subclasses(FwegoFunctionDefinition))
...
def test_recalculated_internal_type_with_incorrect_syntax_formula_sets_to_invalid(
    ...
    formula_field.save()
    assert formula_field.formula_type == FwegoFormulaInvalidType.type
...
def test_accessing_cached_internal_formula_second_time_does_no_queries(
    ...
    with django_assert_num_queries(0):
        ...
        assert formula_field.cached_formula_type.type == FwegoFormulaTextType.type
...
def test_saving_after_properties_have_been_cached_does_recalculation(data_fixture):
    ...
    assert formula_field.cached_formula_type.type == FwegoFormulaNumberType.type
...
def test_can_cache_and_uncache_formula_model_field(
    ...
    assert isinstance(uncached, FwegoExpressionField)
