...
from fwego.contrib.database.fields.field_helpers import (
...
from fwego.contrib.database.views.models import ViewFilter
...
def _test_can_convert_between_fields(data_fixture, field_type_to_test):
    ...
    # Some fwego field types have multiple different 'modes' which result in
...
@pytest.mark.django_db
@patch("fwego.contrib.database.fields.signals.field_created.send")
def test_create_field(send_mock, data_fixture):
    ...
    with pytest.raises(ReservedFwegoFieldNameException):
...
def test_field_which_changes_its_underlying_type_will_have_alter_sql_run(data_fixture):
    ...
    with patch.dict(
        ...
        # Update to the same fwego type, but due to this fields implementation of
...
@pytest.mark.django_db
@patch("fwego.contrib.database.fields.signals.field_deleted.send")
