...
from fwego.contrib.database.fields.periodic_field_update_handler import (
    PeriodicFieldUpdateHandler,
)
from fwego.contrib.database.fields.registries import field_type_registry
from fwego.contrib.database.fields.tasks import (
...
def test_run_periodic_fields_updates_if_necessary(data_fixture, settings):
    settings.FWEGO_PERIODIC_FIELD_UPDATE_UNUSED_WORKSPACE_INTERVAL_MIN = 5
    ...
    with freeze_time("2020-01-01 0:00"):
        ...
        row = RowHandler().create_row(
            user=user, table=table_model.fwego_table, model=table_model
        )
        ...
        row_2 = RowHandler().create_row(
            user=user, table=table_model_2.fwego_table, model=table_model_2
        )
        ...
        row_3 = RowHandler().create_row(
            user=user, table=table_model_3.fwego_table, model=table_model_3
        )
    ...
    with patch(
        "fwego.contrib.database.fields.tasks._run_periodic_field_type_update_per_workspace"
...
def test_workspace_updated_last_will_be_updated_first_this_time(data_fixture, settings):
    settings.FWEGO_PERIODIC_FIELD_UPDATE_UNUSED_WORKSPACE_INTERVAL_MIN = 0
    user = data_fixture.create_user()
