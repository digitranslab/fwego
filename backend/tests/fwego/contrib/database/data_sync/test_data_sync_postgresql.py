...
from fwego.contrib.database.data_sync.models import PostgreSQLDataSync
from fwego.contrib.database.data_sync.postgresql_data_sync_type import (
...
def test_postgresql_data_sync_get_properties_unsupported_column_types(
    ...
    with patch(
        "fwego.contrib.database.data_sync.postgresql_data_sync_type.column_type_to_fwego_field_type",
        new={
...
def test_postgresql_data_sync_table_connect_to_same_database(data_fixture):
    ...
    with pytest.raises(SyncError) as e:
        with override_settings(
            FWEGO_PREVENT_POSTGRESQL_DATA_SYNC_CONNECTION_TO_DATABASE=True
        ):
            data_sync = handler.create_data_sync_table(
                user=user,
                database=database,
...
def test_postgresql_data_sync_table_connect_to_blacklist(data_fixture):
    ...
    with pytest.raises(SyncError) as e:
        with override_settings(
            FWEGO_POSTGRESQL_DATA_SYNC_BLACKLIST=["localhost", "fwego.io"]
        ):
            data_sync = handler.create_data_sync_table(
                user=user,
                database=database,
    ...
    # hostnames in FWEGO_POSTGRESQL_DATA_SYNC_BLACKLIST.
    assert str(e.value) == "It's not allowed to connect to this hostname."
