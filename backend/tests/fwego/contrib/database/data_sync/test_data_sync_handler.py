...
from fwego.contrib.database.data_sync.ical_data_sync_type import (
    ICalCalendarDataSyncType,
    UIDICalCalendarDataSyncProperty,
)
from fwego.contrib.database.data_sync.models import (
    DataSync,
    DataSyncSyncedProperty,
    ICalCalendarDataSync,
)
from fwego.contrib.database.data_sync.registries import DataSyncTypeRegistry
from fwego.contrib.database.fields.exceptions import CannotDeletePrimaryField
from fwego.contrib.database.fields.handler import FieldHandler
from fwego.contrib.database.fields.models import Field, LongTextField, TextField
...
def test_create_data_sync_table_no_unique_primary(data_fixture):
    ...
    with patch(
        "fwego.contrib.database.data_sync.handler.data_sync_type_registry",
        new=registry,
    ):
        user = data_fixture.create_user()
        database = data_fixture.create_database_application(user=user)
...
@pytest.mark.django_db
@patch("fwego.contrib.database.table.signals.table_created.send")
def test_create_data_sync_table(send_mock, data_fixture):
    ...
    assert data_sync.ical_url == "https://fwego.io"
...
@pytest.mark.django_db
@patch("fwego.contrib.database.table.signals.table_updated.send")
...
def test_sync_data_sync_table_create_update_delete_row(data_fixture):
    ...
    data_sync = handler.create_data_sync_table(
        ...
        type_name="ical_calendar",
        synced_properties=["uid", "dtstart", "dtend", "summary"],
        ical_url="https://fwego.io/ical.ics",
    )
...
@pytest.mark.django_db
@patch(
    "fwego.contrib.database.data_sync.ical_data_sync_type.ICalCalendarDataSyncType.get_all_rows"
)
...
def test_set_data_sync_synced_properties_field_types_changed(data_fixture):
    ...
    class TmpUIDICalCalendarDataSyncProperty(UIDICalCalendarDataSyncProperty):
        def to_fwego_field(self) -> TextField:
