...
from fwego.contrib.database.rows.exceptions import RowDoesNotExist
...
@pytest.mark.django_db
@patch("fwego.contrib.database.rows.signals.rows_updated.send")
def test_update_row_by_id(send_mock, data_fixture):
    ...
    with patch(
        "fwego.contrib.database.rows.signals.before_rows_update.send"
    ) as before_send_mock:
        handler.update_row_by_id(
            user=user,
            table=table,
...
@pytest.mark.django_db
@patch("fwego.ws.tasks.broadcast_to_users.delay")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.rows.signals.rows_updated.send")
@patch("fwego.contrib.database.rows.signals.before_rows_update.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.rows.signals.rows_deleted.send")
@patch("fwego.contrib.database.rows.signals.before_rows_delete.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.rows.signals.row_orders_recalculated.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.rows.signals.rows_created.send")
@patch("fwego.contrib.database.views.handler.ViewHandler.field_value_updated")
@patch(
    "fwego.contrib.database.search.handler.SearchHandler.field_value_updated_or_created"
)
