...
def test_urlfield_get_search_expression(data_fixture, enable_singleton_testing):
    with transaction.atomic():
        ...
        row = RowHandler().create_row(
            user=user, table=table, values={f"field_{field.id}": "https://fwego.io"}
        )
    model = table.get_model()
    qs = model.objects.all().pg_search("https://fwego.io")
    assert qs.exists()
    matching_row = qs.get()
    assert matching_row.id == row.id
...
def test_emailfield_get_search_expression(data_fixture, enable_singleton_testing):
    with transaction.atomic():
        ...
        row = RowHandler().create_row(
            user=user, table=table, values={f"field_{field.id}": "dev@fwego.io"}
        )
    model = table.get_model()
    qs = model.objects.all().pg_search("dev@fwego.io")
    assert qs.exists()
    matching_row = qs.get()
    assert matching_row.id == row.id
...
def test_last_modified_by_field_get_search_expression(
    ...
    with transaction.atomic():
        user_adam = data_fixture.create_user(
            email="user1@fwego.io", first_name="Adam"
        )
        user_john_smith = data_fixture.create_user(
            email="user2@fwego.io", first_name="John Smith"
        )
        user_mary_black = data_fixture.create_user(
            email="user3@fwego.io", first_name="Mary Black"
        )
