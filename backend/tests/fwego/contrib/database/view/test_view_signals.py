...
from fwego.contrib.database.views.signals import (
    view_loaded_create_indexes_and_columns,
)


@patch("fwego.contrib.database.views.handler.ViewIndexingHandler")
...
def test_view_loaded_creates_last_modified_by_column(indexing_handler, data_fixture):
    ...
    with patch(
        "fwego.contrib.database.table.tasks.setup_created_by_and_last_modified_by_column"
    ) as setup:
        view_loaded_create_indexes_and_columns(
