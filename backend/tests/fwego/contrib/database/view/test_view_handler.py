...
from fwego.contrib.database.search.handler import ALL_SEARCH_MODES, SearchHandler
...
from fwego.contrib.database.views.handler import (
    PublicViewRows,
    ViewHandler,
    ViewIndexingHandler,
)
...
from fwego.contrib.database.views.registries import (
    view_aggregation_type_registry,
    view_filter_type_registry,
    view_type_registry,
)
from fwego.contrib.database.views.signals import view_loaded
from fwego.contrib.database.views.view_ownership_types import (
    CollaborativeViewOwnershipType,
)
from fwego.contrib.database.views.view_types import GridViewType
from fwego.core.db import get_collation_name
from fwego.core.exceptions import PermissionDenied, UserNotInWorkspace
from fwego.core.trash.handler import TrashHandler
from fwego.test_utils.helpers import setup_interesting_test_table
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_created.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_updated.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_deleted.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_created.send")
@patch("fwego.contrib.database.views.signals.views_reordered.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_field_options_updated.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_filter_created.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_filter_updated.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_filter_deleted.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_sort_created.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_sort_updated.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_sort_deleted.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.rows.signals.rows_created.send")
...
def test_create_view_ownership_type(data_fixture):
    ...
    with patch(
        "fwego.contrib.database.views.registries.view_ownership_type_registry.registry",
        ownership_types,
    ):
        user = data_fixture.create_user()
        table = data_fixture.create_database_table(user=user)
...
@patch("fwego.contrib.database.views.tasks.update_view_index.delay")
@pytest.mark.django_db(transaction=True)
...
def test_update_index_replaces_index_with_diff_collation(
    ...
    with patch("fwego.core.db.get_collation_name", new=lambda: None):
        user = data_fixture.create_user()
        table = data_fixture.create_database_table(user=user)
        text_field = data_fixture.create_text_field(user=user, table=table)
        handler = ViewHandler()
    ...
    with patch(
        "fwego.core.db.get_collation_name", new=lambda: settings.EXPECTED_COLLATION
    ):
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_group_by_created.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_group_by_updated.send")
...
@pytest.mark.django_db
@patch("fwego.contrib.database.views.signals.view_group_by_deleted.send")
