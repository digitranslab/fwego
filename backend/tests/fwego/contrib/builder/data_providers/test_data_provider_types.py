...
from fwego.contrib.builder.data_providers.data_provider_types import (
...
from fwego.core.user_sources.constants import DEFAULT_USER_ROLE_PREFIX
...
@patch(
    "fwego.contrib.builder.data_providers.data_provider_types.FormDataProviderType.validate_data_chunk"
)
...
@patch("fwego.contrib.builder.data_providers.data_provider_types.ElementHandler")
...
def test_data_source_data_provider_get_data_chunk_with_formula_using_list_datasource(
    ...
    page = data_fixture.create_builder_page(user=user, builder=builder)
    data_source2 = data_fixture.create_builder_local_fwego_list_rows_data_source(
        user=user,
        page=page,
        integration=integration,
        view=view2,
...
def test_current_record_provider_get_data_chunk_without_record_index(data_fixture):
    ...
    builder = data_fixture.create_builder_application(user=user)
    page = data_fixture.create_builder_page(user=user, builder=builder)
    workflow_action = data_fixture.create_local_fwego_create_row_workflow_action(
...
def test_data_source_context_data_provider_extract_properties_raises_if_data_source_doesnt_exist(
    ...
    Ensure that InvalidFwegoFormula is raised if the Data Source doesn't exist.
    """

    mock_get_data_source.side_effect = DataSourceDoesNotExist()

    with pytest.raises(InvalidFwegoFormula):
