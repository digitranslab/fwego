...
from fwego.contrib.builder.data_sources.builder_dispatch_context import (
    BuilderDispatchContext,
)
from fwego.contrib.builder.data_sources.exceptions import DataSourceDoesNotExist
from fwego.contrib.builder.data_sources.handler import DataSourceHandler
...
from fwego.core.exceptions import CannotCalculateIntermediateOrder
...
from tests.fwego.contrib.builder.api.user_sources.helpers import (
...
def test_create_data_source(data_fixture):
    ...
    assert isinstance(data_source.service, LocalFwegoGetRow)
...
def test_get_data_sources(data_fixture, specific):
    page = data_fixture.create_builder_page()
    data_source1 = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page
    )
    data_source2 = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page
    )
    data_source3 = data_fixture.create_builder_local_fwego_list_rows_data_source(
        page=page
    )
    ...
    if specific:
        assert isinstance(data_sources[0].service, LocalFwegoGetRow)
        assert isinstance(data_sources[2].service, LocalFwegoListRows)
...
def test_get_data_sources_with_shared(data_fixture):
    ...
    shared_data_source2 = (
        data_fixture.create_builder_local_fwego_list_rows_data_source(
            page=shared_page
        )
    )
    ...
    assert isinstance(data_sources[2].service, LocalFwegoGetRow)
    assert isinstance(data_sources[4].service, LocalFwegoListRows)
...
def test_update_data_source_change_type(data_fixture):
    ...
    service_type = service_type_registry.get("local_fwego_get_row")
    new_service_type = service_type_registry.get("local_fwego_list_rows")
...
def test_update_data_source_change_page_with_conflict(data_fixture):
    ...
    page_dest = data_fixture.create_builder_page(builder=data_source.page.builder)
    data_fixture.create_builder_local_fwego_get_row_data_source(
...
def test_dispatch_data_sources(data_fixture):
    ...
    data_source3 = data_fixture.create_builder_local_fwego_get_row_data_source(
        user=user,
        page=page,
        integration=integration,
        view=view,
...
def test_recalculate_full_orders(data_fixture):
    ...
    data_source4 = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page, order="2.10000000000000000000"
    )
    data_source5 = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page, order="3.00000000000000000000"
    )
    data_source6 = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page, order="1.00000000000000000001"
    )
    data_source7 = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page, order="3.99999999999999999999"
    )
    data_source8 = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page, order="4.00000000000000000001"
    )

    page2 = data_fixture.create_builder_page()

    data_sourceA = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page2, order="1.99999999999999999999"
    )
    data_sourceB = data_fixture.create_builder_local_fwego_get_row_data_source(
...
@pytest.mark.django_db
@patch(
    "fwego.contrib.builder.data_sources.builder_dispatch_context.get_builder_used_property_names"
)
