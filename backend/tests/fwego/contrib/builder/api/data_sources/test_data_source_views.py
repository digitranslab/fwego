...
from fwego.core.services.models import Service
from fwego.core.user_sources.user_source_user import UserSourceUser
from fwego.test_utils.helpers import AnyInt, AnyStr
...
def test_get_data_sources(api_client, data_fixture):
    ...
    assert response_json[0]["type"] == "local_fwego_get_row"
    assert "row_id" in response_json[0]
    assert response_json[1]["id"] == data_source2.id
    assert response_json[1]["type"] == "local_fwego_get_row"
    assert response_json[2]["id"] == data_source3.id
    assert response_json[2]["type"] == "local_fwego_list_rows"
...
def test_create_data_source(api_client, data_fixture):
    ...
    response_json = response.json()
    assert response.status_code == HTTP_200_OK
    assert response_json["type"] == "local_fwego_get_row"
...
def test_create_data_source_bad_request(api_client, data_fixture):
    ...
    response = api_client.post(
        url,
        {"type": "local_fwego_get_row", "table_id": []},
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
...
def test_update_data_source_with_with_name_conflict_is_disallowed_on_same_pages(
    ...
    user, token = data_fixture.create_user_and_token()
    page = data_fixture.create_builder_page(user=user)
    data_source_1 = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page, name="Blog posts"
    )
    data_source_2 = data_fixture.create_builder_local_fwego_get_row_data_source(
...
def test_update_data_source_change_type(api_client, data_fixture):
    ...
    page = data_fixture.create_builder_page(user=user)
    table = data_fixture.create_database_table(user=user)
    service = data_fixture.create_local_fwego_aggregate_rows_service(
        table_id=table.id,
    )
    ...
    response = api_client.patch(
        url,
        {"type": "local_fwego_list_rows", "field_id": None, "table_id": table.id},
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
...
def test_dispatch_data_source_improperly_configured(api_client, data_fixture):
    ...
    page = data_fixture.create_builder_page(user=user, builder=builder)
    data_source0 = data_fixture.create_builder_local_fwego_get_row_data_source(
        user=user,
        page=page,
        integration=integration,
        view=view,
...
def test_dispatch_only_shared_data_sources(data_fixture, api_client):
    ...
    shared_data_source = data_fixture.create_builder_local_fwego_get_row_data_source(
        user=user,
        page=shared_page,
        integration=integration,
        view=view,
...
def test_dispatch_data_sources_get_and_list_rows_with_elements(
    ...
    data_source_2 = data_fixture.create_builder_local_fwego_list_rows_data_source(
        user=user,
        page=data_source_fixture["page"],
        integration=data_source_fixture["integration"],
        table=table_2,
...
@pytest.mark.django_db
@patch(
    "fwego.contrib.builder.api.data_sources.views.DataSourceService.dispatch_data_source"
)
@patch("fwego.contrib.builder.api.data_sources.views.BuilderDispatchContext")
@patch(
    "fwego.contrib.builder.api.data_sources.views.DataSourceHandler.get_data_source"
)
...
@pytest.mark.django_db
@patch(
    "fwego.contrib.builder.api.data_sources.views.DataSourceService.dispatch_page_data_sources"
)
@patch("fwego.contrib.builder.api.data_sources.views.BuilderDispatchContext")
@patch("fwego.contrib.builder.api.data_sources.views.PageHandler.get_page")
