...
def test_get_public_builder_by_domain_name(api_client, data_fixture):
    ...
    domain = data_fixture.create_builder_custom_domain(
        domain_name="test.getfwego.io", published_to=builder_to
    )

    url = reverse(
        "api:builder:domains:get_builder_by_domain_name",
        kwargs={"domain_name": "test.getfwego.io"},
    )
...
def test_get_builder_missing_domain_name(api_client, data_fixture):
    ...
    domain = data_fixture.create_builder_custom_domain(
        domain_name="test.getfwego.io", published_to=page.builder
    )

    url = reverse(
        "api:builder:domains:get_builder_by_domain_name",
        kwargs={"domain_name": "notexists.getfwego.io"},
    )
    response = api_client.get(
        url,
        format="json",
...
def test_get_non_public_builder(api_client, data_fixture):
    ...
    page2 = data_fixture.create_builder_page(builder=page.builder, user=user)
    domain = data_fixture.create_builder_custom_domain(
        domain_name="test.getfwego.io", builder=page.builder
    )
...
def test_publish_builder(mock_run_async_job, api_client, data_fixture):
    ...
    domain = data_fixture.create_builder_custom_domain(
        domain_name="test.getfwego.io", builder=builder_from
    )
...
def test_ask_public_builder_domain_exists(api_client, data_fixture):
    ...
    url = reverse("api:builder:domains:ask_exists") + "?domain=test2.getfwego.io"
    response = api_client.get(url)
    assert response.status_code == 404
    ...
    url = reverse("api:builder:domains:ask_exists") + "?domain=test.getfwego.io"
    response = api_client.get(url)
...
@pytest.mark.django_db
@patch("fwego.contrib.builder.api.domains.public_views.BuilderDispatchContext")
@patch(
    "fwego.contrib.builder.api.domains.public_views.DataSourceService.dispatch_data_source"
)
@patch(
    "fwego.contrib.builder.api.domains.public_views.DataSourceHandler.get_data_source"
)
...
@pytest.mark.django_db
@patch(
    "fwego.contrib.builder.api.domains.public_views.DataSourceService.dispatch_page_data_sources"
)
@patch("fwego.contrib.builder.api.domains.public_views.BuilderDispatchContext")
@patch("fwego.contrib.builder.api.domains.public_views.PageHandler.get_page")
