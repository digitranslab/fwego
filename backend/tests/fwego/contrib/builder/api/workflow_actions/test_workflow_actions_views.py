...
def test_create_create_row_workflow_action(api_client, data_fixture):
    ...
    assert response_json["service"] == {
        ...
        "integration_id": None,
        "row_id": "",
        "type": LocalFwegoUpsertRowServiceType.type,
        "schema": None,
        "table_id": None,
        "field_mappings": [],
        "context_data": None,
...
def test_update_update_row_workflow_action(api_client, data_fixture):
    ...
    page = data_fixture.create_builder_page(user=user, builder=builder)
    element = data_fixture.create_builder_button_element(page=page)
    workflow_action = data_fixture.create_local_fwego_update_row_workflow_action(
        page=page, element=element, event=EventTypes.CLICK, user=user
    )
    service = workflow_action.service
    service_type = service.get_type()
...
@pytest.mark.django_db
def test_dispatch_local_fwego_create_row_workflow_action(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    table, fields, rows = data_fixture.build_table(
        user=user,
        columns=[
...
@pytest.mark.django_db
def test_dispatch_local_fwego_update_row_workflow_action(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    table, fields, rows = data_fixture.build_table(
        user=user,
        columns=[
...
@pytest.mark.django_db
def test_dispatch_local_fwego_upsert_row_workflow_action_with_current_record(
    api_client, data_fixture
):
    user, token = data_fixture.create_user_and_token()
...
@pytest.mark.django_db
def test_dispatch_local_fwego_update_row_workflow_action_using_formula_with_data_source(
    api_client, data_fixture
):
    user, token = data_fixture.create_user_and_token()
...
def test_update_delete_row_workflow_action(api_client, data_fixture):
    ...
    page = data_fixture.create_builder_page(builder=builder)
    element = data_fixture.create_builder_button_element(page=page)
    workflow_action = data_fixture.create_local_fwego_delete_row_workflow_action(
