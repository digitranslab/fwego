...
from fwego.contrib.builder.application_types import BuilderApplicationType
from fwego.contrib.builder.builder_beta_init_application import (
    BuilderApplicationTypeInitApplication,
)
from fwego.contrib.builder.elements.models import (
    ColumnElement,
    Element,
    HeadingElement,
    LinkElement,
...
from fwego.contrib.builder.theme.handler import ThemeHandler
from fwego.contrib.builder.workflow_actions.handler import (
...
from fwego.core.actions import CreateApplicationActionType
from fwego.core.db import specific_iterator
from fwego.core.registries import ImportExportConfig, application_type_registry
...
from fwego.core.user_sources.registries import DEFAULT_USER_ROLE_PREFIX
...
def test_builder_application_export(data_fixture):
    ...
    shared_datasource = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=shared_page, user=user, name="Shared", integration=integration
    )
    datasource1 = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page1, user=user, name="source 1", integration=integration
    )
    datasource2 = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page2, user=user, name="source 2", integration=integration
    )
    datasource3 = data_fixture.create_builder_local_fwego_list_rows_data_source(
...
def test_builder_application_import(data_fixture):
    user = data_fixture.create_user(email="test@fwego.io")
    workspace = data_fixture.create_workspace(user=user)
...
def test_builder_application_doesnt_import_favicon_file(data_fixture):
    ...
    with patch(
        "fwego.contrib.builder.application_types.UserFileHandler"
    ) as mocked_handler:
        builder = BuilderApplicationType().import_serialized(
            workspace, serialized_values, config, {}
        )
...
def test_ensure_new_element_roles_are_sanitized_during_import_for_roles(
    ...
    with patch.object(LocalFwegoUserSourceType, "get_roles") as m:
...
def test_builder_application_exports_file_with_zip_file(
    ...
    zip_file = ExportZipFile(
        compress_level=settings.FWEGO_DEFAULT_ZIP_COMPRESS_LEVEL,
