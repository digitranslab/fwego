...
from fwego.contrib.builder.formula_property_extractor import (
    FormulaFieldVisitor,
...
from fwego.contrib.builder.workflow_actions.service import (
    BuilderWorkflowActionService,
)
from fwego.contrib.builder.workflow_actions.workflow_action_types import (
...
from fwego.core.formula import FwegoFormula
from fwego.core.formula.exceptions import InvalidFwegoFormula
from fwego.core.formula.parser.exceptions import FwegoFormulaSyntaxError
from fwego.core.formula.registries import DataProviderType
from fwego.core.formula.runtime_formula_context import RuntimeFormulaContext
...
@patch("fwego.contrib.builder.mixins.get_parse_tree_for_formula")
...
def test_get_element_property_names_returns_empty_if_invalid_formula(
    ...
    mock_parse_tree.side_effect = FwegoFormulaSyntaxError("Invalid formula!")
...
def test_get_element_property_names_returns_property_names(data_fixture):
    ...
    builder = data_fixture.create_builder_application(user=user)
    page = data_fixture.create_builder_page(user=user, builder=builder)
    data_source_1 = data_fixture.create_builder_local_fwego_list_rows_data_source(
        user=user,
        page=page,
        table=table,
    )
...
def test_get_data_source_property_names_returns_empty_if_invalid_formula(
    ...
    list_rows_service = data_fixture.create_local_fwego_list_rows_service(
        integration=integration,
        search_query="foo",
    )
...
def test_get_data_source_property_names_get_row_returns_property_names(data_fixture):
    ...
    get_row_service = data_fixture.create_local_fwego_get_row_service(
        integration=integration
    )
...
@patch(
    "fwego.contrib.builder.formula_property_extractor.builder_data_provider_type_registry"
)
def test_formula_property_visitor_visit_function_call_handles_formula_error(
    ...
    mock_data_provider_type.extract_properties.side_effect = InvalidFwegoFormula()
    mock_data_provider_registry.get.return_value = mock_data_provider_type

    mock_expression = MagicMock(spec=FwegoFormula.StringLiteralContext)
...
def test_get_builder_used_property_names_returns_merged_property_names_integration(
    ...
    data_source_3 = data_fixture.create_builder_local_fwego_list_rows_data_source(
        user=user,
        page=shared_page,
        table=table,
    )
    data_source_4 = data_fixture.create_builder_local_fwego_list_rows_data_source(
        user=user,
        page=page2,
        table=table,
    )
    ...
    data_source_5 = data_fixture.create_builder_local_fwego_get_row_data_source(
        page=page,
        integration=integration,
        table=table,
    ...
    workflow_action2 = data_fixture.create_local_fwego_create_row_workflow_action(
