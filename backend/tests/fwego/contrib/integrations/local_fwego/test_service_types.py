...
def test_local_fwego_table_service_before_dispatch_validation_error(
    data_fixture,
):
    cls = LocalFwegoTableServiceType
...
@pytest.mark.django_db
def test_local_fwego_table_service_generate_schema_with_no_table(data_fixture):
    user = data_fixture.create_user()
    ...
    get_row_service_type = LocalFwegoGetRowUserServiceType()
    ...
    list_rows_service_type = LocalFwegoListRowsUserServiceType()
...
@pytest.mark.django_db
def test_local_fwego_table_service_generate_schema_with_interesting_test_table(
    data_fixture,
):
    ...
    expected_local_fwego_table_service_schema_fields = {
...
def test_local_fwego_service_type_get_schema_for_return_type():
    mock_service = Mock(id=123)
    cls = LocalFwegoServiceType
    cls.model_class = Mock()
...
def test_local_fwego_table_service_type_schema_name():
    mock_service = Mock(table_id=123)
    assert (
        LocalFwegoGetRowUserServiceType().get_schema_name(mock_service)
        == "Table123Schema"
    )
    assert (
        LocalFwegoListRowsUserServiceType().get_schema_name(mock_service)
        == "Table123Schema"
    )


def test_local_fwego_table_service_type_after_update_table_change_deletes_filters_and_sorts():
    mock_instance = Mock()
    ...
    service_type_cls = LocalFwegoTableServiceType
...
@pytest.mark.django_db
def test_local_fwego_table_service_type_get_context_data(data_fixture):
    user = data_fixture.create_user()
...
@pytest.mark.django_db
def test_local_fwego_table_service_type_get_context_data_schema(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    database = data_fixture.create_database_application(
        workspace=page.builder.workspace
...
@pytest.mark.django_db
def test_local_fwego_view_service_type_prepare_values(data_fixture):
    user = data_fixture.create_user()
    ...
    service_type = LocalFwegoViewServiceType
    service_type.model_class = Mock()
    instance = data_fixture.create_local_fwego_list_rows_service(
