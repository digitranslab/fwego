...
from fwego.contrib.database.views.models import SORT_ORDER_ASC, SORT_ORDER_DESC
from fwego.contrib.integrations.local_fwego.models import LocalFwegoListRows
...
from fwego.core.services.exceptions import ServiceImproperlyConfigured
...
def test_create_local_fwego_list_rows_service(data_fixture):
    ...
    service_type = service_type_registry.get("local_fwego_list_rows")
...
@pytest.mark.django_db
def test_export_import_local_fwego_list_rows_service(data_fixture):
    user = data_fixture.create_user()
    path_params = [
    ...
    service: LocalFwegoListRows = service_type.import_serialized(  # type: ignore
...
@pytest.mark.django_db
def test_update_local_fwego_list_rows_service(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    view = data_fixture.create_grid_view(user)
    integration = data_fixture.create_local_fwego_integration(
...
@pytest.mark.django_db
def test_local_fwego_list_rows_service_dispatch_transform(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table, fields, rows = data_fixture.build_table(
        user=user,
    ...
    service_type = LocalFwegoListRowsUserServiceType()
...
@pytest.mark.django_db
def test_local_fwego_list_rows_service_dispatch_data_permission_denied(
    ...
    with stub_check_permissions(raise_permission_denied=True), pytest.raises(
        ...
        LocalFwegoListRowsUserServiceType().dispatch_data(
            service, {"table": table}, dispatch_context
        )


@pytest.mark.django_db
def test_local_fwego_list_rows_service_before_dispatch_validation_error(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    integration = data_fixture.create_local_fwego_integration(
        application=page.builder, user=user
    ...
    with pytest.raises(ServiceImproperlyConfigured):
        LocalFwegoListRowsUserServiceType().resolve_service_formulas(
            service, dispatch_context
        )


@pytest.mark.django_db
def test_local_fwego_list_rows_service_dispatch_data_with_view_and_service_filters(
    data_fixture,
):
    user = data_fixture.create_user()
...
@pytest.mark.django_db
def test_local_fwego_list_rows_service_dispatch_data_with_varying_filter_types(
    data_fixture,
):
    user = data_fixture.create_user()
    builder = data_fixture.create_builder_application(user=user)
    ...
    cost_150 = data_fixture.create_local_fwego_table_service_filter(
...
@pytest.mark.django_db
def test_local_fwego_list_rows_service_dispatch_data_with_view_and_service_sorts(
    data_fixture,
):
    user = data_fixture.create_user()
    builder = data_fixture.create_builder_application(user=user)
...
@pytest.mark.django_db
def test_local_fwego_list_rows_service_dispatch_data_with_pagination(data_fixture):
    user = data_fixture.create_user()
    builder = data_fixture.create_builder_application(user=user)
    integration = data_fixture.create_local_fwego_integration(
        application=builder, user=user
...
@pytest.mark.django_db
def test_local_fwego_list_rows_service_import_context_path(data_fixture):
    local_fwego_list_rows_service = LocalFwegoListRowsUserServiceType()

    assert local_fwego_list_rows_service.import_context_path([], {}) == []
    assert local_fwego_list_rows_service.import_context_path(["id"], {}) == ["id"]
    assert local_fwego_list_rows_service.import_context_path(
...
@pytest.mark.django_db
def test_import_formula_local_fwego_list_rows_user_service_type(data_fixture):
    """
    Ensure that formulas are imported correctly when importing the
    LocalFwegoListRowsUserServiceType service type.
    """
...
def test_local_fwego_list_rows_user_service_type_import_path(
    path, database_fields, expected
):
    """
    Ensure the LocalFwegoListRowsUserServiceType::import_path() correctly
...
def test_dispatch_transform_passes_field_ids(mock_get_serializer, field_names):
    """
    Test the LocalFwegoListRowsUserServiceType::dispatch_transform() method.
