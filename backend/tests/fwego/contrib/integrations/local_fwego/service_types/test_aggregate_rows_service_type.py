...
from fwego.contrib.integrations.local_fwego.models import LocalFwegoAggregateRows
...
def test_local_fwego_aggregate_rows_service_get_schema_name():
    service_type = service_type_registry.get("local_fwego_aggregate_rows")
    assert service_type.get_schema_name(Mock(id=123)) == "Aggregation123Schema"


@pytest.mark.django_db
def test_local_fwego_aggregate_rows_service_generate_schema(data_fixture):
...
def test_local_fwego_aggregate_rows_resolve_service_formulas():
    service_type = service_type_registry.get("local_fwego_aggregate_rows")
...
@pytest.mark.django_db
def test_create_local_fwego_aggregate_rows_service(data_fixture):
    user = data_fixture.create_user()
...
@pytest.mark.django_db
def test_local_fwego_aggregate_rows_prepare_values_reset_field_when_table_change(
    data_fixture,
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    ...
    service_type = service_type_registry.get("local_fwego_aggregate_rows")
    service = LocalFwegoAggregateRows.objects.create(
        table=view.table,
        view=view,
        field=field,
    )
...
@pytest.mark.django_db
def test_local_fwego_aggregate_rows_prepare_values_incorrect_field(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    dashboard = page.builder
    table = data_fixture.create_database_table(user=user)
...
@pytest.mark.django_db
def test_local_fwego_aggregate_rows_prepare_values_incompatible_aggregation_type(
    data_fixture,
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_fwego_aggregate_rows_dispatch_data_with_table(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    dashboard = page.builder
    table = data_fixture.create_database_table(user=user)
    ...
    assert result["fwego_table_model"]
    assert result["data"] == {"result": Decimal("20")}


@pytest.mark.django_db
def test_local_fwego_aggregate_rows_dispatch_data_with_view(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    dashboard = page.builder
    table = data_fixture.create_database_table(user=user)
...
@pytest.mark.django_db
def test_local_fwego_aggregate_rows_dispatch_data_with_total(data_fixture):
    """
...
@pytest.mark.django_db
def test_local_fwego_aggregate_rows_dispatch_data_with_view_filters(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    dashboard = page.builder
    table = data_fixture.create_database_table(user=user)
...
@pytest.mark.django_db
def test_local_fwego_aggregate_rows_dispatch_data_with_service_filters(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    dashboard = page.builder
    table = data_fixture.create_database_table(user=user)
...
def test_local_fwego_aggregate_rows_dispatch_data_with_search(data_fixture):
...
@pytest.mark.django_db
def test_local_fwego_aggregate_rows_dispatch_data_field_deleted(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    dashboard = page.builder
    table = data_fixture.create_database_table(user=user)
...
@pytest.mark.django_db
def test_local_fwego_aggregate_rows_dispatch_data_field_trashed(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    dashboard = page.builder
    table = data_fixture.create_database_table(user=user)
...
@pytest.mark.django_db
def test_local_fwego_aggregate_rows_dispatch_data_field_type_not_compatible_anymore(
    data_fixture,
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
