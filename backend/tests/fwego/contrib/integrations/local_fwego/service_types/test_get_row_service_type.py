...
from fwego.contrib.builder.data_sources.service import DataSourceService
from fwego.contrib.builder.elements.registries import element_type_registry
from fwego.contrib.builder.elements.service import ElementService
from fwego.contrib.builder.pages.service import PageService
from fwego.contrib.database.api.rows.serializers import RowSerializer
from fwego.contrib.database.views.models import SORT_ORDER_ASC
from fwego.contrib.integrations.local_fwego.models import LocalFwegoGetRow
...
from fwego.core.exceptions import PermissionException
...
from fwego.core.utils import MirrorDict
from fwego.test_utils.helpers import AnyStr, setup_interesting_test_table
from fwego.test_utils.pytest_conftest import FakeDispatchContext, fake_import_formula


@pytest.mark.django_db
def test_create_local_fwego_get_row_service(data_fixture):
    ...
    service_type = LocalFwegoGetRowUserServiceType()
...
@pytest.mark.django_db
def test_export_import_local_fwego_get_row_service(data_fixture):
    user = data_fixture.create_user()
    path_params = [
    ...
    service_type = service_type_registry.get("local_fwego_get_row")
    integration = data_fixture.create_local_fwego_integration(
        application=page.builder, user=user
    )
    service = data_fixture.create_local_fwego_get_row_service(
    ...
    service_filter = data_fixture.create_local_fwego_table_service_filter(
    ...
    service: LocalFwegoGetRow = service_type.import_serialized(  # type: ignore
...
@pytest.mark.django_db
def test_update_local_fwego_get_row_service(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_fwego_get_row_service_dispatch_transform(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table, fields, rows = data_fixture.build_table(
        user=user,
    ...
    dispatch_values = LocalFwegoUpsertRowServiceType().resolve_service_formulas(
...
@pytest.mark.django_db
def test_local_fwego_get_row_service_dispatch_data_with_view_filter(data_fixture):
...
@pytest.mark.django_db
def test_local_fwego_get_row_service_dispatch_data_with_service_search(
...
@pytest.mark.django_db
def test_local_fwego_get_row_service_dispatch_data_with_service_integer_search(
    data_fixture, disable_full_text_search
):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
...
@pytest.mark.django_db
def test_local_fwego_get_row_service_dispatch_data_permission_denied(
    ...
    with stub_check_permissions(raise_permission_denied=True), pytest.raises(
        ...
        LocalFwegoGetRowUserServiceType().dispatch_data(
            service, {"table": table}, dispatch_context
        )


@pytest.mark.django_db
def test_local_fwego_get_row_service_dispatch_validation_error(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table = data_fixture.create_database_table(user=user)
    integration = data_fixture.create_local_fwego_integration(
...
@pytest.mark.django_db
def test_local_fwego_get_row_service_dispatch_data_row_not_exist(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table = data_fixture.create_database_table(user=user)
    integration = data_fixture.create_local_fwego_integration(
...
@pytest.mark.django_db
def test_local_fwego_get_row_service_dispatch_data_no_row_id(data_fixture):
    user = data_fixture.create_user()
    page = data_fixture.create_builder_page(user=user)
    table, fields, rows = data_fixture.build_table(
        user=user,
        ...
        rows=[
            ["Ada Lovelace", "ada@fwego.io"],
            ["Blaise Pascal", "blaise@fwego.io"],
        ],
    )
    integration = data_fixture.create_local_fwego_integration(
        application=page.builder, user=user
...
@pytest.mark.django_db
def test_import_formula_local_fwego_get_row_user_service_type(data_fixture):
    """
    Ensure that formulas are imported correctly when importing the
    LocalFwegoGetRowUserServiceType service type.

    The LocalFwegoGetRowUserServiceType::import_path() only supports 2 path
    parts, as opposed to LocalFwegoListRowsUserServiceType::import_path()
    which supports 3.
...
def test_local_fwego_get_row_user_service_type_import_path(
    path, database_fields, expected
):
    """
    Ensure the LocalFwegoGetRowUserServiceType::import_path() correctly
...
@pytest.mark.django_db
@patch("fwego.contrib.integrations.local_fwego.service_types.CoreHandler")
@pytest.mark.parametrize(
    "view_sorts",
    (
        [],
...
@patch(
    "fwego.contrib.integrations.local_fwego.service_types.get_row_serializer_class"
)
def test_dispatch_transform_passes_field_ids(mock_get_serializer, field_names):
    """
    Test the LocalFwegoGetRowUserServiceType::dispatch_transform() method.
...
def test_can_dispatch_interesting_table(data_fixture):
    ...
    # Test with a sort
    service_sort = data_fixture.create_local_fwego_table_service_sort(
