...
from fwego.contrib.dashboard.data_sources.dispatch_context import (
    DashboardDispatchContext,
)
from fwego.contrib.dashboard.data_sources.exceptions import (
    DashboardDataSourceDoesNotExist,
)
from fwego.contrib.dashboard.data_sources.models import DashboardDataSource
from fwego.contrib.dashboard.data_sources.service import DashboardDataSourceService
...
def test_get_data_sources(data_fixture, stub_check_permissions):
    ...
    data_source1 = data_fixture.create_dashboard_local_fwego_list_rows_data_source(
        dashboard=dashboard, user=user
    )
    data_source2 = data_fixture.create_dashboard_local_fwego_list_rows_data_source(
        dashboard=dashboard, user=user
    )
    data_source3 = (
        data_fixture.create_dashboard_local_fwego_aggregate_rows_data_source(
            dashboard=dashboard, user=user
        )
    )
...
@pytest.mark.django_db
@patch("fwego.contrib.dashboard.data_sources.service.dashboard_data_source_deleted")
...
@pytest.mark.django_db
@patch("fwego.contrib.dashboard.data_sources.service.dashboard_data_source_created")
...
@pytest.mark.django_db
@patch("fwego.contrib.dashboard.data_sources.service.dashboard_data_source_updated")
