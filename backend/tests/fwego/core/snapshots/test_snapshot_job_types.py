...
from fwego.core.jobs.constants import JOB_FAILED
from fwego.core.jobs.handler import JobHandler
from fwego.core.snapshots.job_types import CreateSnapshotJobType
from fwego.core.snapshots.models import Snapshot
...
def test_dangling_snapshots_are_removed(data_fixture):
    ...
    with freeze_time(
        timezone.now()
        + timedelta(minutes=settings.FWEGO_JOB_EXPIRATION_TIME_LIMIT + 10)
    ):
