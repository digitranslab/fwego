...
def test_get_fwego_saas_base_url_without_debug():
    assert get_fwego_saas_base_url() == ("https://api.fwego.io", {})


@override_settings(DEBUG=True)
def test_get_fwego_saas_base_url_with_debug():
    assert get_fwego_saas_base_url() == (
        "http://fwego-saas-backend:8000",
        {"Host": "localhost"},
    )
...
def test_are_hostnames_same():
    ...
    assert are_hostnames_same("fwego.io", "localhost") is False
