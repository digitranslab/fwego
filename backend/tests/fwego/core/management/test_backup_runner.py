...
from fwego.core.management.backup.backup_runner import FwegoBackupRunner
from fwego.core.management.backup.exceptions import InvalidFwegoBackupArchive
from fwego.core.trash.handler import TrashHandler


@pytest.mark.django_db(transaction=True)
@pytest.mark.once_per_day_in_ci
def test_can_backup_and_restore_fwego_reverting_changes(data_fixture, environ):
    runner = FwegoBackupRunner(
    ...
    with tempfile.TemporaryDirectory() as temporary_directory_name:
        ...
        runner.backup_fwego(backup_loc, 1)
        ...
        # backup.
        runner.restore_fwego(backup_loc, ["--clean", "--if-exists"])
...
def test_backup_fwego_dumps_database_in_batches(
    ...
    assert os.path.exists(f"fwego_backup_{dbname}_2020-01-02_12-00-00.tar.gz")
...
@patch("psycopg2.connect")
@patch("subprocess.check_output")
def test_can_change_num_jobs_and_insert_extra_args_for_fwego_backup(
    mock_check_output, mock_connect, mock_tempfile, fs, environ
):
    mock_pyscopg2_call_to_return(
        mock_connect,
...
@patch("psycopg2.connect")
@patch("subprocess.check_output")
def test_backup_fwego_table_batches_includes_all_tables_when_final_batch_small(
    mock_check_output, mock_connect, mock_tempfile, fs, environ
):
    mock_pyscopg2_call_to_return(
        mock_connect,
    ...
    with freeze_time("2020-01-02 12:00"):
        runner.backup_fwego(batch_size=3)
...
@patch("psycopg2.connect")
@patch("subprocess.check_output")
def test_backup_fwego_includes_all_tables_when_batch_size_matches_num_tables(
    mock_check_output, mock_connect, mock_tempfile, fs, environ
):
    ...
    with freeze_time("2020-01-02 12:00"):
        runner.backup_fwego(batch_size=len(tables_returned_by_sql))
...
@patch("psycopg2.connect")
@patch("subprocess.check_output")
def test_backup_fwego_does_no_table_batches_when_no_user_tables_found(
    mock_check_output, mock_connect, mock_tempfile, fs, environ
):
    mock_pyscopg2_call_to_return(
        mock_connect,
...
@patch("subprocess.check_output")
@patch("tarfile.open")
def test_restore_fwego_restores_contained_dumps_in_batches(
    ...
    runner.restore_fwego("backup.tar.gz")
...
@patch("subprocess.check_output")
@patch("tarfile.open")
def test_restore_fwego_passes_extra_args_to_all_pg_restores_and_can_set_jobs(
    mock_tarfile_open, mock_check_output, mock_tempfile, fs, environ
):
    mock_tempdir_to_be(fs, mock_tempfile, "/fake_tmp_dir/")
    fs.create_dir("/fake_tmp_dir/backup.tar.gz/everything_but_user_tables")
    ...
    extra_arg = "--extra-arg"
    runner.restore_fwego("backup.tar.gz", [extra_arg])
...
@patch("subprocess.check_output")
@patch("tarfile.open")
def test_restore_fwego_only_does_first_restore_if_no_user_tables(
    mock_tarfile_open, mock_check_output, mock_tempfile, fs, environ
):
    mock_tempdir_to_be(fs, mock_tempfile, "/fake_tmp_dir/")
    fs.create_dir("/fake_tmp_dir/backup.tar.gz/everything_but_user_tables")
...
@patch("subprocess.check_output")
@patch("tarfile.open")
def test_restore_fwego_raises_exception_if_sub_folder_not_found_after_extract(
    mock_tarfile_open, mock_check_output, mock_tempfile, fs, environ
):
    mock_tempdir_to_be(fs, mock_tempfile, "/fake_tmp_dir/")
    ...
    with pytest.raises(InvalidFwegoBackupArchive):
        runner.restore_fwego("backup.tar.gz")
