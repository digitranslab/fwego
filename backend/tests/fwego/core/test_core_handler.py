...
from fwego.contrib.database.models import Database, Table
from fwego.contrib.database.views.models import GridView, GridViewFieldOptions
...
from fwego.core.operations import ReadWorkspaceOperationType
from fwego.core.registries import ImportExportConfig, plugin_registry
...
@pytest.mark.django_db
@patch("fwego.core.signals.workspace_user_updated.send")
...
@pytest.mark.django_db
@patch("fwego.core.signals.workspace_user_deleted.send")
...
@pytest.mark.django_db
@patch("fwego.core.signals.workspace_created.send")
...
@pytest.mark.django_db
@patch("fwego.core.signals.workspace_restored.send")
...
@pytest.mark.django_db
@patch("fwego.core.signals.workspace_updated.send")
...
@pytest.mark.django_db
@patch("fwego.core.signals.workspace_deleted.send")
...
@pytest.mark.django_db
@patch("fwego.core.signals.workspaces_reordered.send")
...
def test_send_workspace_invitation_email(data_fixture, mailoutbox):
    ...
    assert (
        ...
        f"to {workspace_invitation.workspace.name} - Fwego"
    )
...
def test_send_workspace_invitation_email_in_different_language(
    ...
    assert (
        ...
        f"{workspace_invitation.workspace.name} - Fwego"
    )


@pytest.mark.django_db
@patch("fwego.core.handler.CoreHandler.send_workspace_invitation_email")
...
@pytest.mark.django_db
@patch("fwego.core.handler.CoreHandler.send_workspace_invitation_email")
@override_settings(FWEGO_MAX_PENDING_WORKSPACE_INVITES=1)
...
@pytest.mark.django_db
@patch("fwego.core.signals.application_created.send")
...
@pytest.mark.django_db
@patch("fwego.core.signals.application_updated.send")
...
@pytest.mark.django_db
@patch("fwego.core.signals.applications_reordered.send")
...
@pytest.mark.django_db
@patch("fwego.core.signals.application_deleted.send")
