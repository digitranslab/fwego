...
from fwego.contrib.database.fields.models import SelectOption
...
from fwego.core.models import BlacklistedToken, UserLogEntry, Workspace, WorkspaceUser
from fwego.core.registries import plugin_registry
from fwego.core.user.exceptions import (
    DisabledSignupError,
...
def test_send_reset_password_email(data_fixture, mailoutbox):
    ...
    assert email.subject == "Reset password - Fwego"
...
def test_send_reset_password_email_in_different_language(data_fixture, mailoutbox):
    ...
    assert len(mailoutbox) == 1
    assert mailoutbox[0].subject == "Réinitialiser le mot de passe - Fwego"
...
def test_schedule_user_deletion(data_fixture, mailoutbox):
    ...
    assert len(mailoutbox) == 1
    assert mailoutbox[0].subject == "Account deletion scheduled - Fwego"
...
def test_cancel_user_deletion(data_fixture, mailoutbox):
    ...
    assert len(mailoutbox) == 1
    assert mailoutbox[0].subject == "Account deletion cancelled - Fwego"
...
def test_delete_expired_users_and_related_workspaces_if_last_admin(
    ...
    assert mailoutbox[0].subject == "Account permanently deleted - Fwego"
...
@pytest.mark.django_db(transaction=True)
@override_settings(FWEGO_EMBEDDED_SHARE_HOSTNAME="http://test/")
...
@pytest.mark.django_db
@patch("fwego.core.user.handler.share_onboarding_details_with_fwego")
def test_start_share_onboarding_details_with_fwego(mock_task, data_fixture):
    user = data_fixture.create_user()

    UserHandler().start_share_onboarding_details_with_fwego(
...
@responses.activate
def test_share_onboarding_details_with_fwego_valid_response(data_fixture):
    data_fixture.update_settings(instance_id="1")

    response1 = responses.add(
        responses.POST,
        "http://fwego-saas-backend:8000/api/saas/onboarding/additional-details/",
        status=204,
        match=[
            json_params_matcher(
                {
    ...
    UserHandler().share_onboarding_details_with_fwego(
        email="test@test.nl",
        team="Marketing",
        role="CEO",
        size="11 - 50",
