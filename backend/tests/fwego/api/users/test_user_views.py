...
from fwego.api.user.registries import UserDataType, user_data_registry
...
def test_share_onboarding_details_with_fwego(mock_task, client, data_fixture):
    data_fixture.update_settings(instance_id="1")
    user, token = data_fixture.create_user_and_token()

    response = client.post(
        reverse("api:user:share_onboarding_details_with_fwego"),
        {
            "team": "Marketing",
            "role": "CEO",
            "size": "11 - 50",
