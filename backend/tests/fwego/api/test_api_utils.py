...
from fwego.api.exceptions import QueryParameterValidationException
from fwego.api.registries import RegisteredException, api_exception_registry
...
from fwego.throttling import (
    FWEGO_CONCURRENCY_THROTTLE_REQUEST_ID,
...
@override_settings(
    ...
    FWEGO_CONCURRENT_USER_REQUESTS_THROTTLE_TIMEOUT=30,
)
@patch("fwego.throttling._get_redis_cli", lambda: FakeRedis(server=fake_redis_server))
@pytest.mark.django_db
...
@patch("fwego.throttling.ConcurrentUserRequestsThrottle.on_request_processed")
@patch("fwego.throttling._get_redis_cli", lambda: FakeRedis(server=fake_redis_server))
@pytest.mark.django_db
def test_throttle_set_fwego_concurrency_throttle_request_id_and_middleware_can_get_it(
    ...
    from fwego.api.user.views import DashboardView
    ...
    assert getattr(request, FWEGO_CONCURRENCY_THROTTLE_REQUEST_ID, None) is not None
