...
from fwego.api.settings.registries import SettingsDataRegistry, SettingsDataType
from fwego.core.handler import CoreHandler
from fwego.core.models import Settings
...
@pytest.mark.django_db
def test_update_show_fwego_help_request(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token(is_staff=True)

    response = api_client.patch(
        reverse("api:settings:update"),
        {"show_fwego_help_request": False},
        format="json",
        HTTP_AUTHORIZATION=f"JWT {token}",
    )
    assert response.status_code == HTTP_200_OK
    response_json = response.json()
    assert response_json["show_fwego_help_request"] is False
...
def test_register_settings_data_type(api_client, data_fixture):
    ...
    with patch("fwego.api.settings.views.settings_data_registry", new=registry):
        response = api_client.get(reverse("api:settings:get"))
        assert response.status_code == HTTP_200_OK
        response_json = response.json()
