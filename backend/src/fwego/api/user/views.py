...
from fwego.api.actions.serializers import (
...
from fwego.api.sessions import (
...
from fwego.api.workspaces.invitations.errors import (
    ERROR_GROUP_INVITATION_DOES_NOT_EXIST,
    ERROR_GROUP_INVITATION_EMAIL_MISMATCH,
)
from fwego.core.action.handler import ActionHandler
from fwego.core.action.registries import ActionScopeStr, action_type_registry
from fwego.core.auth_provider.exceptions import (
...
from fwego.core.models import Settings, Template, WorkspaceInvitation
...
from fwego.core.user.utils import generate_session_tokens_for_user
...
from .serializers import (
    ...
    ShareOnboardingDetailsWithFwegoSerializer,
...
class RefreshJSONWebToken(TokenRefreshView):
    ...
    @extend_schema(
        ...
        description=(
            "Generate a new access_token that can be used to continue operating on Fwego "
...
class ShareOnboardingDetailsWithFwegoView(APIView):
    permission_classes = (IsAuthenticated,)

    @extend_schema(exclude=True)
    @transaction.atomic
    @validate_body(ShareOnboardingDetailsWithFwegoSerializer)
    def post(self, request, data):
