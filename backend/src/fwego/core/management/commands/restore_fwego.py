...
class Command(BaseCommand):
    help = """
        Restores a Fwego database back_up file generated by the Fwego
        management command backup_fwego. Only restoring into existing blank databases
        ...
        being updated, not connected to a running version of Fwego for the
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            required=True,
            dest="file",
            help="Restores from the specified file produced by the fwego "
            "management command backup_fwego.",
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            runner.restore_fwego(file, additional_args)
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully restored Fwego to the database called: {database} "
                )
            )
