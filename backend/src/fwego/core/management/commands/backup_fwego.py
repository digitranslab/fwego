...
from fwego.core.management.backup.backup_runner import (
    FwegoBackupRunner,
...
class Command(BaseCommand):
    help = """
        Backs up a Fwego database into a single compressed archive which can be
        restored using the restore_fwego Fwego management command.
        ...
        being updated and not connected to a running version of Fwego for the
        ...
        safe you should only perform back-up's when your Fwego server is shut down
        or you have first copied the database to a new cluster which is not in active
        use.

        The back-up is split into batches as often Fwego database's can end up with
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            "`fwego_backup_{database}_{datetime}.tar.gz`",
        )
    ...
    def handle(self, *args, **options):
        ...
        try:
            backup_file_name = runner.backup_fwego(file, batch_size, additional_args)
            self.stdout.write(
                self.style.SUCCESS(
                    f"Successfully backed up Fwego to {backup_file_name} which can "
                    f"be restored using the ./fwego restore_fwego command. "
                )
            )
        ...
        except InvalidFwegoBackupArchive:
            raise CommandError(
                "Please ensure the provided back-up file is a valid "
                "Fwego backup file produced by ./fwego "
                "backup_fwego"
            )
