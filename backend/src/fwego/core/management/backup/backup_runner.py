...
from fwego.contrib.database.table.constants import USER_TABLE_DATABASE_NAME_PREFIX
...
class FwegoBackupRunner:
    """
    A class which can backup or restore the database used by an installation of
    Fwego. Under the hood it uses `pg_dump` and `pg_restore` commands.
    """
    ...
    def __init__(
        ...
        """
        Constructs a FwegoBackupRunner.
    ...
    def backup_fwego(
        ...
        """
        Backs up an entire Database containing an installation of Fwego.
        ...
        :raises InvalidFwegoBackupArchive: When the provided backup archive does not
            meet the expected fwego backup format.
...
def _get_sorted_user_tables_names(conn) -> List[str]:
    """
    Queries the provided connection for the table and schema names of Fwego's user
...
def _default_backup_location(database):
    ...
    return f"fwego_backup_{database}_{now}.tar.gz"


def add_shared_postgres_command_args(parser):
    """
    The DB connection parameters are shared between the backup_fwego and
    restore_fwego commands.
    """
