...
def clean_up_user_log_entry(self):
    ...
    older_than_days = timedelta(days=settings.FWEGO_USER_LOG_ENTRY_RETENTION_DAYS)
...
@app.task(bind=True, queue="export")
def share_onboarding_details_with_fwego(self, **kwargs):
    """
    Task wrapper that calls the `share_onboarding_details_with_fwego` method.
    """

    from fwego.core.user.handler import UserHandler

    UserHandler().share_onboarding_details_with_fwego(**kwargs)
...
def setup_periodic_tasks(sender, **kwargs):
    ...
    sender.add_periodic_task(
        timedelta(minutes=settings.FWEGO_USER_LOG_ENTRY_CLEANUP_INTERVAL_MINUTES),
