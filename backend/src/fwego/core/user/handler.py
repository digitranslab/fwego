...
from fwego.core.auth_provider.handler import PasswordProviderHandler
from fwego.core.auth_provider.models import AuthProviderModel
from fwego.core.emails import EmailPendingVerificationEmail
from fwego.core.exceptions import (
...
from fwego.core.signals import (
    before_user_deleted,
    user_deleted,
...
from fwego.core.utils import generate_hash, get_fwego_saas_base_url
from fwego.throttling import rate_limit

from ..telemetry.utils import fwego_trace_methods
from .emails import (
    AccountDeleted,
...
from .tasks import share_onboarding_details_with_fwego
...
class UserHandler(metaclass=fwego_trace_methods(tracer)):
    def get_active_user(
        self,
    ...
    def create_user(
        ...
        user = self.force_create_user(
            ...
            name=name,
            password=password,
            # This is the first ever user created in this fwego instance and
            # therefore the administrator user, lets give them staff rights so they
            # can set fwego wide settings.
    ...
    def send_reset_password_email(self, user: AbstractUser, base_url: str):
        ...
        if parsed_base_url.hostname not in (
            ...
            settings.FWEGO_EMBEDDED_SHARE_HOSTNAME,
        ):
    ...
    def get_all_active_users_qs(self) -> QuerySet:
        ...
        Fwego instance. Will filter out users who have be "banned/deactivated" by an
    ...
    def send_email_pending_verification(self, user: User):
        ...
        base_url = (
            settings.FWEGO_EMBEDDED_SHARE_URL or settings.PUBLIC_WEB_FRONTEND_URL
        )
        if not base_url.endswith("/"):
            base_url += "/"
        ...
        rate_limit(
            rate=settings.FWEGO_SEND_VERIFY_EMAIL_RATE_LIMIT,
            key=user.username,
            raise_exception=False,
        )(send_email)()

    def start_share_onboarding_details_with_fwego(
        self, user, team: str, role: str, size: str, country: str
    ):
        """
        Starts a celery task that shares some user information with fwego.io. Note
        ...
        share_onboarding_details_with_fwego.delay(
            email=email, team=team, role=role, size=size, country=country
        )

    def share_onboarding_details_with_fwego(self, email, team, role, size, country):
        """
        Makes an API request to fwego.io that shares the additional information. Note
        that this is only triggered if the user given permission during the onboarding
        in the web-frontend.
        ...
        base_url, headers = get_fwego_saas_base_url()
        ...
        try:
            ...
            logger.warning(
                "The onboarding details could not be shared with the Fwego team "
