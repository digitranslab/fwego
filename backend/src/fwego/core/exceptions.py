...
class MaxNumberOfPendingWorkspaceInvitesReached(Exception):
    ...
    This value is configurable via the `FWEGO_MAX_PENDING_WORKSPACE_INVITES` setting.
    """
...
class LockConflict(Exception):
    ...
    Fwego has been locked for some operation.
    """
...
class MaxLocksPerTransactionExceededException(Exception):
    """
    Fwego can sometimes perform operations that require a Postgres LOCK on a large
    ...
    message = (
        "Fwego has exceeded the maximum number of PostgreSQL locks per transaction. "
        "Please read https://fwego.io/docs/technical/postgresql-locks"
    )
...
class DuplicateApplicationMaxLocksExceededException(
    ...
    message = (
        "Fwego attempted to duplicate an application, but exceeded the maximum "
        "number of PostgreSQL locks per transaction. Please read "
        "https://fwego.io/docs/technical/postgresql-locks"
    )
