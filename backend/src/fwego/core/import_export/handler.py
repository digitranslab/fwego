...
from fwego.config.settings.base import FWEGO_DEFAULT_ZIP_COMPRESS_LEVEL
...
from fwego.core.import_export.utils import chunk_generator
from fwego.core.jobs.constants import JOB_FINISHED
...
from fwego.core.signals import application_created
from fwego.core.storage import (
    ExportZipFile,
...
from fwego.core.utils import ChildProgressBuilder, Progress, grouper, stream_size
...
class ImportExportHandler(metaclass=fwego_trace_methods(tracer)):
    ...
    def export_workspace_applications(
        ...
        zip_file = ExportZipFile(
            compress_level=FWEGO_DEFAULT_ZIP_COMPRESS_LEVEL,
    ...
    def validate_uploaded_file(self, stream: IOBase):
        ...
        if size > settings.FWEGO_FILE_UPLOAD_SIZE_LIMIT_MB:
            raise FileSizeTooLargeError(
                settings.FWEGO_FILE_UPLOAD_SIZE_LIMIT_MB,
    ...
    def mark_resource_for_deletion(
        ...
        user: AbstractUser,
        resource_id: str,
        delete_used_resources_after_days: int = settings.FWEGO_IMPORT_EXPORT_RESOURCE_REMOVAL_AFTER_DAYS,
    ):
        """
