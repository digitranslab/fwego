...
from fwego.contrib.database.db.schema import optional_atomic
...
def get_fwego_saas_base_url() -> [str, dict]:
    """
    Returns the base url of the Fwego SaaS host. In production we always want to
    connect to api.fwego.io, but in development to the saas dev env for testing
    purposes.

    :return: The base url and the headers object that must be added to the request.
    """

    base_url = "https://api.fwego.io"
    headers = {}

    if settings.DEBUG:
        base_url = "http://fwego-saas-backend:8000"
