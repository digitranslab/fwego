...
from fwego.core.telemetry.provider import DifferentSamplerPerLibraryTracerProvider
from fwego.core.telemetry.utils import BatchBaggageSpanProcessor, otel_is_enabled
...
def setup_logging():
    ...
    logger.add(
        sys.stderr, format=loguru_format, level=settings.FWEGO_BACKEND_LOG_LEVEL
    )
...
def setup_telemetry(add_django_instrumentation: bool):
    """
    Sets up logging and when the env var FWEGO_ENABLE_OTEL is set to any non-blank
    ...
    if otel_is_enabled():
        ...
        print("Not configuring telemetry due to FWEGO_ENABLE_OTEL not being set.")
...
def _setup_log_exporting(logger):
    ...
    handler = LogGuruCompatibleLoggerHandler(
        level=settings.FWEGO_BACKEND_LOG_LEVEL,
        logger_provider=logger_provider,
    )
    logger_provider.add_log_record_processor(BatchLogRecordProcessor(exporter))
    logger.add(handler, format="{message}", level=settings.FWEGO_BACKEND_LOG_LEVEL)
