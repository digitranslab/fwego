...
# fwego manually.
FWEGO_OTEL_TRACE_ATTR_PREFIX = "fwego."
...
def _fwego_trace_func(wrapped_func, tracer: Tracer):
        ...
        def __new__(cls, name, bases, local):
            for attr in local:
                ...
                if inspect.isfunction(value):
                    local[attr] = _fwego_trace_func(value, tracer)
...
def fwego_trace(tracer):
    ...
    if not isinstance(tracer, Tracer):
        raise Exception(
            f"Must provider a tracer to fwego_trace, instead you gave me a "
            f"{type(tracer)}. Get "
            "one using "
    ...
    def inner(wrapped_function_or_cls):
        return _fwego_trace_func(wrapped_function_or_cls, tracer)

    return inner


def add_fwego_trace_attrs(**kwargs):
    """
    Simple helper function for quickly adding attributes to the current span. The
    attribute names will be prefixed with the fwego. to namespace them properly.

    :param kwargs: Key value pairs, the key will be the attr name prefixed with
        fwego. and the value will be the span attribute value.
    """

    span = get_current_span()
    for key, value in kwargs.items():
        span.set_attribute(f"{FWEGO_OTEL_TRACE_ATTR_PREFIX}{key}", value)


def otel_is_enabled():
    env_var_set = bool(os.getenv("FWEGO_ENABLE_OTEL", False))
    not_in_tests = (
        os.getenv("DJANGO_SETTINGS_MODULE", "").strip()
        != "fwego.config.settings.test"
    )
