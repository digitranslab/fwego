...
class TrashableItemType(ModelInstanceMixin, Instance, ABC):
    """
    A TrashableItemType specifies a fwego model which can be trashed.
    """
...
class TrashableItemTypeRegistry(ModelRegistryMixin, Registry):
    """
    The TrashableItemTypeRegistry contains models which can be "trashed" in fwego.
    When an instance of a trashable model is trashed it is removed from fwego but
    not permanently. Once trashed an item can then be restored to add it back to
    fwego just as it was when it was trashed.
    """
