...
from .user.tasks import (
    ...
    share_onboarding_details_with_fwego,
)
...
@app.task(
    bind=True,
    queue="export",
    time_limit=settings.FWEGO_SYNC_TEMPLATES_TIME_LIMIT,
)
def sync_templates_task(self):
    from fwego.core.handler import CoreHandler

    CoreHandler().sync_templates(pattern=settings.FWEGO_SYNC_TEMPLATES_PATTERN)
