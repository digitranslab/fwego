...
class Settings(models.Model):
    ...
    show_fwego_help_request = models.BooleanField(
        default=True,
        help_text=(
    ...
    co_branding_logo = models.ForeignKey(
        ...
        related_name="+",
        help_text="Co-branding logo that's placed next to the Fwego logo (176x29).",
    )
...
class TrashEntry(models.Model):
    """
    A TrashEntry is a record indicating that another model in Fwego has a trashed
    row. When a user deletes certain things in Fwego they are not actually deleted
...
class DuplicateApplicationJob(
    ...
    original_application = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        help_text="The Fwego application to duplicate.",
    )
    duplicated_application = models.OneToOneField(
        ...
        on_delete=models.SET_NULL,
        help_text="The duplicated Fwego application.",
    )
