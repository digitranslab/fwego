...
from fwego.core.utils import random_string, sha256_hash, stream_size, truncate_middle
    ...
    def upload_user_file_by_url(self, user, url, storage=None):
        ...
        try:
            ...
            try:
                ...
                if content_length > settings.FWEGO_FILE_UPLOAD_SIZE_LIMIT_MB:
                    raise FileSizeTooLargeError(
                        settings.FWEGO_FILE_UPLOAD_SIZE_LIMIT_MB,
                        "The provided file is too large.",
                    )
            ...
            for chunk in response.iter_content(chunk_size=None):
                content += chunk
                if len(content) > settings.FWEGO_FILE_UPLOAD_SIZE_LIMIT_MB:
                    response.close()
                    raise FileSizeTooLargeError(
                        settings.FWEGO_FILE_UPLOAD_SIZE_LIMIT_MB,
                        "The provided file is too large.",
