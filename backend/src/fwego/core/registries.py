...
from fwego.contrib.database.constants import IMPORT_SERIALIZED_IMPORTING
from fwego.core.auth_provider.registries import AuthenticationProviderTypeRegistry
from fwego.core.exceptions import SubjectTypeNotExist
...
class Plugin(APIUrlsInstanceMixin, Instance):
    ...
    Example:
        ...
        from fwego.core.registries import Plugin, plugin_registry
...
class PluginRegistry(APIUrlsRegistryMixin, Registry):
    ...
    abstraction made specifically for Fwego. It allows a plugin developer to
...
class ApplicationType(
    ...
    Example:
        ...
        from fwego.core.registries import ApplicationType, application_type_registry
    ...
    def export_safe_transaction_context(self, application: "Application") -> Atomic:
        ...
        fwego.contrib.database.db.atomic.read_repeatable_single_database_atomic_transaction)
...
class ApplicationTypeRegistry(
    ...
    application is an abstraction made specifically for Fwego. If added to the
...
class PermissionManagerTypeRegistry(Registry[PermissionManagerType]):
    ...
    Fwego. A permission manager must then be listed in the
...
class ObjectScopeType(Instance, ModelInstanceMixin):
    """
    This type describe an object scope in Fwego. This is useful if you need to know
...
class SubjectType(abc.ABC, Instance, ModelInstanceMixin):
    """
    This type describes a subject that exists in Fwego. A subject is anything that
    ...
    def get_users_included_in_subject(self, subject) -> List["AbstractUser"]:
        ...
        And associated user is any user that receives permissions in Fwego based
...
class SubjectTypeRegistry(Registry[SubjectType], ModelRegistryMixin[Any, SubjectType]):
    """
    This registry holds all the different subject types used across Fwego.
    """
...
class OperationType(abc.ABC, Instance):
    ...
    def context_scope_name(cls) -> str:
        """
        An operation is executed on a context in Fwego. For example the list_fields
...
# used throughout the whole Fwego application. To add a new plugin or application use
# these registries.
