...
from fwego.core.utils import exception_capturer


class InvalidFwegoFormula(Exception):
...
def formula_exception_handler(e):
    ...
    logger.error(
        f"Formula related error occurred: {e}. Please send this error to the fwego "
        f"developers at https://fwego.io/contact."
    )
    logger.exception(e)
