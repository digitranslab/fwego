...
from fwego.core.notifications.models import Notification
from fwego.core.notifications.registries import notification_type_registry
from fwego.core.registries import email_context_registry
...
class BaseEmailMessage(EmailMultiAlternatives):
    ...
    Example:
        class TestEmail(BaseEmailMessage):
            ...
            template_name = 'fwego/core/example.html'
...
class EmailPendingVerificationEmail(BaseEmailMessage):
    template_name = "fwego/core/user/email_pending_verification.html"
...
class WorkspaceInvitationEmail(BaseEmailMessage):
    template_name = "fwego/core/workspace_invitation.html"
    ...
    def get_subject(self):
        return _(
            "%(by)s invited you to %(workspace_name)s - Fwego",
        ) % {
...
class NotificationsSummaryEmail(BaseEmailMessage):
    template_name = "fwego/core/notifications_summary.html"
    ...
    def get_subject(self):
        ...
        if count == 1:
            return _("You have 1 new notification - Fwego")

        return _("You have %(count)d new notifications - Fwego") % {"count": count}
