...
from fwego.core.user.utils import normalize_email_address
...
from .telemetry.utils import fwego_trace_methods, disable_instrumentation
...
class CoreHandler(metaclass=fwego_trace_methods(tracer)):
    ...
    def create_workspace_invitation(
        ...
        max_invites = settings.FWEGO_MAX_PENDING_WORKSPACE_INVITES
    ...
    def sync_templates(self, storage=None, pattern: str | None = None):
        ...
        sync_templates_import_export_config = ImportExportConfig(
            ...
            # Without reducing disk space usage Fwego after first time install
        ...
        for template_file_path in tqdm(
            template_files_paths,
            desc="Syncing Fwego templates. Disable by setting "
    ...
    def _sync_template(
        ...
        if "fwego_template_version" not in parsed_json:
            return
