...
def patch_cachalot_for_fwego():
    """
    This function patches the cachalot library to make it work with fwego
    ...
    fwego creates dynamic models on the fly so we can't know what tables will
    ...
    fwego_table_names_regex = re.compile(
        r"^(?:{}|{}|{}|{})\d+".format(
            USER_TABLE_DATABASE_NAME_PREFIX,
            LINK_ROW_THROUGH_TABLE_PREFIX,
            MULTIPLE_COLLABORATOR_THROUGH_TABLE_PREFIX,
    ...
    def patched_is_cachable(table):
        return is_fwego_table(table) or original_is_cachable(table)
    ...
    def patched_get_tables_from_sql(
        ...
        fwego_tables = fwego_tables_regex.findall(lowercased_sql)
        return set(fwego_tables) | original_get_tables_from_sql(
