...
from fwego.contrib.builder.data_providers.exceptions import (
    DataProviderChunkInvalidException,
)
from fwego.contrib.database.api.fields.serializers import FieldSerializer
from fwego.contrib.database.api.rows.serializers import (
    RowSerializer,
    get_row_serializer_class,
)
from fwego.contrib.database.api.utils import extract_field_ids_from_list
from fwego.contrib.database.fields.exceptions import (
    FieldDoesNotExist,
    IncompatibleField,
)
...
from fwego.contrib.database.table.service import TableService
from fwego.contrib.database.views.exceptions import (
    AggregationTypeDoesNotExist,
    ViewDoesNotExist,
)
from fwego.contrib.database.views.service import ViewService
from fwego.contrib.integrations.local_fwego.api.serializers import (
    LocalFwegoTableServiceFieldMappingSerializer,
)
from fwego.contrib.integrations.local_fwego.integration_types import (
    LocalFwegoIntegrationType,
)
from fwego.contrib.integrations.local_fwego.mixins import (
    LocalFwegoTableServiceFilterableMixin,
    LocalFwegoTableServiceSearchableMixin,
    LocalFwegoTableServiceSortableMixin,
    LocalFwegoTableServiceSpecificRowMixin,
)
...
from fwego.core.services.dispatch_context import DispatchContext
from fwego.core.services.exceptions import DoesNotExist, ServiceImproperlyConfigured
from fwego.core.services.registries import (
    DispatchTypes,
    ListServiceTypeMixin,
    ServiceType,
)
from fwego.core.services.types import (
    ServiceDict,
    ServiceFilterDictSubClass,
    ServiceSortDictSubClass,
    ServiceSubClass,
)
from fwego.core.utils import atomic_if_not_already

if TYPE_CHECKING:
    from fwego.contrib.database.table.models import GeneratedTableModel, Table


class LocalFwegoServiceType(ServiceType):
    """
    The `ServiceType` for all `LocalFwego` integration services.
    """
...
class LocalFwegoTableServiceType(LocalFwegoServiceType):
    """
    The `ServiceType` for `LocalFwegoTableService` subclasses.
    """
    ...
    serializer_field_overrides = {
        "table_id": serializers.IntegerField(
            required=False,
            allow_null=True,
            help_text="The id of the Fwego table we want the data for.",
        ),
        "integration_id": serializers.IntegerField(
            required=False,
            allow_null=True,
            help_text="The id of the Fwego integration we want the data for.",
        ),
    }

    # Three lists which represent the Fwego `FieldType` which are not permitted
    ...
    def build_queryset(
        self,
        service: LocalFwegoTableService,
        table: "Table",
    ...
    def resolve_service_formulas(
        ...
        """
        A hook called before the `LocalFwegoTableServiceType` subclass dispatch
        calls. It ensures we check the service has a `Table` before execution.

        :param service: A `LocalFwegoTableService` instance.
    ...
    def import_property_name(
        ...
        """
        Responsible for taking a Local Fwego property name (`Field.db_column`),
    ...
    def create_instance_from_serialized(
        ...
        **kwargs,
        ...
        # Create sortings
        LocalFwegoTableServiceSort.objects.bulk_create(
            [
                LocalFwegoTableServiceSort(
                    **service_sorting,
                    order=index,
    ...
    def update_service_sortings(
        self,
        service: Union[LocalFwegoGetRow, LocalFwegoListRows],
    ...
    def prepare_values(
        ...
        # added it in `LocalFwegoViewServiceType.prepare_values`.
    ...
    def generate_schema(self, service: ServiceSubClass) -> Optional[Dict[str, Any]]:
        ...
        :param service: A `LocalFwegoTableService` subclass.
    ...
    def get_schema_name(self, service: ServiceSubClass) -> str:
        """
        The default `LocalFwegoTableService` schema name.
    ...
    def get_used_field_names(
        self, service: LocalFwegoTableService, dispatch_context: DispatchContext
    ) -> Optional[List[str]]:
        """
    ...
    def get_table_model(
        self, service: LocalFwegoTableService
    ...
    def get_table_field_objects(self, service: LocalFwegoTableService) -> List[Dict]:
        """
    ...
    def get_json_type_from_response_serializer_field(
        ...
        :param field: The Fwego Field we want a type for.
        :param field_type: The Fwego FieldType we want a type for.
...
class LocalFwegoViewServiceType(LocalFwegoTableServiceType):
    """
    The `ServiceType` for `LocalFwegoViewService` subclasses.
    """
        ...
        return {
            **super().serializer_field_overrides,
            "view_id": serializers.IntegerField(
                required=False,
                allow_null=True,
                help_text="The id of the Fwego view we want the data for.",
            ),
        }

    class SerializedDict(LocalFwegoTableServiceType.SerializedDict):
        view_id: int
...
class LocalFwegoListRowsUserServiceType(
    ListServiceTypeMixin,
    LocalFwegoTableServiceSearchableMixin,
    LocalFwegoTableServiceFilterableMixin,
    LocalFwegoTableServiceSortableMixin,
    LocalFwegoViewServiceType,
):
    """
    This service gives access to a list of rows from the same Fwego instance as the
    one hosting the application.
    """

    integration_type = LocalFwegoIntegrationType.type
    type = "local_fwego_list_rows"
    model_class = LocalFwegoListRows
    ...
    def simple_formula_fields(self):
        return (
            ...
            + LocalFwegoTableServiceSearchableMixin.mixin_simple_formula_fields
        )
    ...
    def allowed_fields(self):
        return (
            ...
            + LocalFwegoTableServiceFilterableMixin.mixin_allowed_fields
            + LocalFwegoTableServiceSearchableMixin.mixin_allowed_fields
        )
    ...
    def serializer_field_names(self):
        return (
            ...
            + LocalFwegoTableServiceSortableMixin.mixin_serializer_field_names
            + LocalFwegoTableServiceFilterableMixin.mixin_serializer_field_names
            + LocalFwegoTableServiceSearchableMixin.mixin_serializer_field_names
        )
    ...
    def serializer_field_overrides(self):
        return {
            **super().serializer_field_overrides,
            **LocalFwegoTableServiceSortableMixin.mixin_serializer_field_overrides,
            **LocalFwegoTableServiceSearchableMixin.mixin_serializer_field_overrides,
            **LocalFwegoTableServiceFilterableMixin.mixin_serializer_field_overrides,
        }

    class SerializedDict(
        LocalFwegoViewServiceType.SerializedDict,
        LocalFwegoTableServiceSortableMixin.SerializedDict,
        LocalFwegoTableServiceSearchableMixin.SerializedDict,
        LocalFwegoTableServiceFilterableMixin.SerializedDict,
    ):
        pass
    ...
    def serialize_property(
        ...
        :param service: The LocalFwegoListRows service.
    ...
    def dispatch_data(
        ...
        :param service: the local fwego get row service.
    ...
    def dispatch_transform(self, dispatch_data: Dict[str, Any]) -> Any:
        ...
        serializer = get_row_serializer_class(
            dispatch_data["fwego_table_model"],
            RowSerializer,
            is_response=True,
            field_ids=field_ids,
        )
    ...
    def get_record_names(
        ...
        try:
            ...
            # See: https://github.com/digitranslab/fwego/-/issues/3062
...
class LocalFwegoAggregateRowsUserServiceType(
    LocalFwegoTableServiceSearchableMixin,
    LocalFwegoTableServiceFilterableMixin,
    LocalFwegoViewServiceType,
):
    """
    This service gives access to aggregations over fields in a Fwego table or view.
    """

    integration_type = LocalFwegoIntegrationType.type
    type = "local_fwego_aggregate_rows"
    model_class = LocalFwegoAggregateRows
    dispatch_type = DispatchTypes.DISPATCH_DATA_SOURCE

    def get_schema_name(self, service: LocalFwegoAggregateRows) -> str:
        """
        The Local Fwego aggregation schema name added to the `title` in
    ...
    def get_context_data(self, service: LocalFwegoAggregateRows) -> None:
        """
        The Local Fwego aggregate rows service type does not provide any
    ...
    def get_context_data_schema(self, service: LocalFwegoAggregateRows) -> None:
        """
        The Local Fwego aggregate rows service type does not provide any
        ...
        return {
            ...
            **LocalFwegoTableServiceFilterableMixin.mixin_serializer_field_overrides,
            "field_id": serializers.IntegerField(
                required=False,
                allow_null=True,
                help_text="The id of the Fwego field we want to aggregate on.",
            ),
        }
    ...
    def serialize_property(
        ...
        :param service: The LocalFwegoAggregateRows service.
        :param prop_name: The property name we're serializing.
        :return: Any
        """
    ...
    def resolve_service_formulas(
        self,
        service: LocalFwegoAggregateRows,
        dispatch_context: DispatchContext,
    ) -> Dict[str, Any]:
        """
        A hook called before the `LocalFwegoTableServiceType` subclass dispatch
    ...
    def dispatch_data(
        ...
        :param service: the local fwego aggregate rows service.
        :param resolved_values: If the service has any formulas, this dictionary will
            contain their resolved values.
        :param dispatch_context: The context used for the dispatch.
        :return: Aggregations.
...
class LocalFwegoGetRowUserServiceType(
    LocalFwegoTableServiceSearchableMixin,
    LocalFwegoTableServiceFilterableMixin,
    LocalFwegoTableServiceSortableMixin,
    LocalFwegoTableServiceSpecificRowMixin,
    ...
    Fwego instance as the one hosting the application.
    """

    integration_type = LocalFwegoIntegrationType.type
    type = "local_fwego_get_row"
    model_class = LocalFwegoGetRow
    dispatch_type = DispatchTypes.DISPATCH_DATA_SOURCE
    ...
    def simple_formula_fields(self):
        return (
            super().simple_formula_fields
            + LocalFwegoTableServiceSearchableMixin.mixin_simple_formula_fields
            + LocalFwegoTableServiceSpecificRowMixin.mixin_simple_formula_fields
        )
    ...
    def allowed_fields(self):
        return (
            ...
            + LocalFwegoTableServiceFilterableMixin.mixin_allowed_fields
            + LocalFwegoTableServiceSearchableMixin.mixin_allowed_fields
            + LocalFwegoTableServiceSpecificRowMixin.mixin_allowed_fields
        )
    ...
    def serializer_field_names(self):
        return (
            ...
            + LocalFwegoTableServiceFilterableMixin.mixin_serializer_field_names
            + LocalFwegoTableServiceSearchableMixin.mixin_serializer_field_names
            + LocalFwegoTableServiceSpecificRowMixin.mixin_serializer_field_names
        )
    ...
    def serializer_field_overrides(self):
        return {
            ...
            **LocalFwegoTableServiceFilterableMixin.mixin_serializer_field_overrides,
            **LocalFwegoTableServiceSearchableMixin.mixin_serializer_field_overrides,
            **LocalFwegoTableServiceSpecificRowMixin.mixin_serializer_field_overrides,
        }
    ...
    class SerializedDict(
        ...
        LocalFwegoTableServiceSearchableMixin.SerializedDict,
        LocalFwegoTableServiceFilterableMixin.SerializedDict,
        LocalFwegoTableServiceSpecificRowMixin.SerializedDict,
    ):
        pass
...
class LocalFwegoUpsertRowServiceType(
    LocalFwegoTableServiceType, LocalFwegoTableServiceSpecificRowMixin
):
    """
    A `LocalFwego` service type which will create or update rows in
    the service's target table when it is used in conjunction with
    workflow actions.
    """

    integration_type = LocalFwegoIntegrationType.type
    type = "local_fwego_upsert_row"
    model_class = LocalFwegoUpsertRow
    ...
    def serializer_field_overrides(self):
        return {
            **super().serializer_field_overrides,
            "field_mappings": LocalFwegoTableServiceFieldMappingSerializer(
                many=True,
                required=False,
    ...
    class SerializedDict(
        LocalFwegoTableServiceType.SerializedDict,
        LocalFwegoTableServiceSpecificRowMixin.SerializedDict,
    ):
    ...
    def after_update(
        ...
        if "field_mappings" in values and instance.table_id:
            ...
            LocalFwegoTableServiceFieldMapping.objects.bulk_create(
                bulk_field_mappings
            )
    ...
    def formula_generator(
        ...
        LocalFwegoTableServiceType.

        The manner in which formula fields are stored will vary for each class
        that implements LocalFwegoTableServiceType. E.g. a formula might be
    ...
    def dispatch_data(
        ...
        been provided, in this `LocalFwegoUpsertRow` service's table.

        :param service: the local fwego upsert row service.
        :param resolved_values: If the service has any formulas, this dictionary will
            contain their resolved values.
        ...
        return {"data": row, "fwego_table_model": model}
...
class LocalFwegoDeleteRowServiceType(
    LocalFwegoTableServiceType, LocalFwegoTableServiceSpecificRowMixin
):
    integration_type = LocalFwegoIntegrationType.type
    type = "local_fwego_delete_row"
    model_class = LocalFwegoDeleteRow
    dispatch_type = DispatchTypes.DISPATCH_WORKFLOW_ACTION
    ...
    def dispatch_data(
        ...
        in this `LocalFwegoDeleteRow` service's table.

        :param service: the local fwego delete row service.
        :param resolved_values: If the service has any formulas, this dictionary will
            contain their resolved values.
        :param dispatch_context: the context used for formula resolution.
        ...
        return {"data": {}, "fwego_table_model": model}
