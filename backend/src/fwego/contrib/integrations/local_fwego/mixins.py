...
from fwego.contrib.database.fields.field_filters import FilterBuilder
from fwego.contrib.database.fields.models import Field
from fwego.contrib.database.search.handler import SearchHandler
from fwego.contrib.database.views.filters import AdHocFilters
...
from fwego.contrib.integrations.local_fwego.api.serializers import (
    LocalFwegoTableServiceFilterSerializer,
    LocalFwegoTableServiceSortSerializer,
)
from fwego.contrib.integrations.local_fwego.models import LocalFwegoViewService
from fwego.core.formula import FwegoFormula, resolve_formula
...
from fwego.core.formula.validator import ensure_integer, ensure_string
...
from fwego.core.services.types import ServiceDict, ServiceSubClass
from fwego.core.services.utils import ServiceAdhocRefinements
...
class LocalFwegoTableServiceFilterableMixin:
    """
    A mixin for LocalFwego{Table,View}Service services so that when they dispatch,
    ...
    mixin_serializer_field_overrides = {
        "filters": LocalFwegoTableServiceFilterSerializer(
    ...
    def get_dispatch_filters(
        ...
        """
        Responsible for defining how the `LocalFwego` services are filtered. To issue
        a `dispatch`, a `LocalFwego` service must be pointing to a table.

        If we only have a `table` and no `view`, then we will query for, and apply,
        any `LocalFwegoTableServiceFilter` found for this service.

        If we also have a `view`, then we will query for, and apply, any `ViewFilter`
        found for this view.

        :param service: The `LocalFwego` service we're dispatching.
        ...
        if isinstance(service, LocalFwegoViewService) and service.view_id:
    ...
    def formula_generator(
        self, service: "LocalFwegoTableServiceFilterableMixin"
    ) -> Generator[str | Instance, str, None]:
        """
        Generator that iterates over formula fields for LocalFwego Services.
        ...
        for service_filter in service.service_filters.all():
            if service_filter.value_is_formula:
                # Service types like LocalFwegoGetRow do not have a value attribute.
...
class LocalFwegoTableServiceSortableMixin:
    """
    A mixin for LocalFwegoTableService services so that when they dispatch, sortings
    applied to their service's table or view are applied to the queryset.
    """

    mixin_serializer_field_names = ["sortings"]
    mixin_serializer_field_overrides = {
        "sortings": LocalFwegoTableServiceSortSerializer(
    ...
    def get_dispatch_sorts(
        ...
        """
        Responsible for defining how the `LocalFwego` services are sorted. To issue
        a `dispatch`, a `LocalFwego` service must be pointing to a table.

        If we find any `LocalFwegoTableServiceSort` applied to this service, we will
        *only* sort on their `OrderBy` expressions.

        If we find no `LocalFwegoTableServiceSort`, then we will attempt to find any
...
class LocalFwegoTableServiceSearchableMixin:
    """
    A mixin for `LocalFwegoTable` service types so that when they dispatch,
...
class LocalFwegoTableServiceSpecificRowMixin:
