...
from fwego.api.polymorphic import PolymorphicSerializer
from fwego.api.user_files.serializers import UserFileURLAndThumbnailsSerializerMixin
from fwego.api.user_files.validators import user_file_name_validator
from fwego.contrib.database.fields.constants import (
    FWEGO_BOOLEAN_FIELD_FALSE_VALUES,
...
class FwegoBooleanField(serializers.BooleanField):
    """
    A Fwego specific `BooleanField` that extends the `TRUE_VALUES` and
    `FALSE_VALUES` in DRF so that we support "checked" and "unchecked".
    """

    TRUE_VALUES = FWEGO_BOOLEAN_FIELD_TRUE_VALUES
    FALSE_VALUES = FWEGO_BOOLEAN_FIELD_FALSE_VALUES
