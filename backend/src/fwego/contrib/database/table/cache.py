...
We then store cache field_attrs in the cache key:
    `full_table_model_{table_id}_{min_model_version}_{FWEGO_VERSION}`
...
from fwego.version import VERSION as FWEGO_VERSION
...
def table_model_cache_entry_key(table_id: int) -> str:
    return f"full_table_model_{table_id}_{FWEGO_VERSION}"
...
def clear_generated_model_cache():
    print("Clearing Fwego's internal generated model cache...")
    if hasattr(generated_models_cache, "delete_pattern"):
        ...
        raise ImproperlyConfigured(
            "Fwego must be run with a redis cache outside of " "tests."
        )
    print("Done clearing cache.")


def invalidate_table_in_model_cache(table_id: int):
    if settings.FWEGO_DISABLE_MODEL_CACHE:
        return None
