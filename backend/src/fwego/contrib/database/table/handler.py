...
from fwego.contrib.database.fields.constants import RESERVED_FWEGO_FIELD_NAMES
...
from fwego.contrib.database.table.expressions import (
    FwegoTableFileUniques,
    FwegoTableRowCount,
)
...
from fwego.core.usage.registries import USAGE_UNIT_MB
from fwego.core.user_files.models import UserFile
from fwego.core.utils import ChildProgressBuilder, Progress, find_unused_name, grouper
...
class TableUsageHandler:
    ...
    def calculate_table_storage_usage(cls, table_id):
        return UserFile.objects.filter(
            unique__in=FwegoTableFileUniques(table_id)
    ...
    def create_tables_usage_for_new_database(cls, database_id: int):
        ...
        entries = [
            TableUsageUpdate(
                table_id=table.id, row_count=FwegoTableRowCount(table.id)
            )
    ...
    def _create_tables_usage(
        ...
        def _create_table_usage(table_id: int) -> TableUsage:
            return TableUsage(
                table_id=table_id,
                row_count=FwegoTableRowCount(table_id),
...
class TableHandler(metaclass=fwego_trace_methods(tracer)):
    @classmethod
    def get_tables(
    ...
    def normalize_initial_table_data(
        ...
        :raises ReservedFwegoFieldNameException: When the field name is reserved by
            Fwego.
        :raises InvalidFwegoFieldName: When the field name is invalid (empty).
        ...
        if len(field_name_set.intersection(RESERVED_FWEGO_FIELD_NAMES)) > 0:
    ...
    def _create_related_link_fields_in_existing_tables_to_import(
        ...
        from fwego.contrib.database.fields.field_types import LinkRowFieldType
    ...
    def duplicate_table(
        ...
        id_mapping: Dict[str, Any] = {
            ...
            # `src/fwego/contrib/database/views/registries.py::import_serialized`
