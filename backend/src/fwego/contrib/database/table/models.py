...
from fwego.cachalot_patch import cachalot_enabled
...
from fwego.contrib.database.fields.fields import IgnoreMissingForeignKey
from fwego.contrib.database.fields.models import (
    CreatedOnField,
    Field,
    LastModifiedField,
)
from fwego.contrib.database.fields.registries import FieldType, field_type_registry
from fwego.contrib.database.fields.utils import get_field_id_from_field_key
from fwego.contrib.database.search.handler import SearchHandler, SearchModes
from fwego.contrib.database.table.cache import (
    get_cached_model_field_attrs,
    set_cached_model_field_attrs,
)
from fwego.contrib.database.table.constants import (
...
from fwego.contrib.database.views.exceptions import ViewFilterTypeNotAllowedForField
from fwego.contrib.database.views.registries import view_filter_type_registry
from fwego.core.db import MultiFieldPrefetchQuerysetMixin, specific_iterator
from fwego.core.fields import AutoTrueBooleanField
from fwego.core.jobs.mixins import (
    JobWithUndoRedoIds,
...
from fwego.core.telemetry.utils import fwego_trace
from fwego.core.utils import split_comma_separated_string
...
class TableModelQuerySet(MultiFieldPrefetchQuerysetMixin, models.QuerySet):
    ...
    def search_all_fields(
        ...
        if search_mode == SearchModes.MODE_FT_WITH_COUNT:
            ...
            if self.model.fwego_table.tsvectors_are_supported:
...
class GeneratedTableModel(HierarchicalModelMixin, models.Model):
    """
    Mixed into Model classes which have been generated by Fwego.
    Can also be used to identify instances of generated fwego models
    like `isinstance(possible_fwego_model, GeneratedTableModel)`.
    """
    ...
    def info(cls):
        ...
        table = Table(
            title=f"{cls.fwego_table.name} ({cls.fwego_table.id})", box=box.ROUNDED
        )
    ...
    @classmethod
    def get_parent(cls):
        return cls.fwego_table

    @classmethod
    def get_root(cls):
        return cls.fwego_table.get_root()
...
class GeneratedModelAppsProxy:
    ...
    def __init__(self, fwego_models=None, app_label=None):
        self.fwego_models = fwego_models or {}
        self.fwego_app_label = app_label or "database_table"
    ...
    def get_models(self, *args, **kwargs):
        ...
        return apps.get_models(*args, **kwargs) + list(self.fwego_models.values())
    ...
    def register_model(self, app_label, model):
        ...
        keep separate Django's model registry from Fwego's generated models. In this
        ...
        with self._lock:
            ...
            if not hasattr(model, "_generated_table_model"):
                ...
                # fwego models we can later use to resolve the pending operations.
                if not hasattr(self, "fwego_models"):
                    self.fwego_models = model._meta.auto_created.fwego_models

            self.fwego_models[model_name] = model
            self.do_all_pending_operations()
            self._clear_fwego_models_cache()
            ...
            try:
                del apps.all_models[self.fwego_app_label]
            except KeyError:
                pass

    def _clear_fwego_models_cache(self):
        for model in self.fwego_models.values():
    ...
    def do_all_pending_operations(self):
        ...
        for _ in range(max_iterations):
            ...
            pending_operations_for_app_label = [
                ...
                if app_label == self.fwego_app_label
            ]
            for _, model_name in list(pending_operations_for_app_label):
                model = self.fwego_models[model_name]
...
class Table(
    ...
    def get_model(
        ...
        :param managed: Whether the created model should be managed by Django or not.
            ...
            generally Fwego itself manages most aspects of returned generated models.
        :type managed: bool
        ...
        attrs = {
            ...
            "fwego_table": self,
            "fwego_table_id": self.id,
            "fwego_models": apps.fwego_models,
        ...
        use_cache = (
            ...
            and not settings.FWEGO_DISABLE_MODEL_CACHE
        )
    ...
    def _add_needs_background_update_column(self, field_attrs, indexes):
        field_attrs[ROW_NEEDS_BACKGROUND_UPDATE_COLUMN_NAME] = AutoTrueBooleanField(
            ...
            "cascading update run by Fwego itself.",
        )
    ...
    def _fetch_and_generate_field_attrs(
        ...
        while len(fields) > 0:
            ...
            if filtered and add_dependencies:
                from fwego.contrib.database.fields.dependencies.handler import (
            ...
            # are not displayed by fwego anywhere.
