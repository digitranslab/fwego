...
from fwego.contrib.database.fields.mixins import (
    DATE_FORMAT_CHOICES,
    DATE_TIME_FORMAT_CHOICES,
    BaseDateMixin,
)
from fwego.contrib.database.fields.utils.duration import DURATION_FORMATS
from fwego.contrib.database.formula import (
    FWEGO_FORMULA_ARRAY_TYPE_CHOICES,
    FWEGO_FORMULA_TYPE_CHOICES,
    FormulaHandler,
)
from fwego.contrib.database.mixins import ParentFieldTrashableModelMixin
from fwego.contrib.database.table.cache import invalidate_table_in_model_cache
...
from fwego.core.utils import remove_special_characters, to_snake_case
...
class Field(
    ...
    """
    Fwego base field model. All custom fields should inherit from this class.
...
class FormulaField(Field):
    ...
    formula_type = models.TextField(
        choices=FWEGO_FORMULA_TYPE_CHOICES,
        default="invalid",
    )
    array_formula_type = models.TextField(
        choices=FWEGO_FORMULA_ARRAY_TYPE_CHOICES,
        default=None,
        null=True,
    )
    number_decimal_places = models.IntegerField(
    ...
    def mark_as_invalid_and_save(self, error: str):
        ...
        invalid_type = FwegoFormulaInvalidType(error)
...
class CountField(FormulaField):
    ...
    def save(self, *args, **kwargs):
        from fwego.contrib.database.formula.ast.function_defs import FwegoCount
        from fwego.contrib.database.formula.ast.tree import FwegoFieldReference

        field_reference = FwegoFieldReference(
            getattr(self.through_field, "name", ""), None, None
        )
        self.formula = f"{FwegoCount.type}({field_reference})"
        super().save(*args, **kwargs)
...
class LookupField(FormulaField):
    ...
    def save(self, *args, **kwargs):
        ...
        expression = str(
            FwegoFieldReference(self.through_field_name, self.target_field_name, None)
        )
...
class DuplicateFieldJob(
    ...
    original_field = models.ForeignKey(
        ...
        on_delete=models.SET_NULL,
        help_text="The Fwego field to duplicate.",
    )
    ...
    duplicated_field = models.OneToOneField(
        ...
        on_delete=models.SET_NULL,
        help_text="The duplicated Fwego field.",
    )
