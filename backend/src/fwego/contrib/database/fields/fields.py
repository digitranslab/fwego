...
from fwego.contrib.database.fields.utils.duration import duration_value_to_timedelta
from fwego.contrib.database.formula import FwegoExpression
from fwego.core.fields import SyncedDateTimeField


class FwegoLastModifiedField(SyncedDateTimeField):
...
class FwegoExpressionField(models.Field):
    """
    A Custom Django field which is always set to the value of the provided Fwego
    Expression.
    """
    ...
    def __init__(
        self,
        expression: Optional[FwegoExpression],
        expression_field: Field,
        *args,
        **kwargs,
    ):
        """
        :param expression: The Fwego expression used to calculate this fields value.
    ...
    def __copy__(self):
        ...
        obj.__class__ = FwegoExpressionField
        return obj
    ...
    def __reduce__(self):
        ...
        if len(reduced_tuple) == 3:
            ...
            return reduced_tuple[0], (FwegoExpressionField,), reduced_tuple[2]
        else:
    ...
    def get_transform(self, name):
        # When a model field of this type is pickled and stored in the Fwego model
