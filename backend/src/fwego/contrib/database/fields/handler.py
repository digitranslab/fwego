...
from fwego.contrib.database.db.schema import (
    lenient_schema_editor,
    safe_django_schema_editor,
)
from fwego.contrib.database.db.sql_queries import (
    sql_create_try_cast,
    sql_drop_try_cast,
)
from fwego.contrib.database.fields.constants import (
    RESERVED_FWEGO_FIELD_NAMES,
    UPSERT_OPTION_DICT_KEY,
    DeleteFieldStrategyEnum,
)
from fwego.contrib.database.fields.field_converters import (
...
def _validate_field_name(
    ...
    :param raise_if_name_missing: When True raises a InvalidFwegoFieldName if the
        name key is not in field_values. When False does not return and immediately
        returns if the key is missing.
    :raises InvalidFwegoFieldName: If "name" is
    ...
    if name in RESERVED_FWEGO_FIELD_NAMES:
        raise ReservedFwegoFieldNameException(
            f"A field named {name} cannot be created as it already exists as a "
            f"reserved Fwego field name."
        )
...
class FieldHandler(metaclass=fwego_trace_methods(tracer)):
    def get_field(
        self,
        field_id: int,
    ...
    def get_specific_field_for_update(
        ...
        queryset = Field.objects.select_related("table").select_for_update(
            of=("self", "table"), nowait=settings.FWEGO_NOWAIT_FOR_LOCKS
        )
    ...
    def create_field(
        ...
        **kwargs,
        ...
        :param skip_django_schema_editor_add_field: Indicates whether the
            ...
            if you are making two Fwego fields which share the same m2m table. For
    ...
    def update_field(
        ...
        **kwargs,
        ...
        fwego_field_type_changed = from_field_type.type != to_field_type_name
        field_cache = FieldCache()

        if fwego_field_type_changed:
        ...
        if converter:
            ...
            if fwego_field_type_changed:
                # If the fwego type has changed we always want to force run any alter
                # column SQL as otherwise it might not run if the two fwego fields
        ...
        field_updated.send(
            ...
            old_field=old_field,
            related_fields=updated_fields,
            field_type_changed=fwego_field_type_changed,
            user=user,
        )
        update_collector.send_additional_field_updated_signals()
    ...
    def delete_field(
        ...
        if delete_strategy == DeleteFieldStrategyEnum.PERMANENTLY_DELETE:
            from fwego.contrib.database.trash.trash_types import (
