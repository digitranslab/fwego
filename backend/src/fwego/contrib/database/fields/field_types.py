...
from fwego.contrib.database.api.utils import (
    LinkRowJoin,
    get_thousand_and_decimal_separator,
)
from fwego.contrib.database.api.views.errors import (
    ERROR_VIEW_DOES_NOT_EXIST,
    ERROR_VIEW_NOT_IN_TABLE,
)
from fwego.contrib.database.db.functions import RandomUUID
from fwego.contrib.database.export_serialized import DatabaseExportSerializedStructure
from fwego.contrib.database.fields.filter_support.formula import (
...
from fwego.contrib.database.models import Table
from fwego.contrib.database.table.handler import TableHandler
from fwego.contrib.database.types import SerializedRowHistoryFieldMetadata
from fwego.contrib.database.validators import UnicodeRegexValidator
from fwego.contrib.database.views.exceptions import ViewDoesNotExist, ViewNotInTable
from fwego.contrib.database.views.models import OWNERSHIP_TYPE_COLLABORATIVE, View
from fwego.core.db import (
    CombinedForeignKeyAndManyToManyMultipleFieldPrefetch,
    collate_expression,
    specific_queryset,
)
from fwego.core.expressions import DateTrunc
from fwego.core.fields import SyncedDateTimeField
...
from fwego.core.import_export.utils import file_chunk_generator
from fwego.core.models import UserFile, WorkspaceUser
from fwego.core.registries import ImportExportConfig
from fwego.core.storage import ExportZipFile, get_default_storage
from fwego.core.user_files.exceptions import UserFileDoesNotExist
from fwego.core.user_files.handler import UserFileHandler
from fwego.core.utils import list_to_comma_separated_string

from .constants import (
    FWEGO_BOOLEAN_FIELD_TRUE_VALUES,
...
from .fields import FwegoExpressionField, FwegoLastModifiedField
...
if TYPE_CHECKING:
    from fwego.contrib.database.table.models import FieldObject, GeneratedTableModel
...
class TextFieldMatchingRegexFieldType(FieldType, ABC):
    ...
    def to_fwego_formula_type(self, field) -> FwegoFormulaType:
        return FwegoFormulaTextType(nullable=True)

    def from_fwego_formula_type(self, formula_type: FwegoFormulaCharType):
...
class CharFieldMatchingRegexFieldType(TextFieldMatchingRegexFieldType):
    ...
    def to_fwego_formula_type(self, field) -> FwegoFormulaType:
        return FwegoFormulaCharType(nullable=True)
...
class TextFieldType(CollationSortMixin, FieldType):
    ...
    def from_fwego_formula_type(
        self, formula_type: FwegoFormulaTextType
    ) -> TextField:
        return TextField()
...
class URLFieldType(CollationSortMixin, TextFieldMatchingRegexFieldType):
    ...
    def to_fwego_formula_type(self, field) -> FwegoFormulaType:
        return FwegoFormulaURLType(nullable=True)
...
class NumberFieldType(FieldType):
    ...
    def to_fwego_formula_type(self, field: NumberField) -> FwegoFormulaType:
    ...
    def from_fwego_formula_type(
        self, formula_type: FwegoFormulaNumberType
    ) -> NumberField:
        return NumberField(
...
class RatingFieldType(FieldType):
    ...
    def to_fwego_formula_type(self, field) -> FwegoFormulaType:
        return FwegoFormulaNumberType(0)
...
class BooleanFieldType(FieldType):
    ...
    def get_alter_column_prepare_new_value(self, connection, from_field, to_field):
        ...
        true_values = ",".join(["'%s'" % v for v in FWEGO_BOOLEAN_FIELD_TRUE_VALUES])
        return f"""
    ...
    def get_serializer_field(self, instance, **kwargs):
        return FwegoBooleanField(**{"required": False, "default": False, **kwargs})
    ...
    def to_fwego_formula_type(self, field: NumberField) -> FwegoFormulaType:
        return FwegoFormulaBooleanType()

    def from_fwego_formula_type(
        self, boolean_formula_type: FwegoFormulaBooleanType
    ) -> BooleanField:
        return BooleanField()
...
class DateFieldType(FieldType):
    ...
    def to_fwego_formula_type(self, field: DateField) -> FwegoFormulaType:
        return FwegoFormulaDateType(
            field.date_format,
    ...
    def from_fwego_formula_type(
        self, formula_type: FwegoFormulaDateType
    ) -> DateField:
        return DateField(
...
class LastModifiedFieldType(CreatedOnLastModifiedBaseFieldType):
    ...
    model_field_class = FwegoLastModifiedField
...
class DurationFieldType(FieldType):
    ...
    def to_fwego_formula_type(self, field) -> FwegoFormulaType:
        return FwegoFormulaDurationType(
...
class LinkRowFieldType(
    ...
    def after_model_generation(self, instance, model, field_name):
        ...
        model.fwego_models[model_name] = model

        # Check if the related table model is already in the model.fwego_models.
        if instance.is_self_referencing:
            ...
            related_model = model.fwego_models.get(related_model_name)
            # If we do not have a related table model already we can generate a new one.
            if related_model is None:
                related_model = instance.link_row_table.get_model(
                    manytomany_models=model.fwego_models,
                    app_label=model._meta.app_label,
                )
                model.fwego_models[related_model_name] = related_model
    ...
    def to_fwego_formula_type(self, field) -> FwegoFormulaType:
        ...
        if primary_field is None:
            return FwegoFormulaInvalidType("references unknown or deleted table")
        else:
            primary_field = primary_field.specific
            related_field_type = field_type_registry.get_by_model(primary_field)
            return related_field_type.to_fwego_formula_type(primary_field)

    def to_fwego_formula_expression(
        self, field
    ) -> FwegoExpression[FwegoFormulaType]:
        primary_field = field.link_row_table_primary_field
        return FormulaHandler.get_lookup_field_reference_expression(
            field, primary_field, self.to_fwego_formula_type(field)
        )
...
class FileFieldType(FieldType):
    ...
    def to_fwego_formula_type(self, field) -> FwegoFormulaType:
        return FwegoFormulaArrayType(FwegoFormulaSingleFileType(nullable=True))

    def from_fwego_formula_type(self, formula_type) -> Field:
        return self.model_class()
...
class SingleSelectFieldType(CollationSortMixin, SelectOptionBaseFieldType):
    ...
    def to_fwego_formula_type(self, field):
        return FwegoFormulaSingleSelectType(nullable=True)
...
class MultipleSelectFieldType(
    ...
    def to_fwego_formula_type(self, field) -> FwegoFormulaType:
        return FwegoFormulaMultipleSelectType(nullable=True)
...
class FormulaFieldType(FormulaArrayFilterSupport, ReadOnlyFieldType):
    ...
    allowed_fields = FWEGO_FORMULA_TYPE_ALLOWED_FIELDS + CORE_FORMULA_FIELDS
    request_serializer_field_names = (
        FWEGO_FORMULA_TYPE_REQUEST_SERIALIZER_FIELD_NAMES + CORE_FORMULA_FIELDS
    )
    request_serializer_field_overrides = {
    ...
    serializer_field_names = (
        FWEGO_FORMULA_TYPE_SERIALIZER_FIELD_NAMES + CORE_FORMULA_FIELDS
    )

    @property
    def serializer_field_overrides(self):
        from fwego.contrib.database.formula.types.formula_types import (
            get_fwego_formula_type_serializer_field_overrides,
        )
    ...
    def get_search_expression(
        ...
        return self.to_fwego_formula_type(field.specific).get_search_expression(
            field, queryset
        )

    def is_searchable(self, field: FormulaField) -> bool:
        return self.to_fwego_formula_type(field.specific).is_searchable(field)

    @staticmethod
    def array_of(formula_type: str):
        return FwegoFormulaArrayType.formula_array_type_as_str(formula_type)
    ...
    def get_field_instance_and_type_from_formula_field(
        ...
        """
        Gets the FwegoFormulaType the provided formula field currently has and the
        Fwego FieldType used to work with a formula of that formula type.

        :param formula_field_instance: An instance of a formula field.
        :return: The FwegoFormulaType of the formula field instance.
        """

        formula_type = self.to_fwego_formula_type(formula_field_instance)
        return formula_type.get_fwego_field_instance_and_type()
    ...
    def get_model_field(self, instance: FormulaField, **kwargs):
        ...
        SpecializedFwegoExpressionField = type(
            expression_field_type.__class__.__name__ + "FwegoExpressionField",
            (FwegoExpressionField,),
            {},
        )
        return SpecializedFwegoExpressionField(
            null=True,
            blank=True,
    ...
    def to_fwego_formula_type(self, field: FormulaField) -> FwegoFormulaType:
    ...
    def to_fwego_formula_expression(
        ...
        if field.expand_formula_when_referenced:
            ...
            return super().to_fwego_formula_expression(field)
    ...
    def run_periodic_update(
        ...
        from fwego.contrib.database.fields.dependencies.update_collector import (
    ...
    def _update_field_values(
        ...
        update_statement = (
            FormulaHandler.fwego_expression_to_update_django_expression(
    ...
    def after_create(self, field, model, user, connection, before, field_kwargs):
        ...
        expr = FormulaHandler.fwego_expression_to_update_django_expression(
    ...
    def check_can_order_by(self, field):
        return self.to_fwego_formula_type(field.specific).can_order_by

    def check_can_group_by(self, field):
        return self.to_fwego_formula_type(field.specific).can_group_by

    def get_order(
        self, field, field_name, order_direction, table_model=None
    ) -> OptionallyAnnotatedOrderBy:
        return self.to_fwego_formula_type(field.specific).get_order(
            field, field_name, order_direction, table_model=table_model
        )

    def get_value_for_filter(self, row: "GeneratedTableModel", field):
        return self.to_fwego_formula_type(field.specific).get_value_for_filter(
            row, field
        )
    ...
    def can_represent_date(self, field: "Field") -> bool:
        return self.to_fwego_formula_type(field.specific).can_represent_date

    def can_represent_files(self, field):
        return self.to_fwego_formula_type(field.specific).can_represent_files

    def can_represent_select_options(self, field):
        return self.to_fwego_formula_type(field.specific).can_represent_select_options
...
class CountFieldType(FormulaFieldType):
    ...
    can_get_unique_values = False
    allowed_fields = FWEGO_FORMULA_TYPE_ALLOWED_FIELDS + [
        "through_field_id",
    ]
    serializer_field_names = FWEGO_FORMULA_TYPE_ALLOWED_FIELDS + [
        "through_field_id",
        "formula_type",
    ]
    serializer_field_overrides = {
...
class LookupFieldType(FormulaFieldType):
    ...
    serializer_field_names = FWEGO_FORMULA_TYPE_SERIALIZER_FIELD_NAMES + [
        "through_field_id",
        "through_field_name",
        "target_field_id",
        "target_field_name",
...
class MultipleCollaboratorsFieldType(
    ...
    def get_serializer_help_text(self, instance):
        return (
            ...
            "collaborators through the object's `id` property. The id is Fwego "
...
class UUIDFieldType(ReadOnlyFieldType):
    ...
    def to_fwego_formula_expression(self, field):
        # Cast the uuid to text, to make it compatible with all the text related
        # functions.
        totext = formula_function_registry.get("totext")
        return totext(super().to_fwego_formula_expression(field))

    def to_fwego_formula_type(self, field) -> FwegoFormulaType:
        return FwegoFormulaTextType(nullable=True, unwrap_cast_to_text=False)
