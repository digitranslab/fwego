...
from fwego.contrib.database.fields.constants import UPSERT_OPTION_DICT_KEY
...
from fwego.contrib.database.views.utils import AnnotatedAggregation
from fwego.core.registries import ImportExportConfig
from fwego.core.registry import (
...
if TYPE_CHECKING:
    from fwego.contrib.database.fields.dependencies.handler import FieldDependants
    ...
    from fwego.contrib.database.table.models import (
        FieldObject,
        GeneratedTableModel,
        Table,
    )
...
class FieldType(
    ...
    Example:
        ...
        from fwego.contrib.database.fields.registry import (
    ...
    def get_alter_column_prepare_old_value(self, connection, from_field, to_field):
        ...
        fwego type which share the same underlying database column type.
    ...
    def force_same_type_alter_column(self, from_field, to_field):
        ...
        run even when from_field and to_field are the same Fwego field type and sql
        column type. If your field has the same fwego type but will convert into
    ...
    def get_export_value(
        ...
        """
        Should convert this field type's internal fwego value to a form suitable
    ...
    def to_fwego_formula_type(self, field: Field):
        """
        Should return the Fwego Formula Type to use when referencing a field of this
        type in a formula.

        :param field: The specific instance of the field that a formula type should
            be created for.
        :return: The Fwego Formula Type that represents this field in a formula.
        """

        from fwego.contrib.database.formula import FwegoFormulaInvalidType

        return FwegoFormulaInvalidType(
            f"A field of type {self.type} cannot be referenced in a Fwego formula."
        )

    def from_fwego_formula_type(self, formula_type) -> Field:
        """
        Should return the Fwego Field Type when converting a formula type back
        to a field type.

        :param formula_type: The specific instance of a formula type that a field type
            model should be created for.
        :return: A Fwego Field Type model instance that represents this formula type.
        """
    ...
    def to_fwego_formula_expression(self, field):
        """
        Should return a Typed Fwego Formula Expression to use when referencing the
        field in a formula.

        :param field: The specific instance of the field that a typed formula
            expression should be created for.
        :return: A typed fwego formula expression which when evaluated represents a
            reference to field.
        """

        from fwego.contrib.database.formula import FormulaHandler

        return FormulaHandler.get_normal_field_reference_expression(
            field, self.to_fwego_formula_type(field)
        )
...
class FieldTypeRegistry(
    ...
    type is an abstraction made specifically for Fwego. If added to the registry a
...
# throughout the whole Fwego application to add a new field type.
