...
FWEGO_TABLE_ROW_COUNT_FUNC = """
CREATE OR REPLACE FUNCTION get_fwego_table_row_count(table_id INT) RETURNS BIGINT AS $$
DECLARE
    row_count BIGINT;
BEGIN
    BEGIN
...
FWEGO_TABLE_FILE_UNIQUES_QUERY_FUN = """
CREATE OR REPLACE FUNCTION _get_fwego_table_file_uniques(table__id INT) RETURNS TABLE(file_unique TEXT, field_id INT, table_id INT) AS $$
DECLARE
    field RECORD;
    filename TEXT;
BEGIN
...
FWEGO_TABLE_FILE_UNIQUES_FUNC = """
CREATE OR REPLACE FUNCTION get_distinct_fwego_table_file_uniques(table_id INT) RETURNS TEXT[] AS $$
DECLARE
    file_uniques TEXT[];
BEGIN
    BEGIN
        EXECUTE 'SELECT array_agg(distinct file_unique) from _get_fwego_table_file_uniques(' || table_id || ');' into file_uniques;
        return file_uniques;
    EXCEPTION WHEN OTHERS THEN
        return null;
    END;
...
def forward(apps, schema_editor):
    with connection.cursor() as cursor:
        cursor.execute(FWEGO_TABLE_ROW_COUNT_FUNC)
        cursor.execute(FWEGO_TABLE_FILE_UNIQUES_QUERY_FUN)
        cursor.execute(FWEGO_TABLE_FILE_UNIQUES_FUNC)


def reverse(apps, schema_editor):
    with connection.cursor() as cursor:
        cursor.execute("DROP FUNCTION get_fwego_table_row_count(INT)")
        cursor.execute("DROP FUNCTION _get_fwego_table_file_uniques(INT)")
        cursor.execute("DROP FUNCTION get_fwego_table_file_uniques(INT)")
