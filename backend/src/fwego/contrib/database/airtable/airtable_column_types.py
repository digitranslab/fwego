...
class TextAirtableColumnType(AirtableColumnType):
    ...
    def to_fwego_field(self, raw_airtable_table, raw_airtable_column):
    ...
    def to_fwego_export_serialized_value(
        ...
        if isinstance(fwego_field, (EmailField, URLField)):
            try:
...
class RichTextTextAirtableColumnType(AirtableColumnType):
    ...
    def _extract_value_from_airtable_rich_value_dict(
        ...
        attempts to return a @userId reference string. In the future if Fwego has
        a rich text field and the ability to reference users in them we should map
        this airtable userId to the corresponding Fwego user id.
        """
...
class NumberAirtableColumnType(AirtableColumnType):
    ...
    def to_fwego_export_serialized_value(
        ...
        if value is not None and not fwego_field.number_negative and value < 0:
            value = None
...
class RatingAirtableColumnType(AirtableColumnType):
    ...
    def to_fwego_field(self, raw_airtable_table, values):
...
class DateAirtableColumnType(AirtableColumnType):
    ...
    def to_fwego_export_serialized_value(
        ...
        if fwego_field.date_include_time:
...
class FormulaAirtableColumnType(AirtableColumnType):
    ...
    def to_fwego_export_serialized_value(
        ...
        if isinstance(fwego_field, CreatedOnField):
            ...
            # the correct value.
            return None
        if isinstance(fwego_field, LastModifiedField):
