...
from fwego.contrib.database.formula.ast.exceptions import UnknownFieldReference
...
from fwego.core.formula.exceptions import formula_exception_handler
from fwego.core.formula.parser.exceptions import MaximumFormulaSizeError


def fwego_expression_to_update_django_expression(
    fwego_expression: FwegoExpression[FwegoFormulaType],
    model: Type[Model],
):
    return _fwego_expression_to_django_expression(fwego_expression, model, None)


def fwego_expression_to_single_row_update_django_expression(
    fwego_expression: FwegoExpression[FwegoFormulaType],
    model_instance: Model,
):
    return _fwego_expression_to_django_expression(
        fwego_expression, type(model_instance), model_instance, insert=False
    )
...
def fwego_expression_to_insert_django_expression(
    ...
    return _fwego_expression_to_django_expression(
        fwego_expression, type(model_instance), model_instance, insert=True
    )


def _fwego_expression_to_django_expression(
    fwego_expression: FwegoExpression[FwegoFormulaType],
    model: Type[Model],
    ...
    """
    Takes a FwegoExpression and converts it to a Django Expression which calculates
    ...
    :param fwego_expression: The FwegoExpression to convert.
    ...
    try:
        if isinstance(fwego_expression.expression_type, FwegoFormulaInvalidType):
            return Value(None)
        else:
            inserting_aggregate = (
                fwego_expression.aggregate and model_instance is not None and insert
            )
            if inserting_aggregate:
                ...
                return fwego_expression.expression_type.placeholder_empty_value()
            else:
                generator = FwegoExpressionToDjangoExpressionGenerator(
                    model, model_instance
                )
                return fwego_expression.accept(generator).expression
...
class FwegoExpressionToDjangoExpressionGenerator(
    FwegoFormulaASTVisitor[FwegoFormulaType, WrappedExpressionWithMetadata]
):
    """
    Visits a FwegoExpression replacing it with the equivalent Django Expression.
    ...
    def __init__(
        ...
        self.model = model
        self.context = FwegoExpressionContext(model, model_instance)

    def visit_field_reference(
        self, field_reference: FwegoFieldReference[FwegoFormulaType]
    ...
    def _setup_lookup_expression(
        self, field_reference: FwegoFieldReference
    ) -> WrappedExpressionWithMetadata:
    ...
    def visit_function_call(
        self, function_call: FwegoFunctionCall[FwegoFormulaType]
    ) -> WrappedExpressionWithMetadata:
    ...
    def visit_string_literal(
        self, string_literal: FwegoStringLiteral[FwegoFormulaType]
    ) -> WrappedExpressionWithMetadata:
    ...
    def visit_int_literal(
        self, int_literal: FwegoIntegerLiteral[FwegoFormulaType]
    ) -> WrappedExpressionWithMetadata:
        return WrappedExpressionWithMetadata(
            Value(
                int_literal.literal,
