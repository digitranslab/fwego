...
from fwego.contrib.database.fields.expressions import (
    extract_jsonb_array_values_to_single_string,
    extract_jsonb_list_values_to_array,
    json_extract_path,
)
from fwego.contrib.database.fields.field_filters import (
    AnnotatedQ,
    OptionallyAnnotatedQ,
)
from fwego.contrib.database.fields.field_sortings import OptionallyAnnotatedOrderBy
from fwego.contrib.database.fields.filter_support.base import (
...
from fwego.contrib.database.fields.filter_support.exceptions import (
    FilterNotSupportedException,
)
from fwego.contrib.database.fields.filter_support.single_select import (
    SingleSelectFormulaTypeFilterSupport,
)
from fwego.contrib.database.fields.mixins import get_date_time_format
from fwego.contrib.database.fields.utils.duration import (
    D_H_M_S,
...
from fwego.contrib.database.formula.types.exceptions import UnknownFormulaType
from fwego.contrib.database.formula.types.formula_type import (
    FwegoFormulaInvalidType,
    FwegoFormulaType,
    FwegoFormulaTypeHasEmptyFwegoExpression,
    FwegoFormulaValidType,
    UnTyped,
)
from fwego.core.storage import get_default_storage
from fwego.core.utils import list_to_comma_separated_string


class FwegoJSONBObjectBaseType(FwegoFormulaValidType, ABC):
    pass


class FwegoFormulaBaseTextType(FwegoFormulaTypeHasEmptyFwegoExpression):
    can_group_by = True

    @property
    def comparable_types(self) -> List[Type["FwegoFormulaValidType"]]:
        return [
    ...
    @property
    def limit_comparable_types(self) -> List[Type["FwegoFormulaValidType"]]:
        ...
        # in
        return [FwegoFormulaTextType, FwegoFormulaCharType, FwegoFormulaLinkType]

    @property
    def addable_types(self) -> List[Type["FwegoFormulaValidType"]]:
        return [FwegoFormulaTextType, FwegoFormulaCharType, FwegoFormulaLinkType]

    def add(
        self,
        add_func_call: "FwegoFunctionCall[UnTyped]",
    ...
    def placeholder_empty_fwego_expression(
        self,
    ) -> "FwegoExpression[FwegoFormulaValidType]":
        return literal("")


class FwegoFormulaTextType(
    HasValueEmptyFilterSupport,
    HasValueFilterSupport,
    HasValueContainsFilterSupport,
    HasValueContainsWordFilterSupport,
    HasValueLengthIsLowerThanFilterSupport,
    FwegoFormulaBaseTextType,
    FwegoFormulaTypeHasEmptyFwegoExpression,
    FwegoFormulaValidType,
):
    type = "text"
    fwego_field_type = "text"
    ...
    def cast_to_text(
        self,
        to_text_func_call: "FwegoFunctionCall[UnTyped]",
...
class FwegoFormulaURLType(FwegoFormulaTextType, FwegoFormulaValidType):
    type = "url"
    fwego_field_type = "url"


class FwegoFormulaCharType(FwegoFormulaTextType, FwegoFormulaValidType):
    type = "char"
    fwego_field_type = "text"
    can_order_by_in_array = True
    can_group_by = True
...
class FwegoFormulaLinkType(FwegoJSONBObjectBaseType):
    type = "link"
    fwego_field_type = None
    can_order_by = False
    can_group_by = False
    ...
    @property
    def subtractable_types(self) -> List[Type["FwegoFormulaValidType"]]:
        return []
    ...
    def get_fwego_field_instance_and_type(self):
        return self, self
    ...
    def get_serializer_field(self, instance, **kwargs) -> Optional[Field]:
        ...
        from fwego.contrib.database.formula.types.serializers import LinkSerializer
...
class FwegoFormulaButtonType(FwegoFormulaLinkType):
    type = "button"


class FwegoFormulaNumberType(
    FwegoFormulaTypeHasEmptyFwegoExpression, FwegoFormulaValidType
):
    type = "number"
    fwego_field_type = "number"
    ...
    @property
    def multipliable_types(self) -> List[Type["FwegoFormulaValidType"]]:
        return [type(self), FwegoFormulaDurationType]

    @property
    def dividable_types(self) -> List[Type["FwegoFormulaValidType"]]:
        return [type(self)]
    ...
    def minus(
        self,
        minus_func_call: "FwegoFunctionCall[UnTyped]",
        arg1: "FwegoExpression[FwegoFormulaNumberType]",
        arg2: "FwegoExpression[FwegoFormulaNumberType]",
    ):
    ...
    def multiply(
        self,
        multiply_func_call: "FwegoFunctionCall[UnTyped]",
        arg1: "FwegoExpression[FwegoFormulaNumberType]",
        arg2: "FwegoExpression[FwegoFormulaNumberType]",
    ):
        if isinstance(arg2.expression_type, FwegoFormulaDurationType):
    ...
    def divide(
        self,
        divide_func_call: "FwegoFunctionCall[UnTyped]",
        arg1: "FwegoExpression[FwegoFormulaNumberType]",
        arg2: "FwegoExpression[FwegoFormulaNumberType]",
    ):
        ...
        return divide_func_call.with_valid_type(
            FwegoFormulaNumberType(NUMBER_MAX_DECIMAL_PLACES)
        )

    def should_recreate_when_old_type_was(self, old_type: "FwegoFormulaType") -> bool:
        if isinstance(old_type, FwegoFormulaNumberType):
            return self.number_decimal_places != old_type.number_decimal_places
        else:
            return True

    def wrap_at_field_level(self, expr: "FwegoExpression[FwegoFormulaType]"):
        return formula_function_registry.get("error_to_nan")(expr)

    def unwrap_at_field_level(self, expr: "FwegoFunctionCall[FwegoFormulaType]"):
...
class FwegoFormulaBooleanType(
    ...
    def try_coerce_to_not_null(
        self, expr: "FwegoExpression[FwegoFormulaValidType]"
    ):
        return expr
    ...
    def _get_prep_value(self, value: str):
        ...
        fwego_field_type = field_type_registry.get(self.fwego_field_type)
        # boolean field type doesn't expect instance value
        field_instance = fwego_field_type.get_model_field(None)
        try:
...
def _calculate_addition_interval_type(
    ...
    if isinstance(arg1_type, FwegoFormulaDateIntervalTypeMixin) and isinstance(
        arg2_type, FwegoFormulaDateIntervalTypeMixin
    ):
        # interval + interval = interval
        resulting_type = arg1_type
    elif isinstance(arg1_type, FwegoFormulaDateIntervalTypeMixin):
        # interval + date = date
...
class FwegoFormulaDateIntervalTypeMixin:
    """
    Empty mixin to allow us to check if a type is a date interval type or a duration
    type. NOTE: This can be removed once the FwegoFormulaDateIntervalType is removed.
    """
...
class FwegoFormulaDateIntervalType(
    FwegoFormulaTypeHasEmptyFwegoExpression,
    FwegoFormulaValidType,
    FwegoFormulaDateIntervalTypeMixin,
):
    type = "date_interval"
    fwego_field_type = None
    can_group_by = True
    ...
    @property
    def addable_types(self) -> List[Type["FwegoFormulaValidType"]]:
        return [type(self), FwegoFormulaDateType]
    ...
    def minus(
        ...
        return minus_func_call.with_valid_type(
            FwegoFormulaDateIntervalType(
                nullable=arg1.expression_type.nullable or arg2.expression_type.nullable
            )
        )

    def get_fwego_field_instance_and_type(self):
        # Until Fwego has a duration field type implement the required methods below
        return self, self
    ...
    def get_model_field(self, instance, **kwargs) -> models.Field:
        from fwego.contrib.database.fields.fields import (
...
class FwegoFormulaDurationType(
    FwegoFormulaTypeHasEmptyFwegoExpression,
    FwegoFormulaValidType,
    FwegoFormulaDateIntervalTypeMixin,
):
    type = "duration"
    fwego_field_type = "duration"
...
class FwegoFormulaDateType(FwegoFormulaValidType):
    type = "date"
    fwego_field_type = "date"
    user_overridable_formatting_option_fields = [
        "date_format",
        "date_include_time",
        "date_time_format",
    ...
    @property
    def addable_types(self) -> List[Type["FwegoFormulaValidType"]]:
        return [FwegoFormulaDateIntervalType, FwegoFormulaDurationType]

    @property
    def subtractable_types(self) -> List[Type["FwegoFormulaValidType"]]:
        return [type(self), FwegoFormulaDateIntervalType, FwegoFormulaDurationType]
    ...
    def minus(
        ...
        arg1_type = arg1.expression_type
        arg2_type = arg2.expression_type
        if isinstance(arg2_type, FwegoFormulaDateType):
            # date - date = duration
            resulting_type = FwegoFormulaDurationType(
    ...
    def should_recreate_when_old_type_was(self, old_type: "FwegoFormulaType") -> bool:
        if isinstance(old_type, FwegoFormulaDateType):
...
class FwegoFormulaSingleFileType(FwegoJSONBObjectBaseType):
    type = "single_file"
    can_group_by = False
    can_order_by = False
    ...
    def get_response_serializer_field(self, instance, **kwargs):
        from fwego.contrib.database.api.fields.serializers import (
    ...
    def get_export_value(self, value, field_object, rich_value=False) -> Any:
        ...
        if "url" in file:
            ...
            from fwego.core.user_files.handler import UserFileHandler
...
class FwegoFormulaArrayType(
    ...
    def __init__(self, sub_type: FwegoFormulaValidType, **kwargs):
        super().__init__(**kwargs)
    ...
    def collapse_many(self, expr: "FwegoExpression[FwegoFormulaType]"):
    ...
    def get_fwego_field_instance_and_type(self):
        # Until Fwego has a array field type implement the required methods below
        return self, self
    ...
    def get_serializer_field(self, instance, **kwargs) -> Optional[Field]:
        ...
        from fwego.contrib.database.api.fields.serializers import ArrayValueSerializer

        (
            instance,
            field_type,
        ) = self.sub_type.get_fwego_field_instance_and_type()
    ...
    def get_export_value(self, value, field_object, rich_value=False) -> Any:
        ...
        field_instance, field_type = self.sub_type.get_fwego_field_instance_and_type()
        field_obj = {
    ...
    def get_serializer_field_overrides(cls):
        ...
        from fwego.contrib.database.api.formula.serializers import (
            FwegoFormulaSelectOptionsSerializer,
        )

        return {
            "select_options": FwegoFormulaSelectOptionsSerializer(
...
class FwegoFormulaSingleSelectType(
    SingleSelectFormulaTypeFilterSupport,
    FwegoJSONBObjectBaseType,
):
    type = "single_select"
    fwego_field_type = "single_select"
    can_order_by = True
    can_order_by_in_array = True
    can_group_by = False
    ...
    def get_response_serializer_field(self, instance, **kwargs) -> Optional[Field]:
        instance, field_type = super().get_fwego_field_instance_and_type()
...
class FwegoFormulaMultipleSelectType(FwegoJSONBObjectBaseType):
    type = "multiple_select"
    fwego_field_type = "multiple_select"
    can_order_by = False
    can_order_by_in_array = False
    can_group_by = False
...
FWEGO_FORMULA_TYPES = [
    ...
    FwegoFormulaLinkType,
    FwegoFormulaDateIntervalType,  # Deprecated in favor of FwegoFormulaDurationType
    FwegoFormulaDurationType,
    FwegoFormulaDateType,
    FwegoFormulaBooleanType,
    FwegoFormulaNumberType,
...
FWEGO_FORMULA_TYPE_ALLOWED_FIELDS = list(
    set(
        allowed_field for f in FWEGO_FORMULA_TYPES for allowed_field in f.all_fields()
    )
)

FWEGO_FORMULA_TYPE_CHOICES = [(f.type, f.type) for f in FWEGO_FORMULA_TYPES]
FWEGO_FORMULA_ARRAY_TYPE_CHOICES = [
    (f.type, f.type)
    for f in FWEGO_FORMULA_TYPES
    if f.type != FwegoFormulaArrayType.type
]

FWEGO_FORMULA_TYPE_SERIALIZER_FIELD_NAMES = list(
    set(
        allowed_field
        for f in FWEGO_FORMULA_TYPES
...
FWEGO_FORMULA_TYPE_REQUEST_SERIALIZER_FIELD_NAMES = list(
    set(
        allowed_field
        for f in FWEGO_FORMULA_TYPES
...
def get_fwego_formula_type_serializer_field_overrides():
    return {
        key: value
        for f in FWEGO_FORMULA_TYPES
...
def calculate_number_type(
    arg_types: List[FwegoFormulaNumberType], min_decimal_places=0
):
    ...
    return FwegoFormulaNumberType(number_decimal_places=max_number_decimal_places)


def _lookup_formula_type_from_string(formula_type_string):
    for possible_type in FWEGO_FORMULA_TYPES:
...
def literal(
    ...
    """
    A helper function for building FwegoExpressions with literals
    :param arg: The literal
    :return: The literal wrapped in the corresponding valid typed FwegoExpression
        literal.
    """

    if isinstance(arg, str):
        return FwegoStringLiteral(arg, FwegoFormulaTextType())
    elif isinstance(arg, bool):
        return FwegoBooleanLiteral(arg, FwegoFormulaBooleanType())
    elif isinstance(arg, int):
        return FwegoIntegerLiteral(
            arg, FwegoFormulaNumberType(number_decimal_places=0)
        )
    elif isinstance(arg, Decimal):
        decimal_literal_expr = FwegoDecimalLiteral(arg, None)
        return decimal_literal_expr.with_valid_type(
            FwegoFormulaNumberType(decimal_literal_expr.num_decimal_places())
        )
    else:
