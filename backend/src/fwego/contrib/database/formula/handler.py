...
from fwego.contrib.database.formula.ast.function import CollapseManyFwegoFunction
...
from fwego.contrib.database.formula.expression_generator.generator import (
    fwego_expression_to_insert_django_expression,
    fwego_expression_to_single_row_update_django_expression,
    fwego_expression_to_update_django_expression,
)
from fwego.contrib.database.formula.migrations.migrations import (
    FWEGO_FORMULA_VERSION,
)
from fwego.contrib.database.formula.parser.ast_mapper import (
    FwegoFieldReferenceVisitor,
    raw_formula_to_untyped_expression,
)
from fwego.contrib.database.formula.parser.update_field_names import (
    update_field_names,
)
from fwego.contrib.database.formula.types.formula_type import FwegoFormulaType
from fwego.contrib.database.formula.types.formula_types import (
    _lookup_formula_type_from_string,
    literal,
)
from fwego.contrib.database.formula.types.typer import (
    calculate_typed_expression,
    recreate_formula_field_if_needed,
)
from fwego.contrib.database.formula.types.visitors import (
...
from fwego.core.telemetry.utils import fwego_trace_methods
...
def _needs_periodic_update(expression: FwegoExpression):
    functions_used: Set[FwegoFunctionDefinition] = expression.accept(
        FunctionsUsedVisitor()
    )
    return any(getattr(f, "needs_periodic_update", False) for f in functions_used)


def _expression_requires_refresh_after_insert(expression: FwegoExpression):
    """
    WARNING: This function is directly used by migration code. Please ensure
    backwards compatibility when adding fields etc.

    Some fwego expressions cannot be computed in a single INSERT xx INTO yy statement.
...
def _has_lookup_expressions(expression):
    ...
    return isinstance(expr, FwegoFunctionCall) and isinstance(
        expr.function_def, CollapseManyFwegoFunction
    )


class FormulaHandler(metaclass=fwego_trace_methods(tracer)):
    """
    ...
    @classmethod
    def fwego_expression_to_update_django_expression(
        cls, expression: FwegoExpression, model: Type[Model]
    ) -> Expression:
        """
        Converts the provided fwego expression to a django expression that can be
        ...
        :param expression: A fully typed internal Fwego expression.
        ...
        return fwego_expression_to_update_django_expression(expression, model)

    @classmethod
    def fwego_expression_to_row_update_django_expression(
        cls,
        expression: FwegoExpression,
        ...
        return fwego_expression_to_single_row_update_django_expression(
            expression, model_instance
        )

    @classmethod
    def fwego_expression_to_insert_django_expression(
        cls,
        expression: FwegoExpression,
        model_instance: Model,
    ) -> Expression:
        """
        Converts the provided fwego expression that can be used when inserting a
        new row.
        ...
        fwego_expression_to_single_row_update_django_expression after it has been
        ...
        return fwego_expression_to_insert_django_expression(
            expression, model_instance
        )

    @classmethod
    def get_normal_field_reference_expression(
        cls, field, formula_type: FwegoFormulaType
    ) -> FwegoExpression:
        """
        Returns the Fwego Expression that represents internally a normal Fwego
        ...
        :return: A Fwego Expression that can be used in internal formulas to represent
            a reference to field.
        """

        return FwegoFieldReference[FwegoFormulaType](
    ...
    def rename_field_references_in_formula_string(
        ...
        :param formula_to_update: A string containing a fwego formula expression to
    ...
    def get_field_dependencies_from_expression(
        cls, source_field, expression: FwegoExpression, table, field_cache
    ) -> FieldDependencies:
        """
    ...
    def raw_formula_to_untyped_expression(cls, formula_string):
        """
        Converts the provided formula string to an untyped FwegoExpression which is
        ...
        :param formula_string: A string containing a formula in the Fwego Formula
            expression language.
        """
    ...
    @classmethod
    def get_formula_type_from_field(cls, formula_field) -> FwegoFormulaType:
        """
        ...
        :return: Returns a populated instance of a FwegoFormulaType object which
    ...
    def get_typed_internal_expression_from_field(
        ...
        Expression using the two fwego_expression_to_{update,insert}_django_expression
        methods above.
        ...
        :param formula_field: The formula field instance to get the internal
            expression for.
        :return: A typed internal Fwego Expression.
        """
    ...
    def recalculate_formula_field_cached_properties(cls, formula_field, field_cache):
        ...
        except FwegoFormulaException as e:
        ...
        formula_field.version = FWEGO_FORMULA_VERSION
    ...
    def get_parse_tree_for_formula(cls, formula: str):
        ...
        Returns the raw antlr parse tree for a formula string in the fwego formula
        language.

        :param formula: A string possibly in the fwego formula language.
    ...
    def get_dependencies_field_names(cls, formula: str) -> Set[Tuple[str, str]]:
        ...
        dependency_field_names = tree.accept(FwegoFieldReferenceVisitor()) or set()
    ...
    def recalculate_formula_and_get_update_expression(
        ...
        return FormulaHandler.fwego_expression_to_update_django_expression(
    ...
    def get_lookup_field_reference_expression(cls, field, primary_field, formula_type):
        ...
        return FwegoFieldReference(field.db_column, db_column, formula_type)
