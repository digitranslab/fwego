...
from fwego.contrib.database.search.exceptions import (
    PostgresFullTextSearchDisabledException,
)
from fwego.contrib.database.search.expressions import LocalisedSearchVector
from fwego.contrib.database.search.regexes import (
...
from fwego.core.utils import ChildProgressBuilder, exception_capturer

if TYPE_CHECKING:
    from fwego.contrib.database.fields.models import Field
    from fwego.contrib.database.table.handler import TableForUpdate
...
class SearchHandler(
    ...
    def special_char_tokenizer(cls, expression: Expression) -> Func:
        ...
        Emails:
          With input "peter@fwego.com" this will result in tokens:
          1. peter
          2. fwego.io
        URLs
          With input "https://fwego.io/jobs/" this will result in tokens:
          1. https
          2. fwego.io
          3. jobs
        Dates
    ...
    def escape_postgres_query(cls, text, per_token_wildcard: bool = False) -> str:
        ...
        :param per_token_wildcard: Determines whether we add the `:*` wildcard to
            ...
            but is problematic for Fwego's frontend, so we only add the wildcard at
    ...
    def after_field_created(cls, field: "Field", skip_search_updates: bool = False):
        """
        :param field: The Fwego field which was created in this table.
    ...
    def after_field_perm_delete(
        ...
        """
        :param field: The current Fwego field which was deleted from this table.
        :return: None
        """
    ...
    def try_slower_but_best_effort_tsv_update(
        ...
        """
        Given the complexity of all possible field configurations in Fwego there is
    ...
    def _trigger_async_tsvector_task_if_needed(
        ...
        if table.tsvectors_are_supported:
            from fwego.contrib.database.search.tasks import (
                async_update_tsvector_columns,
            )
            from fwego.contrib.database.tasks import (
