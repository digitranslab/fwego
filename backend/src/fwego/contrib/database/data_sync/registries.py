...
from fwego.contrib.database.data_sync.export_serialized import (
    DataSyncExportSerializedStructure,
)
from fwego.contrib.database.data_sync.models import DataSync, DataSyncSyncedProperty
...
class DataSyncProperty(ABC):
    ...
    def to_fwego_field(self) -> Field:
        """
        Should return an unsaved Fwego field instance. This is the field object that
        will be used to automatically create the field.

        :return: An unsaved Fwego field model object.
        """

    def get_metadata(
        self, fwego_field: Field, existing_metadata: Optional[dict] = None
    ) -> Optional[dict]:
        """
        ...
        :param fwego_field: The saved/created Fwego field in the synced table.
...
class DataSyncType(
    ...
    def get_all_rows(
        ...
        run through the `to_fwego_value` method of the related property to convert
        them to the Fwego format. It must contain all the rows, even if there are
