...
from fwego.contrib.database.db.schema import safe_django_schema_editor
from fwego.contrib.database.fields.constants import DeleteFieldStrategyEnum
...
from fwego.contrib.database.operations import CreateTableDatabaseTableOperationType
...
from fwego.contrib.database.table.operations import UpdateDatabaseTableOperationType
from fwego.contrib.database.table.signals import table_created, table_updated
    ...
    def create_data_sync_table(
        ...
        **kwargs: dict,
        ...
        for index, synced_property in enumerate(synced_properties):
            ...
            fwego_field = data_sync_property.to_fwego_field()
            fwego_field.order = index
            fwego_field.table = table
            fwego_field.read_only = True
            fwego_field.immutable_type = True
            fwego_field.immutable_properties = data_sync_property.immutable_properties
            if data_sync_property.unique_primary and not has_primary:
                has_primary = True
                fwego_field.primary = True
            fwego_field.save()
            metadata = data_sync_property.get_metadata(fwego_field)
        ...
        for existing_id, existing_record in existing_rows_in_table.items():
            if existing_id in rows_of_data_sync:
                ...
                for enabled_property in enabled_properties:
                    ...
                    fwego_row_value = existing_record[key_to_field_id[key]]
                    data_sync_property = key_to_property[key]
                    if not data_sync_property.is_equal(fwego_row_value, value):
    ...
    def set_data_sync_synced_properties(
        ...
        for synced_property in synced_properties:
            ...
            if synced_property not in enabled_property_keys:
                ...
                new_field = data_sync_property.to_fwego_field()
                ...
                if (
                    ...
                    # in the local Fwego data sync to map the select option cell
                    # values.
        ...
        for data_sync_property in properties_to_be_added:
            ...
            field_kwargs = fwego_field.__dict__
            ...
            field = handler.create_field(
                user=user,
                table=data_sync.table,
                type_name=fwego_field_type.type,
                name=new_name,
                **field_kwargs,
            )
        ...
        for data_sync_property, new_metadata in properties_to_be_updated:
            ...
            enabled_property.field = handler.update_field(
                ...
                new_type_name=fwego_field_type.type,
                **field_kwargs,
            )
