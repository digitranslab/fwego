...
from fwego.core.utils import ChildProgressBuilder, are_hostnames_same
...
class LongTextPostgreSQLSyncProperty(BasePostgreSQLSyncProperty):
    ...
    def to_fwego_field(self) -> LongTextField:
...
class BooleanPostgreSQLSyncProperty(BasePostgreSQLSyncProperty):
    ...
    def to_fwego_field(self) -> BooleanField:
...
class NumberPostgreSQLSyncProperty(BasePostgreSQLSyncProperty):
    ...
    def is_equal(self, fwego_row_value: Any, data_sync_row_value: Any) -> bool:
        try:
            return round(fwego_row_value, self.decimal_places) == round(
...
class DatePostgreSQLSyncProperty(BasePostgreSQLSyncProperty):
    ...
    def to_fwego_field(self) -> DateField:
        return DateField(
            name=self.name,
            date_format="ISO",
...
column_type_to_fwego_field_type = {
...
class PostgreSQLDataSyncType(DataSyncType):
    ...
    def _connection(self, instance):
        ...
        fwego_postgresql_connection = (
            settings.FWEGO_PREVENT_POSTGRESQL_DATA_SYNC_CONNECTION_TO_DATABASE
        ...
        data_sync_blacklist = any(
            ...
            for hostname in settings.FWEGO_POSTGRESQL_DATA_SYNC_BLACKLIST
        )

        if fwego_postgresql_connection or data_sync_blacklist:
    ...
    def get_properties(self, instance) -> List[DataSyncProperty]:
        ...
        for column_name, column_type, decimal_places in columns:
            ...
            property_class = [
                column_type_to_fwego_field_type[key]
                for key in column_type_to_fwego_field_type.keys()
