...
from fwego.core.management.utils import run_command_concurrently
from fwego.core.utils import Progress, grouper
    ...
    def handle(self, *args, **options):
        ...
        if concurrency == 1:
            ...
            try:
                ...
                self.stdout.write(
                    self.style.ERROR(
                        "Your Fwego installation has Postgres full-text "
...
def validate_replicated_tables(source_table_model, replicated_table_models):
    ...
    for model in replicated_table_models:
        ...
        if model_field_map != source_field_map:
            exc_msg = (
                f"The fields in table {model.fwego_table_id} do not match "
                f"those in source table {source_table_model.fwego_table_id}."
            )
            subtractive_changes = dict(set(source_field_map) - set(model_field_map))
            if subtractive_changes:
                exc_msg += f"\n\nFields missing from table {model.fwego_table_id}:\n"
            ...
            if additive_changes:
                exc_msg += f"\n\nFields added in table {model.fwego_table_id}:\n"
...
def generate_values_for_one_or_more_tables(models, fake, cache):
    ...
    for model in models:
        for _, field_object in model._field_objects.items():
            ...
            field_object["fwego_table_id"] = model.fwego_table_id
    ...
    for _, meta in grouped_fields_by_name_and_type.items():
        ...
        for field_object in meta["field_objects"]:
            ...
            table_id = field_object["fwego_table_id"]
            ...
            for _ in group:
                ...
                for model in models:
                    instance, relations = create_row_instance_and_relations(
                        fields_grouped_by_table[model.fwego_table_id],
                        table,
                        model,
                        fake,
                        cache,
                    ...
                    rows[model.fwego_table_id].append((instance, relations))
                    progress.increment(1)

            for model in models:
                pbar.refresh()
                created_rows = bulk_create_rows(model, rows[model.fwego_table_id])
