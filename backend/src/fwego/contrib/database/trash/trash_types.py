...
from fwego.contrib.database.rows.signals import rows_created
...
from fwego.contrib.database.views.handler import ViewHandler, ViewIndexingHandler
from fwego.contrib.database.views.models import View
from fwego.contrib.database.views.registries import (
    view_ownership_type_registry,
    view_type_registry,
)
from fwego.contrib.database.views.signals import view_created
...
class RowTrashableItemType(TrashableItemType):
    ...
    def get_parent(self, trashed_item: Any) -> Optional[Any]:
        return self._get_table(trashed_item.fwego_table_id)
    ...
    def permanently_delete_item(self, row, trash_item_lookup_cache=None):
        RichTextFieldMention.objects.filter(
            table_id=row.fwego_table_id, row_id=row.id
        ).delete()
        row.delete()
