...
from fwego.contrib.database.rows.handler import (
...
class CreateRowActionType(UndoableActionType):
    ...
    def do(
        ...
        See the fwego.contrib.database.rows.handler.RowHandler.create_row
        ...
        :param user_field_names: Whether or not the values are keyed by the internal
            Fwego field name (field_1,field_2 etc) or by the user field names.
...
class CreateRowsActionType(UndoableActionType):
    ...
    def do(
        ...
        belongs to the related workspace. It also calls the rows_created signal.
        See the fwego.contrib.database.rows.handler.RowHandler.create_rows
        for more information.
...
class ImportRowsActionType(UndoableActionType):
    ...
    def do(
        ...
        See the fwego.contrib.database.rows.handler.RowHandler.import_rows
        for more information.
...
class DeleteRowActionType(UndoableActionType):
    ...
    def do(
        ...
        See the fwego.contrib.database.rows.handler.RowHandler.delete_row_by_id
        for more information.
...
class DeleteRowsActionType(UndoableActionType):
    ...
    def do(
        ...
        See the fwego.contrib.database.rows.handler.RowHandler.delete_rows
        for more information.
...
class MoveRowActionType(UndoableActionType):
    ...
    def do(
        ...
        See the fwego.contrib.database.rows.handler.RowHandler.move_row
        for more information.
...
class UpdateRowActionType(UndoableActionType):
    ...
    def do(
        ...
        See the fwego.contrib.database.rows.handler.RowHandler.update_row
        for more information.
        ...
        :param model: If the correct model has already been generated it can be
            provided so that it does not have to be generated for a second time.
        :param user_field_names: Whether or not the values are keyed by the internal
            Fwego field names (field_1,field_2 etc) or by the user field names.
        :raises RowDoesNotExist: When the row with the provided id does not exist.
...
class UpdateRowsActionType(UndoableActionType):
    ...
    def do(
        ...
        See the fwego.contrib.database.rows.handler.RowHandler.update_rows
        for more information.
        Undoing this action restores the original values.
        Redoing set the new values again.
