...
class SafeFwegoPostgresSchemaEditor:
    """
...
def safe_django_schema_editor(atomic=True, name=None, classes=None, **kwargs):
    ...
    BaseDatabaseSchemaEditor. Inside of Fwego this schema editor should always be
    ...
    add two separate ManyToMany model fields to the Generated fwego django models.
    ...
    if name is None:
        name = "FwegoSafeDjangoPostgresSchemaEditor"

    if classes is None:
        classes = []

    classes.append(SafeFwegoPostgresSchemaEditor)

    regular_schema_editor = connection.SchemaEditorClass

    if not issubclass(regular_schema_editor, SafeFwegoPostgresSchemaEditor):
        # Only override the connections schema editor if we haven't already done it
        # in an outer safe schema editor context.
        FwegoSafeDjangoPostgresSchemaEditor = _build_schema_editor_class(
            name, classes
        )
        connection.SchemaEditorClass = FwegoSafeDjangoPostgresSchemaEditor
