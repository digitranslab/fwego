...
def get_webhook_request_function() -> Callable:
    ...
    setting FWEGO_WEBHOOKS_ALLOW_PRIVATE_ADDRESS.
    """

    if settings.FWEGO_WEBHOOKS_ALLOW_PRIVATE_ADDRESS is True:
        ...
        fwego_advocate = RequestsAPIWrapper(addr_validator)

        return fwego_advocate.request
...
def get_advocate_address_validator() -> AddrValidator:
    ...
    return AddrValidator(
        ip_blacklist=settings.FWEGO_WEBHOOKS_IP_BLACKLIST,
        ip_whitelist=settings.FWEGO_WEBHOOKS_IP_WHITELIST,
        hostname_blacklist=settings.FWEGO_WEBHOOKS_URL_REGEX_BLACKLIST,
    )
...
def url_validator(value):
    ...
    enter a url which could be in the network of where fwego is running. It makes
    ...
    try:
        validating_create_connection(
            ...
            timeout=settings.FWEGO_WEBHOOKS_URL_CHECK_TIMEOUT_SECS,
        )
        return value
