...
def get_queue(webhook_id):
    ...
    return RedisQueue(
        queue_key,
        redis_connection,
        max_length=settings.FWEGO_MAX_WEBHOOK_CALLS_IN_QUEUE_PER_WEBHOOK,
    )
...
@app.task(
    bind=True,
    max_retries=settings.FWEGO_WEBHOOKS_MAX_RETRIES_PER_CALL,
    queue="export",
)
def call_webhook(
    self,
    ...
    **kwargs: dict,
    ...
    try:
        with transaction.atomic():
            ...
            if success and webhook.failed_triggers != 0:
                ...
                < settings.FWEGO_WEBHOOKS_MAX_CONSECUTIVE_TRIGGER_FAILURES
            ):
    ...
    # that to happen.
    if not success and retries < settings.FWEGO_WEBHOOKS_MAX_RETRIES_PER_CALL:
