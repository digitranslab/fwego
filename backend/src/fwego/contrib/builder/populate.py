...
from fwego.contrib.builder.domains.models import CustomDomain
from fwego.contrib.builder.elements.handler import ElementHandler
from fwego.contrib.builder.elements.models import CollectionField
...
def load_test_data():
    ...
    CustomDomain.objects.filter(domain_name="test1.getfwego.io").delete()
    CustomDomain.objects.filter(domain_name="test2.getfwego.io").delete()
    CustomDomain.objects.filter(domain_name="test3.getfwego.io").delete()
    CustomDomain.objects.create(
        builder=builder, domain_name="test1.getfwego.io", order=1
    )
    CustomDomain.objects.create(
        builder=builder, domain_name="test2.getfwego.io", order=2
    )
    CustomDomain.objects.create(
        builder=builder, domain_name="test3.getfwego.io", order=3
    )
    ...
    try:
        ...
        integration = IntegrationHandler().create_integration(
            integration_type, builder, name="Local fwego", authorized_user=user
        )
    ...
    try:
        ...
        ElementHandler().create_element(
            ...
            target="blank",
            navigate_to_url='"https://fwego.io"',
