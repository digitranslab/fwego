...
from fwego.cachalot_patch import patch_cachalot_for_fwego
from fwego.config.settings.utils import (
    Setting,
...
from fwego.core.telemetry.utils import otel_is_enabled
from fwego.throttling_types import RateLimit
from fwego.version import VERSION
...
FWEGO_PLUGIN_DIR_PATH = Path(os.environ.get("FWEGO_PLUGIN_DIR", "/fwego/plugins"))

if FWEGO_PLUGIN_DIR_PATH.exists():
    FWEGO_PLUGIN_FOLDERS = [
        file
        for file in FWEGO_PLUGIN_DIR_PATH.iterdir()
        if file.is_dir() and Path(file, "backend").exists()
    ]
else:
    FWEGO_PLUGIN_FOLDERS = []

FWEGO_BACKEND_PLUGIN_NAMES = [d.name for d in FWEGO_PLUGIN_FOLDERS]
FWEGO_OSS_ONLY = bool(os.getenv("FWEGO_OSS_ONLY", ""))
if FWEGO_OSS_ONLY:
    FWEGO_BUILT_IN_PLUGINS = []
else:
    FWEGO_BUILT_IN_PLUGINS = ["fwego_premium", "fwego_enterprise"]
...
DEBUG = os.getenv("FWEGO_BACKEND_DEBUG", "off") == "on"

ALLOWED_HOSTS = ["localhost", "127.0.0.1"]
ALLOWED_HOSTS += os.getenv("FWEGO_EXTRA_ALLOWED_HOSTS", "").split(",")
...
INSTALLED_APPS = [
    ...
    "fwego.api",
    "fwego.ws",
    "fwego.contrib.database",
    "fwego.contrib.integrations",
    "fwego.contrib.builder",
    "fwego.contrib.dashboard",
    *FWEGO_BUILT_IN_PLUGINS,
]
...
if FWEGO_BACKEND_PLUGIN_NAMES:
    print(f"Loaded backend plugins: {','.join(FWEGO_BACKEND_PLUGIN_NAMES)}")
    INSTALLED_APPS.extend(FWEGO_BACKEND_PLUGIN_NAMES)
...
MIDDLEWARE = [
    ...
    "fwego.api.user_sources.middleware.AddUserSourceUserMiddleware",
    "django.contrib.messages.middleware.MessageMiddleware",
    "django.middleware.clickjacking.XFrameOptionsMiddleware",
    "fwego.middleware.FwegoCustomHttp404Middleware",
    "fwego.middleware.ClearContextMiddleware",
]

if otel_is_enabled():
    MIDDLEWARE += ["fwego.core.telemetry.middleware.FwegoOTELMiddleware"]

ROOT_URLCONF = "fwego.config.urls"
...
WSGI_APPLICATION = "fwego.config.wsgi.application"
ASGI_APPLICATION = "fwego.config.asgi.application"


# `ASGI_HTTP_MAX_CONCURRENCY` sets max concurrent asgi requests to be processed by
# the asgi application. It's configurable with `FWEGO_ASGI_HTTP_MAX_CONCURRENCY`
# env variable.
# The default is None - no concurrency limit
ASGI_HTTP_MAX_CONCURRENCY = (
    int(os.getenv("FWEGO_ASGI_HTTP_MAX_CONCURRENCY") or 0) or None
)
...
FWEGO_GROUP_STORAGE_USAGE_QUEUE = os.getenv(
    "FWEGO_GROUP_STORAGE_USAGE_QUEUE", "export"
)
FWEGO_ROLE_USAGE_QUEUE = os.getenv("FWEGO_GROUP_STORAGE_USAGE_QUEUE", "export")

CELERY_BROKER_URL = REDIS_URL
CELERY_TASK_ROUTES = {
    "fwego.contrib.database.export.tasks.run_export_job": {"queue": "export"},
    "fwego.contrib.database.export.tasks.clean_up_old_jobs": {"queue": "export"},
    "fwego.core.trash.tasks.mark_old_trash_for_permanent_deletion": {
        "queue": "export"
    },
    "fwego.core.trash.tasks.permanently_delete_marked_trash": {"queue": "export"},
    "fwego.core.usage.tasks": {"queue": FWEGO_GROUP_STORAGE_USAGE_QUEUE},
    "fwego.contrib.database.table.tasks.run_row_count_job": {"queue": "export"},
    "fwego.core.jobs.tasks.clean_up_jobs": {"queue": "export"},
}
...
if "DATABASE_URL" in os.environ:
    ...
    DATABASES = {
        "default": {
            ...
            "NAME": os.getenv("DATABASE_NAME", "fwego"),
            "USER": os.getenv("DATABASE_USER", "fwego"),
            "PASSWORD": os.getenv("DATABASE_PASSWORD", "fwego"),
...
CACHALOT_TIMEOUT = int(os.getenv("FWEGO_CACHALOT_TIMEOUT", 60 * 60 * 24 * 7))
FWEGO_CACHALOT_ONLY_CACHABLE_TABLES = os.getenv(
    "FWEGO_CACHALOT_ONLY_CACHABLE_TABLES", None
)
FWEGO_CACHALOT_MODE = os.getenv("FWEGO_CACHALOT_MODE", "default")
if FWEGO_CACHALOT_MODE == "full":
    CACHALOT_ONLY_CACHABLE_TABLES = []

elif FWEGO_CACHALOT_ONLY_CACHABLE_TABLES:
    ...
    CACHALOT_ONLY_CACHABLE_TABLES = FWEGO_CACHALOT_ONLY_CACHABLE_TABLES.split(",")
else:
    CACHALOT_ONLY_CACHABLE_TABLES = [
        "auth_user",
        "django_content_type",
        ...
        "fwego_premium_license",
        "fwego_premium_licenseuser",
        "fwego_enterprise_role",
        "fwego_enterprise_roleassignment",
        "fwego_enterprise_team",
        "fwego_enterprise_teamsubject",
    ]

# This list will have priority over CACHALOT_ONLY_CACHABLE_TABLES.
FWEGO_CACHALOT_UNCACHABLE_TABLES = os.getenv(
    "FWEGO_CACHALOT_UNCACHABLE_TABLES", None
)

if FWEGO_CACHALOT_UNCACHABLE_TABLES:
    CACHALOT_UNCACHABLE_TABLES = list(
        filter(bool, FWEGO_CACHALOT_UNCACHABLE_TABLES.split(","))
    )

CACHALOT_ENABLED = os.getenv("FWEGO_CACHALOT_ENABLED", "false") == "true"
CACHALOT_CACHE = "cachalot"
CACHALOT_UNCACHABLE_TABLES = [
    "django_migrations",
    "core_action",
    "database_token",
    "fwego_enterprise_auditlogentry",
]

BUILDER_PUBLICLY_USED_PROPERTIES_CACHE_TTL_SECONDS = int(
    # Default TTL is 10 minutes: 60 seconds * 10
    os.getenv("FWEGO_BUILDER_PUBLICLY_USED_PROPERTIES_CACHE_TTL_SECONDS")
    or 600
)
...
CELERY_SINGLETON_BACKEND_CLASS = (
    "fwego.celery_singleton_backend.RedisBackendForSingleton"
)

# This flag enable automatic index creation for table views based on sortings.
AUTO_INDEX_VIEW_ENABLED = os.getenv("FWEGO_AUTO_INDEX_VIEW_ENABLED", "true") == "true"
AUTO_INDEX_LOCK_EXPIRY = os.getenv("FWEGO_AUTO_INDEX_LOCK_EXPIRY", 60 * 2)
...
AUTH_PASSWORD_VALIDATORS = [
    ...
    {
        "NAME": "fwego.core.user.password_validation.MaximumLengthValidator",
    },
]
...
REST_FRAMEWORK = {
    ...
    "DEFAULT_AUTHENTICATION_CLASSES": (
        "fwego.api.user_sources.authentication.UserSourceJSONWebTokenAuthentication",
        "fwego.api.authentication.JSONWebTokenAuthentication",
    ),
    "DEFAULT_RENDERER_CLASSES": ("rest_framework.renderers.JSONRenderer",),
    "DEFAULT_SCHEMA_CLASS": "fwego.api.openapi.AutoSchema",
}

# Limits the number of concurrent requests per user.
# If FWEGO_MAX_CONCURRENT_USER_REQUESTS is not set, then the default value of -1
# will be used which means the throttling is disabled.
FWEGO_MAX_CONCURRENT_USER_REQUESTS = int(
    os.getenv("FWEGO_MAX_CONCURRENT_USER_REQUESTS", "") or -1
)

if FWEGO_MAX_CONCURRENT_USER_REQUESTS > 0:
    REST_FRAMEWORK["DEFAULT_THROTTLE_CLASSES"] = [
        "fwego.throttling.ConcurrentUserRequestsThrottle",
    ]

    REST_FRAMEWORK["DEFAULT_THROTTLE_RATES"] = {
        "concurrent_user_requests": FWEGO_MAX_CONCURRENT_USER_REQUESTS
    }

    MIDDLEWARE += [
        "fwego.middleware.ConcurrentUserRequestsMiddleware",
    ]

# The maximum number of seconds that a request can be throttled for.
FWEGO_CONCURRENT_USER_REQUESTS_THROTTLE_TIMEOUT = int(
    os.getenv("FWEGO_CONCURRENT_USER_REQUESTS_THROTTLE_TIMEOUT", 30)
)

PUBLIC_VIEW_AUTHORIZATION_HEADER = "Fwego-View-Authorization"
...
ACCESS_TOKEN_LIFETIME = timedelta(
    minutes=int(os.getenv("FWEGO_ACCESS_TOKEN_LIFETIME_MINUTES", 10))  # 10 minutes
)
REFRESH_TOKEN_LIFETIME = timedelta(
    hours=int(os.getenv("FWEGO_REFRESH_TOKEN_LIFETIME_HOURS", 24 * 7))  # 7 days
)
...
SIMPLE_JWT = {
    ...
    # It is recommended that you set FWEGO_JWT_SIGNING_KEY so it is independent
    ...
    "SIGNING_KEY": os.getenv("FWEGO_JWT_SIGNING_KEY") or os.getenv("SECRET_KEY"),
    "USER_AUTHENTICATION_RULE": lambda user: user is not None,
}

SPECTACULAR_SETTINGS = {
    "TITLE": "Fwego API spec",
    "DESCRIPTION": "For more information about our REST API, please visit "
    "[this page](https://fwego.io/docs/apis%2Frest-api).\n\n"
    "For more information about our deprecation policy, please visit "
    "[this page](https://fwego.io/docs/apis%2Fdeprecations).",
    "CONTACT": {"url": "https://fwego.io/contact"},
    "LICENSE": {
        "name": "MIT",
        "url": "https://github.com/digitranslab/fwego/-/blob/master/LICENSE",
    },
    "VERSION": "1.30.1",
...
FWEGO_FILE_UPLOAD_SIZE_LIMIT_MB = int(
    Decimal(os.getenv("FWEGO_FILE_UPLOAD_SIZE_LIMIT_MB", 1024 * 1024)) * 1024 * 1024
)  # ~1TB by default

FWEGO_OPENAI_UPLOADED_FILE_SIZE_LIMIT_MB = int(
    os.getenv("FWEGO_OPENAI_UPLOADED_FILE_SIZE_LIMIT_MB", 512)
)
...
if AWS_STORAGE_ENABLED:
    ...
    AWS_S3_FILE_BUFFER_SIZE = FWEGO_FILE_UPLOAD_SIZE_LIMIT_MB
    set_settings_from_env_if_present(
        AttrDict(vars()),
        [
...
FWEGO_PUBLIC_URL = os.getenv("FWEGO_PUBLIC_URL")
if FWEGO_PUBLIC_URL:
    PUBLIC_BACKEND_URL = FWEGO_PUBLIC_URL
    PUBLIC_WEB_FRONTEND_URL = FWEGO_PUBLIC_URL
    if FWEGO_PUBLIC_URL == "http://localhost":
        print(
            "WARNING: Fwego is configured to use a FWEGO_PUBLIC_URL of "
            "http://localhost. If you attempt to access Fwego on any other hostname "
            "requests to the backend will fail as they will be from an unknown host. "
            "Please set FWEGO_PUBLIC_URL if you will be accessing Fwego "
    ...
    if "PUBLIC_BACKEND_URL" not in os.environ:
        print(
            "WARNING: Fwego is configured to use a PUBLIC_BACKEND_URL of "
            "http://localhost:8000. If you attempt to access Fwego on any other "
            ...
            "Fwego from any other URL then http://localhost."
        )
    if "PUBLIC_WEB_FRONTEND_URL" not in os.environ:
        print(
            "WARNING: Fwego is configured to use a default PUBLIC_WEB_FRONTEND_URL "
            "of http://localhost:3000. Emails sent by Fwego will use links pointing "
            ...
            "the URL where users can access your Fwego server."
        )

FWEGO_EMBEDDED_SHARE_URL = os.getenv("FWEGO_EMBEDDED_SHARE_URL")
if not FWEGO_EMBEDDED_SHARE_URL:
    FWEGO_EMBEDDED_SHARE_URL = PUBLIC_WEB_FRONTEND_URL
...
FWEGO_EMBEDDED_SHARE_HOSTNAME = urlparse(FWEGO_EMBEDDED_SHARE_URL).hostname
...
ROW_PAGE_SIZE_LIMIT = int(os.getenv("FWEGO_ROW_PAGE_SIZE_LIMIT", 200))
...
FWEGO_UNIQUE_ROW_VALUES_SIZE_LIMIT = int(
    os.getenv("FWEGO_UNIQUE_ROW_VALUES_SIZE_LIMIT", 100)
)
...
FWEGO_INITIAL_CREATE_SYNC_TABLE_DATA_LIMIT = int(
    os.getenv("FWEGO_INITIAL_CREATE_SYNC_TABLE_DATA_LIMIT", 5000)
)

MEDIA_URL_PATH = "/media/"
MEDIA_URL = os.getenv("MEDIA_URL", urljoin(PUBLIC_BACKEND_URL, MEDIA_URL_PATH))
MEDIA_ROOT = os.getenv("MEDIA_ROOT", "/fwego/media")
...
STALE_MENTIONS_CLEANUP_INTERVAL_MINUTES = int(
    os.getenv("FWEGO_STALE_MENTIONS_CLEANUP_INTERVAL_MINUTES", "") or 360
)

MIDNIGHT_CRONTAB_STR = "0 0 * * *"
FWEGO_STORAGE_USAGE_JOB_CRONTAB = get_crontab_from_env(
    "FWEGO_STORAGE_USAGE_JOB_CRONTAB", default_crontab=MIDNIGHT_CRONTAB_STR
)

ONE_AM_CRONTRAB_STR = "0 1 * * *"
FWEGO_SEAT_USAGE_JOB_CRONTAB = get_crontab_from_env(
    "FWEGO_SEAT_USAGE_JOB_CRONTAB", default_crontab=ONE_AM_CRONTRAB_STR
)
...
EMAIL_NOTIFICATIONS_ENABLED = str_to_bool(
    os.getenv("FWEGO_EMAIL_NOTIFICATIONS_ENABLED", "true")
)
...
EMAIL_NOTIFICATIONS_LIMIT_PER_TASK = {
    "instant": int(os.getenv("FWEGO_EMAIL_NOTIFICATIONS_LIMIT_INSTANT", 50)),
    "daily": int(os.getenv("FWEGO_EMAIL_NOTIFICATIONS_LIMIT_DAILY", 1000)),
    "weekly": int(os.getenv("FWEGO_EMAIL_NOTIFICATIONS_LIMIT_WEEKLY", 5000)),
}
...
EMAIL_NOTIFICATIONS_INSTANT_CRONTAB = get_crontab_from_env(
    "FWEGO_EMAIL_NOTIFICATIONS_INSTANT_CRONTAB", default_crontab="* * * * *"
)
...
EMAIL_NOTIFICATIONS_DAILY_HOUR_OF_DAY = int(
    os.getenv("FWEGO_EMAIL_NOTIFICATIONS_DAILY_HOUR_OF_DAY", 0)
)
...
EMAIL_NOTIFICATIONS_WEEKLY_DAY_OF_WEEK = int(
    os.getenv("FWEGO_EMAIL_NOTIFICATIONS_WEEKLY_DAY_OF_WEEK", 0)
)
...
EMAIL_NOTIFICATIONS_AUTO_RETRY_IF_LIMIT_REACHED_AFTER = (
    int(os.getenv("FWEGO_EMAIL_NOTIFICATIONS_AUTO_RETRY_IF_LIMIT_REACHED_AFTER", 0))
    or None
)
...
MAX_NOTIFICATIONS_LISTED_PER_EMAIL = int(
    os.getenv("FWEGO_MAX_NOTIFICATIONS_LISTED_PER_EMAIL", 10)
)
...
MAX_EMAILS_PER_MINUTE = int(os.getenv("FWEGO_MAX_EMAILS_PER_MINUTE", 50))
CELERY_EMAIL_TASK_CONFIG = {
    "rate_limit": f"{int(MAX_EMAILS_PER_MINUTE / CELERY_EMAIL_CHUNK_SIZE)}/m",
}

FWEGO_SEND_VERIFY_EMAIL_RATE_LIMIT = RateLimit.from_string(
    os.getenv("FWEGO_SEND_VERIFY_EMAIL_RATE_LIMIT", "5/h")
)

login_action_limit_from_env = os.getenv("FWEGO_LOGIN_ACTION_LOG_LIMIT")
...
FWEGO_SYNC_TEMPLATES_PATTERN = os.getenv("FWEGO_SYNC_TEMPLATES_PATTERN", None)

MAX_FIELD_LIMIT = int(os.getenv("FWEGO_MAX_FIELD_LIMIT", 600))

INITIAL_MIGRATION_FULL_TEXT_SEARCH_MAX_FIELD_LIMIT = int(
    os.getenv(
        "FWEGO_INITIAL_MIGRATION_FULL_TEXT_SEARCH_MAX_FIELD_LIMIT", MAX_FIELD_LIMIT
    )
)


# set max events to be returned by every ICal feed. Empty value means no limit.
FWEGO_ICAL_VIEW_MAX_EVENTS = try_int(
    os.getenv("FWEGO_ICAL_VIEW_MAX_EVENTS", None), None
)
...
PERIODIC_FIELD_UPDATE_TIMEOUT_MINUTES = int(
    os.getenv("FWEGO_PERIODIC_FIELD_UPDATE_TIMEOUT_MINUTES", 9)
)
PERIODIC_FIELD_UPDATE_CRONTAB = get_crontab_from_env(
    "FWEGO_PERIODIC_FIELD_UPDATE_CRONTAB", default_crontab=EVERY_TEN_MINUTES
)
FWEGO_PERIODIC_FIELD_UPDATE_UNUSED_WORKSPACE_INTERVAL_MIN = int(
    os.getenv("FWEGO_PERIODIC_FIELD_UPDATE_UNUSED_WORKSPACE_INTERVAL_MIN", 60)
)
PERIODIC_FIELD_UPDATE_QUEUE_NAME = os.getenv(
    "FWEGO_PERIODIC_FIELD_UPDATE_QUEUE_NAME", "export"
)

FWEGO_WEBHOOKS_MAX_CONSECUTIVE_TRIGGER_FAILURES = int(
    os.getenv("FWEGO_WEBHOOKS_MAX_CONSECUTIVE_TRIGGER_FAILURES", 8)
)
FWEGO_WEBHOOKS_MAX_RETRIES_PER_CALL = int(
    os.getenv("FWEGO_WEBHOOKS_MAX_RETRIES_PER_CALL", 8)
)
FWEGO_WEBHOOKS_MAX_PER_TABLE = int(os.getenv("FWEGO_WEBHOOKS_MAX_PER_TABLE", 20))
FWEGO_WEBHOOKS_MAX_CALL_LOG_ENTRIES = int(
    os.getenv("FWEGO_WEBHOOKS_MAX_CALL_LOG_ENTRIES", 10)
)
FWEGO_WEBHOOKS_REQUEST_TIMEOUT_SECONDS = int(
    os.getenv("FWEGO_WEBHOOKS_REQUEST_TIMEOUT_SECONDS", 5)
)
FWEGO_WEBHOOKS_ALLOW_PRIVATE_ADDRESS = bool(
    os.getenv("FWEGO_WEBHOOKS_ALLOW_PRIVATE_ADDRESS", False)
)
FWEGO_WEBHOOKS_IP_BLACKLIST = [
    ip_network(ip.strip())
    for ip in os.getenv("FWEGO_WEBHOOKS_IP_BLACKLIST", "").split(",")
    if ip.strip() != ""
]
FWEGO_WEBHOOKS_IP_WHITELIST = [
    ip_network(ip.strip())
    for ip in os.getenv("FWEGO_WEBHOOKS_IP_WHITELIST", "").split(",")
    if ip.strip() != ""
]
FWEGO_WEBHOOKS_URL_REGEX_BLACKLIST = [
    re.compile(url_regex.strip())
    for url_regex in os.getenv("FWEGO_WEBHOOKS_URL_REGEX_BLACKLIST", "").split(",")
    if url_regex.strip() != ""
]
FWEGO_WEBHOOKS_URL_CHECK_TIMEOUT_SECS = int(
    os.getenv("FWEGO_WEBHOOKS_URL_CHECK_TIMEOUT_SECS", "10")
)
FWEGO_MAX_WEBHOOK_CALLS_IN_QUEUE_PER_WEBHOOK = (
    int(os.getenv("FWEGO_MAX_WEBHOOK_CALLS_IN_QUEUE_PER_WEBHOOK", "0")) or None
)
...
if bool(os.getenv("FWEGO_ENABLE_SECURE_PROXY_SSL_HEADER", False)):
...
FWEGO_BACKEND_LOG_LEVEL = os.getenv("FWEGO_BACKEND_LOG_LEVEL", "INFO")
FWEGO_BACKEND_DATABASE_LOG_LEVEL = os.getenv(
    "FWEGO_BACKEND_DATABASE_LOG_LEVEL", "ERROR"
)

FWEGO_JOB_EXPIRATION_TIME_LIMIT = int(
    os.getenv("FWEGO_JOB_EXPIRATION_TIME_LIMIT", 30 * 24 * 60)  # 30 days
)
FWEGO_JOB_SOFT_TIME_LIMIT = int(
    os.getenv("FWEGO_JOB_SOFT_TIME_LIMIT", 60 * 30)  # 30 minutes
)
FWEGO_JOB_CLEANUP_INTERVAL_MINUTES = int(
    os.getenv("FWEGO_JOB_CLEANUP_INTERVAL_MINUTES", 5)  # 5 minutes
)
FWEGO_ROW_HISTORY_CLEANUP_INTERVAL_MINUTES = int(
    os.getenv("FWEGO_ROW_HISTORY_CLEANUP_INTERVAL_MINUTES", 30)  # 30 minutes
)
FWEGO_ROW_HISTORY_RETENTION_DAYS = int(
    os.getenv("FWEGO_ROW_HISTORY_RETENTION_DAYS", 180)
)
FWEGO_MAX_ROW_REPORT_ERROR_COUNT = int(
    os.getenv("FWEGO_MAX_ROW_REPORT_ERROR_COUNT", 30)
)
FWEGO_MAX_SNAPSHOTS_PER_GROUP = int(os.getenv("FWEGO_MAX_SNAPSHOTS_PER_GROUP", 50))
FWEGO_SNAPSHOT_EXPIRATION_TIME_DAYS = int(
    os.getenv("FWEGO_SNAPSHOT_EXPIRATION_TIME_DAYS", 360)  # 360 days
)
FWEGO_USER_LOG_ENTRY_CLEANUP_INTERVAL_MINUTES = int(
    os.getenv("FWEGO_USER_LOG_ENTRY_CLEANUP_INTERVAL_MINUTES", 60)  # 60 minutes
)
# 61 days to accommodate timezone changes in admin dashboard
FWEGO_USER_LOG_ENTRY_RETENTION_DAYS = int(
    os.getenv("FWEGO_USER_LOG_ENTRY_RETENTION_DAYS", 61)
)
...
FWEGO_MAX_PENDING_WORKSPACE_INVITES = int(
    os.getenv("FWEGO_MAX_PENDING_WORKSPACE_INVITES", 0)
)

FWEGO_IMPORT_EXPORT_RESOURCE_CLEANUP_INTERVAL_MINUTES = int(
    os.getenv("FWEGO_IMPORT_EXPORT_RESOURCE_CLEANUP_INTERVAL_MINUTES", 5)
)
FWEGO_IMPORT_EXPORT_RESOURCE_REMOVAL_AFTER_DAYS = int(
    os.getenv("FWEGO_IMPORT_EXPORT_RESOURCE_REMOVAL_AFTER_DAYS", 5)
)

# The maximum number of rows that will be exported when exporting a table.
# If `0` then all rows will be exported.
FWEGO_IMPORT_EXPORT_TABLE_ROWS_COUNT_LIMIT = int(
    os.getenv("FWEGO_IMPORT_EXPORT_TABLE_ROWS_COUNT_LIMIT", 0)
)
...
if "fwego_enterprise" not in INSTALLED_APPS:
    PERMISSION_MANAGERS.remove("role")
if "fwego_premium" not in INSTALLED_APPS:
...
LOGGING = {
    ...
    "loggers": {
        "django.db.backends": {
            "handlers": ["console"],
            "level": FWEGO_BACKEND_DATABASE_LOG_LEVEL,
            "propagate": True,
        },
    },
    "root": {
        "handlers": ["console"],
        "level": FWEGO_BACKEND_LOG_LEVEL,
    },
}
...
FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION = os.getenv(
    "FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION", None
)

if FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION is None:
    ...
    FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION = SYNC_TEMPLATES_ON_STARTUP
else:
...
FWEGO_SYNC_TEMPLATES_TIME_LIMIT = int(
    os.getenv("FWEGO_SYNC_TEMPLATES_TIME_LIMIT", 60 * 30)
)

APPEND_SLASH = False

FWEGO_DISABLE_MODEL_CACHE = bool(os.getenv("FWEGO_DISABLE_MODEL_CACHE", ""))
FWEGO_NOWAIT_FOR_LOCKS = not bool(
    os.getenv("FWEGO_WAIT_INSTEAD_OF_409_CONFLICT_ERROR", False)
)

FWEGO_PERSONAL_VIEW_LOWEST_ROLE_ALLOWED = (
    os.getenv("FWEGO_PERSONAL_VIEW_LOWEST_ROLE_ALLOWED", "viewer").strip().upper()
)
...
MIGRATION_LOCK_ID = os.getenv("FWEGO_MIGRATION_LOCK_ID", 123456)
DEFAULT_SEARCH_MODE = os.getenv("FWEGO_DEFAULT_SEARCH_MODE", "compat")


# Search specific configuration settings.
CELERY_SEARCH_UPDATE_HARD_TIME_LIMIT = int(
    os.getenv("FWEGO_CELERY_SEARCH_UPDATE_HARD_TIME_LIMIT", 60 * 30)
)
# By default, Fwego will use Postgres full-text as its
...
USE_PG_FULLTEXT_SEARCH = str_to_bool(
    (os.getenv("FWEGO_USE_PG_FULLTEXT_SEARCH", "true"))
)
PG_SEARCH_CONFIG = os.getenv("FWEGO_PG_SEARCH_CONFIG", "simple")
AUTO_VACUUM_AFTER_SEARCH_UPDATE = str_to_bool(os.getenv("FWEGO_AUTO_VACUUM", "true"))
TSV_UPDATE_CHUNK_SIZE = int(os.getenv("FWEGO_TSV_UPDATE_CHUNK_SIZE", "2000"))
...
FWEGO_BUILDER_DOMAINS = os.getenv("FWEGO_BUILDER_DOMAINS", None)
FWEGO_BUILDER_DOMAINS = (
    FWEGO_BUILDER_DOMAINS.split(",") if FWEGO_BUILDER_DOMAINS is not None else []
)
...
for plugin in [*FWEGO_BUILT_IN_PLUGINS, *FWEGO_BACKEND_PLUGIN_NAMES]:
    try:
...
FWEGO_OPENAI_API_KEY = os.getenv("FWEGO_OPENAI_API_KEY", None)
FWEGO_OPENAI_ORGANIZATION = os.getenv("FWEGO_OPENAI_ORGANIZATION", "") or None
FWEGO_OPENAI_MODELS = os.getenv("FWEGO_OPENAI_MODELS", "")
FWEGO_OPENAI_MODELS = (
    FWEGO_OPENAI_MODELS.split(",") if FWEGO_OPENAI_MODELS else []
)

FWEGO_OPENROUTER_API_KEY = os.getenv("FWEGO_OPENROUTER_API_KEY", None)
FWEGO_OPENROUTER_ORGANIZATION = (
    os.getenv("FWEGO_OPENROUTER_ORGANIZATION", "") or None
)
FWEGO_OPENROUTER_MODELS = os.getenv("FWEGO_OPENROUTER_MODELS", "")
FWEGO_OPENROUTER_MODELS = (
    FWEGO_OPENROUTER_MODELS.split(",") if FWEGO_OPENROUTER_MODELS else []
)

FWEGO_ANTHROPIC_API_KEY = os.getenv("FWEGO_ANTHROPIC_API_KEY", None)
FWEGO_ANTHROPIC_MODELS = os.getenv("FWEGO_ANTHROPIC_MODELS", "")
FWEGO_ANTHROPIC_MODELS = (
    FWEGO_ANTHROPIC_MODELS.split(",") if FWEGO_ANTHROPIC_MODELS else []
)

FWEGO_MISTRAL_API_KEY = os.getenv("FWEGO_MISTRAL_API_KEY", None)
FWEGO_MISTRAL_MODELS = os.getenv("FWEGO_MISTRAL_MODELS", "")
FWEGO_MISTRAL_MODELS = (
    FWEGO_MISTRAL_MODELS.split(",") if FWEGO_MISTRAL_MODELS else []
)

FWEGO_OLLAMA_HOST = os.getenv("FWEGO_OLLAMA_HOST", None)
FWEGO_OLLAMA_MODELS = os.getenv("FWEGO_OLLAMA_MODELS", "")
FWEGO_OLLAMA_MODELS = (
    FWEGO_OLLAMA_MODELS.split(",") if FWEGO_OLLAMA_MODELS else []
)

FWEGO_PREVENT_POSTGRESQL_DATA_SYNC_CONNECTION_TO_DATABASE = str_to_bool(
    os.getenv("FWEGO_PREVENT_POSTGRESQL_DATA_SYNC_CONNECTION_TO_DATABASE", "true")
)
FWEGO_POSTGRESQL_DATA_SYNC_BLACKLIST = os.getenv(
    "FWEGO_POSTGRESQL_DATA_SYNC_BLACKLIST", ""
)
FWEGO_POSTGRESQL_DATA_SYNC_BLACKLIST = (
    FWEGO_POSTGRESQL_DATA_SYNC_BLACKLIST.split(",")
    if FWEGO_POSTGRESQL_DATA_SYNC_BLACKLIST
    else []
)
...
FWEGO_DEFAULT_ZIP_COMPRESS_LEVEL = 5

FWEGO_MAX_HEALTHY_CELERY_QUEUE_SIZE = int(
    os.getenv("FWEGO_MAX_HEALTHY_CELERY_QUEUE_SIZE", "") or 10
)
