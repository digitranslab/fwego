...
export FWEGO_VERSION="1.30.1"

# Used by docker-entrypoint.sh to start the dev server
# If not configured you'll receive this: CommandError: "0.0.0.0:" is not a valid port number or address:port pair.
FWEGO_BACKEND_PORT="${FWEGO_BACKEND_PORT:-8000}"
...
DATABASE_PASSWORD="${DATABASE_PASSWORD:-fwego}"
DATABASE_OPTIONS="${DATABASE_OPTIONS:-}"
# Or you can provide a Postgresql connection url
DATABASE_URL="${DATABASE_URL:-}"

FWEGO_POSTGRES_STARTUP_CHECK_ATTEMPTS="${FWEGO_POSTGRES_STARTUP_CHECK_ATTEMPTS:-5}"

# Backend server related variables
MIGRATE_ON_STARTUP=${MIGRATE_ON_STARTUP:-true}
SYNC_TEMPLATES_ON_STARTUP=${SYNC_TEMPLATES_ON_STARTUP:-true}
FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION=${FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION:-$SYNC_TEMPLATES_ON_STARTUP}
FWEGO_BACKEND_BIND_ADDRESS=${FWEGO_BACKEND_BIND_ADDRESS:-0.0.0.0}
FWEGO_BACKEND_LOG_LEVEL=${FWEGO_BACKEND_LOG_LEVEL:-INFO}
FWEGO_ENABLE_SECURE_PROXY_SSL_HEADER=${FWEGO_ENABLE_SECURE_PROXY_SSL_HEADER:-}

FWEGO_AMOUNT_OF_WORKERS=${FWEGO_AMOUNT_OF_WORKERS:-}
FWEGO_AMOUNT_OF_GUNICORN_WORKERS=${FWEGO_AMOUNT_OF_GUNICORN_WORKERS:-3}

# Celery related variables
FWEGO_RUN_MINIMAL=${FWEGO_RUN_MINIMAL:-}
FWEGO_CELERY_BEAT_STARTUP_DELAY=${FWEGO_CELERY_BEAT_STARTUP_DELAY:-15}
FWEGO_CELERY_BEAT_DEBUG_LEVEL=${FWEGO_CELERY_BEAT_DEBUG_LEVEL:-INFO}
...
wait_for_postgres() {
for i in $( seq 0 "$FWEGO_POSTGRES_STARTUP_CHECK_ATTEMPTS" )
do
  if ! postgres_ready; then
    echo "Waiting for PostgreSQL to become available attempt " \
         "$i/$FWEGO_POSTGRES_STARTUP_CHECK_ATTEMPTS ..."
    sleep 2
  else
...
show_help() {
    echo """
The available Fwego backend related commands, services and healthchecks are shown
below:

ADMIN COMMANDS:
setup           : Runs all setup commands (migrate and update_formulas)
manage          : Manage Fwego and its database
bash            : Start a bash shell with the correct env setup
backup          : Backs up Fwego's database to DATA_DIR/backups by default
restore         : Restores Fwego's database restores from DATA_DIR/backups by default
python          : Run a python command
shell           : Start a Django Python shell
shell           : Start a Django Python shell
wait_for_db     : Waits FWEGO_POSTGRES_STARTUP_CHECK_ATTEMPTS attempts for the
...
SERVICE COMMANDS:
gunicorn            : Start Fwego backend django using a prod ready gunicorn server:
                         * Waits for the postgres database to be available first
                           checking FWEGO_POSTGRES_STARTUP_CHECK_ATTEMPTS times (default 5)
                           before exiting.
                         * Automatically migrates the database on startup unless
                           MIGRATE_ON_STARTUP is set to something other than 'true'.
                         * Binds to FWEGO_BACKEND_BIND_ADDRESS which defaults to 0.0.0.0
...
DEV COMMANDS (most will only work in the fwego_backend_dev image):
django-dev      : Start a normal Fwego backend django development server, performs
...
install-plugin  : Installs a fwego plugin.
"""
}
...
run_setup_commands_if_configured(){
  ...
  if [ "$MIGRATE_ON_STARTUP" = "true" ] ; then
    if [ "${FWEGO_DISABLE_LOCKED_MIGRATIONS:-}" = "true" ] ; then
      migration_command="migrate"
    else
      migration_command="locked_migrate"
    fi
    echo "python /fwego/backend/src/fwego/manage.py $migration_command"
    OTEL_SERVICE_NAME=backend-migrate python /fwego/backend/src/fwego/manage.py "$migration_command"
  fi
}

start_celery_worker(){
  startup_plugin_setup
  if [[ -n "$FWEGO_RUN_MINIMAL" ]]; then
    EXTRA_CELERY_ARGS=(--without-heartbeat --without-gossip --without-mingle)
  else
    EXTRA_CELERY_ARGS=()
  fi
  if [[ -n "$FWEGO_AMOUNT_OF_WORKERS" ]]; then
    EXTRA_CELERY_ARGS+=(--concurrency "$FWEGO_AMOUNT_OF_WORKERS")
  fi
  exec celery -A fwego worker "${EXTRA_CELERY_ARGS[@]}" -l INFO "$@"
}
...
run_backend_server(){
  ...
  if [[ -n "$FWEGO_ENABLE_SECURE_PROXY_SSL_HEADER" ]]; then
  ...
  if [[ "$1" = "wsgi" ]]; then
    STARTUP_ARGS=(fwego.config.wsgi:application)
  elif [[ "$1" = "asgi" ]]; then
    STARTUP_ARGS=(-k uvicorn.workers.UvicornWorker fwego.config.asgi:application)
  else
  ...
  exec gunicorn --workers="$FWEGO_AMOUNT_OF_GUNICORN_WORKERS" \
    --worker-tmp-dir "${TMPDIR:-/dev/shm}" \
    --log-file=- \
    --access-logfile=- \
    --capture-output \
    "${EXTRA_GUNICORN_ARGS[@]}" \
    -b "${FWEGO_BACKEND_BIND_ADDRESS:-0.0.0.0}":"${FWEGO_BACKEND_PORT}" \
    --log-level="${FWEGO_BACKEND_LOG_LEVEL}" \
    "${STARTUP_ARGS[@]}" \
    "${@:2}"
}
...
setup_otel_vars(){
  ...
  EXTRA_OTEL_RESOURCE_ATTRIBUTES="service.namespace=Fwego,"
  EXTRA_OTEL_RESOURCE_ATTRIBUTES+="service.version=${FWEGO_VERSION},"
  EXTRA_OTEL_RESOURCE_ATTRIBUTES+="deployment.environment=${FWEGO_DEPLOYMENT_ENV:-unknown}"

  if [[ -n "${OTEL_RESOURCE_ATTRIBUTES:-}" ]]; then
    # If the container has been launched with some extra otel attributes, make sure not
    # to override them with our Fwego specific ones.
...
source /fwego/venv/bin/activate
...
case "$1" in
    django-dev)
        wait_for_postgres
        run_setup_commands_if_configured
        echo "Running Development Server on 0.0.0.0:${FWEGO_BACKEND_PORT}"
        echo "Press CTRL-p CTRL-q to close this session without stopping the container."
        export OTEL_SERVICE_NAME=backend-dev
        attachable_exec python /fwego/backend/src/fwego/manage.py runserver "${FWEGO_BACKEND_BIND_ADDRESS:-0.0.0.0}:${FWEGO_BACKEND_PORT}"
    ;;
    django-dev-no-attach)
        ...
        echo "Running Development Server on 0.0.0.0:${FWEGO_BACKEND_PORT}"
        export OTEL_SERVICE_NAME=backend-dev
        python /fwego/backend/src/fwego/manage.py runserver "${FWEGO_BACKEND_BIND_ADDRESS:-0.0.0.0}:${FWEGO_BACKEND_PORT}"
    ;;
    gunicorn)
    ...
    backend-healthcheck)
      ...
      curlf "http://localhost:$FWEGO_BACKEND_PORT/api/_health/"
    ;;
    bash)
        exec /bin/bash -c "${@:2}"
    ;;
    manage)
        ...
        exec python3 /fwego/backend/src/fwego/manage.py "${@:2}"
    ;;
    python)
        exec python3 "${@:2}"
    ;;
    setup)
      echo "python3 /fwego/backend/src/fwego/manage.py migrate"
      OTEL_SERVICE_NAME=backend-migrate DONT_UPDATE_FORMULAS_AFTER_MIGRATION=yes python3 /fwego/backend/src/fwego/manage.py migrate
      echo "python3 /fwego/backend/src/fwego/manage.py update_formulas"
      OTEL_SERVICE_NAME=backend-update-formulas python3 /fwego/backend/src/fwego/manage.py update_formulas
    ;;
    shell)
        export OTEL_SERVICE_NAME=backend-shell
        exec python3 /fwego/backend/src/fwego/manage.py shell
    ;;
    lint-shell)
    ...
    ;;
    celery-worker)
      if [[ -n "${FWEGO_RUN_MINIMAL}" && $FWEGO_AMOUNT_OF_WORKERS == "1" ]]; then
    ...
    celery-worker-healthcheck)
      ...
      exec celery -A fwego inspect ping -d "default-worker@$HOSTNAME" -t 10 "${@:2}"
    ;;
    celery-exportworker)
      if [[ -n "${FWEGO_RUN_MINIMAL}" && $FWEGO_AMOUNT_OF_WORKERS == "1" ]]; then
    ...
    celery-exportworker-healthcheck)
      ...
      exec celery -A fwego inspect ping -d "export-worker@$HOSTNAME" -t 10 "${@:2}"
    ;;
    celery-beat)
      ...
      echo "Sleeping for $FWEGO_CELERY_BEAT_STARTUP_DELAY before starting beat to prevent "\
           "startup errors."
      sleep "$FWEGO_CELERY_BEAT_STARTUP_DELAY"
      export OTEL_SERVICE_NAME="celery-beat"
      exec celery -A fwego beat -l "${FWEGO_CELERY_BEAT_DEBUG_LEVEL}" -S redbeat.RedBeatScheduler "${@:2}"
    ;;
    celery-flower)
      exec celery -A fwego flower "$@"
    ;;
    watch-py)
    ...
    backup)
        ...
        exec python3 /fwego/backend/src/fwego/manage.py backup_fwego \
            -h "$DATABASE_HOST" \
            -d "$DATABASE_NAME" \
            -U "$DATABASE_USER" \
            -p "$DATABASE_PORT" \
    ...
    restore)
        ...
        export PGPASSWORD=$DATABASE_PASSWORD
        exec python3 /fwego/backend/src/fwego/manage.py restore_fwego \
            -h "$DATABASE_HOST" \
            -d "$DATABASE_NAME" \
            -U "$DATABASE_USER" \
            -p "$DATABASE_PORT" \
    ...
    ;;
    install-plugin)
      exec /fwego/plugins/install_plugin.sh --runtime "${@:2}"
    ;;
    uninstall-plugin)
      exec /fwego/plugins/uninstall_plugin.sh "${@:2}"
    ;;
    list-plugins)
      exec /fwego/plugins/list_plugins.sh "${@:2}"
    ;;
    *)
