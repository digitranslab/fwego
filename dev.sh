...
new_tab() {
  ...
  if [[ "$OSTYPE" == "linux-gnu"* ]]; then
    ...
    if $PRINT_WARNING; then
        ...
        fwego's dev environment, please add support!"
        PRINT_WARNING=false
    fi
    print_manual_instructions "$COMMAND"
  fi
...
launch_e2e_tab(){
  PWD=$(pwd)
  new_tab "e2e tests" "export FWEGO_E2E_STARTUP_MAX_WAIT_TIME_SECONDS=1200; cd $PWD/e2e-tests && bash --init-file <(echo 'history -s yarn run test-ci;./run-e2e-tests-locally.sh')"
}

show_help() {
    echo """
./dev.sh starts the fwego development environment and by default attempts to
...
dont_migrate    : Disable automatic database migration on fwego startup.
dont_sync       : Disable automatic template sync on fwego startup.
...
case "${1:-noneleft}" in
    ...
    ;;
    restart_wipe)
        echo "./dev.sh: Restarting Env and Wiping fwego_pgdata volume if exists"
        shift
        down=true
        up=true
        delete_volumes=true
...
This will cause file permission errors when Fwego starts up.

They are probably build files created by the old Fwego Docker images owned by root.
...
if [[ -z "${FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION:-}" ]]; then
if [ "$sync_templates" = true ] ; then
export FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION="true"
else
# Because of the defaults set in the docker-compose file we need to explicitly turn
# this off as just not setting it will get the default "true" value.
export FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION="false"
fi
else
  echo "./dev.sh Using the already set value for the env variable FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION = $FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION"
fi
...
export REDIS_PASSWORD=fwego
export DATABASE_PASSWORD=fwego
export SECRET_KEY=fwego
if [[ "$dev" = true ]]; then
  ...
  export FWEGO_DEPLOYMENT_ENV="development-$USER"
fi
...
if [ "$dont_attach" != true ] && [ "$up" = true ] ; then
  ...
  if [ "$all_in_one" = true ]; then
    launch_tab_and_attach "fwego_all_in_one" "fwego_all_in_one"
    launch_tab_and_attach "mailhog" "mailhog"
  fi

  if [ "$all_in_one_dev" = true ]; then
    launch_tab_and_attach "fwego_all_in_one_dev" "fwego_all_in_one_dev"
    launch_tab_and_exec "web frontend lint" \
            "fwego_all_in_one_dev" \
            "/bin/bash /fwego.sh web-frontend-cmd lint-fix"
    launch_tab_and_exec "backend lint" \
            "fwego_all_in_one_dev" \
            "/bin/bash /fwego.sh backend-cmd lint-shell"
    launch_tab_and_attach "mailhog" "mailhog"
  fi

  if [ "$cloudron" = true ]; then
    launch_tab_and_attach "fwego_cloudron" "fwego_cloudron"
  ...
  if [ "$heroku" = true ]; then
    launch_tab_and_attach "fwego_heroku" "fwego_heroku"
    if [ "$attach_all" = true ] ; then
      launch_tab_and_attach "db" "db"
  ...
  if [ "$local" = true ] || [ "$dev" = true ]; then
    ...
    if [ "$dev" = true ] ; then
      launch_tab_and_exec "web frontend lint" \
              "web-frontend" \
              "/bin/bash /fwego/web-frontend/docker/docker-entrypoint.sh lint-fix"
      launch_tab_and_exec "backend lint" \
              "backend" \
              "/bin/bash /fwego/backend/docker/docker-entrypoint.sh lint-shell"
