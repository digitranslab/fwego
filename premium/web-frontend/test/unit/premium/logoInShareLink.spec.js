...
describe('Premium Share View Link Tests', () => {
  ...
  test('User without global premium cannot toggle off the Fwego logo', async () => {
    ...
    const logoSharingOption = shareViewLinkContext
      ...
      .filter((el) => el.text().includes('shareLinkOptions.fwegoLogo.label'))
      .at(0)
  ...
  test('User with global premium can toggle off the Fwego logo', async () => {
    const workspace = { id: 1, name: 'testWorkspace' }
    await testApp.getStore().dispatch('workspace/forceCreate', workspace)
    const tableId = 1
    const databaseId = 3
