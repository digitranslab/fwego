...
def test_view_signals_not_collaborative(
    ...
    with patch("fwego.ws.registries.broadcast_to_channel_group") as broadcast:
    ...
    with patch(
        "fwego.contrib.database.ws.views.signals.broadcast_to_users"
    ) as broadcast:
        ViewHandler().create_view(
            user=user,
            table=table,
...
@pytest.mark.django_db(transaction=True)
@patch("fwego.contrib.database.ws.pages.TablePageType.broadcast")
...
@pytest.mark.django_db(transaction=True)
@patch("fwego.contrib.database.ws.views.signals.broadcast_to_users_ownership_change")
