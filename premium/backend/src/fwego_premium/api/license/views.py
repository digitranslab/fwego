...
from fwego.api.user.errors import ERROR_USER_NOT_FOUND
...
class AdminLicensesView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            ...
            "can be found on our pricing page https://fwego.io/pricing."
        ),
        responses={
...
class AdminLicenseLookupUsersView(APIView):
    ...
    @map_exceptions({License.DoesNotExist: ERROR_LICENSE_DOES_NOT_EXIST})
    def get(self, request, id):
        fwego_license = License.objects.get(pk=id)
        search = request.GET.get("search")
        queryset = User.objects.filter(
            ~Q(id__in=fwego_license.users.all().values_list("user_id", flat=True))
        ).order_by("first_name")
