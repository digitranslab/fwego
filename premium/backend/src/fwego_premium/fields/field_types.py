...
from fwego_premium.api.fields.exceptions import (
    ERROR_GENERATIVE_AI_DOES_NOT_SUPPORT_FILE_FIELD,
)
from fwego_premium.fields.exceptions import GenerativeAITypeDoesNotSupportFileField
...
from fwego.contrib.database.formula import FwegoFormulaType
...
class AIFieldType(CollationSortMixin, SelectOptionBaseFieldType):
    ...
    def get_fwego_field_type(self, instance):
        output_type = ai_field_output_registry.get(instance.ai_output_type)
        fwego_field_type = field_type_registry.get_by_type(
            output_type.fwego_field_type
        )
        return fwego_field_type

    def get_serializer_field(self, instance, **kwargs):
        kwargs["read_only"] = True
        fwego_field_type = self.get_fwego_field_type(instance)
        return fwego_field_type.get_serializer_field(instance, **kwargs)

    def get_response_serializer_field(self, instance, **kwargs):
        fwego_field_type = self.get_fwego_field_type(instance)
        return fwego_field_type.get_response_serializer_field(instance, **kwargs)

    def get_model_field(self, instance, **kwargs):
        fwego_field_type = self.get_fwego_field_type(instance)
        return fwego_field_type.get_model_field(instance, **kwargs)
    ...
    def random_value(self, instance, fake, cache):
        fwego_field_type = self.get_fwego_field_type(instance)
        return fwego_field_type.random_value(instance, fake, cache)

    def to_fwego_formula_type(self, field) -> FwegoFormulaType:
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.to_fwego_formula_type(field)

    def get_value_for_filter(self, row: "GeneratedTableModel", field: Field) -> any:
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.get_value_for_filter(row, field)

    def get_alter_column_prepare_old_value(self, connection, from_field, to_field):
        fwego_field_type = self.get_fwego_field_type(from_field)
        return fwego_field_type.get_alter_column_prepare_old_value(
            connection, from_field, to_field
        )

    def get_alter_column_prepare_new_value(self, connection, from_field, to_field):
        fwego_field_type = self.get_fwego_field_type(to_field)
        return fwego_field_type.get_alter_column_prepare_new_value(
            connection, from_field, to_field
        )

    def contains_query(self, field_name, value, model_field, field):
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.contains_query(field_name, value, model_field, field)

    def contains_word_query(self, field_name, value, model_field, field):
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.contains_word_query(
            field_name, value, model_field, field
        )

    def check_can_order_by(self, field: Field) -> bool:
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.check_can_order_by(field)

    def check_can_group_by(self, field: Field) -> bool:
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.check_can_group_by(field)

    def get_search_expression(self, field: Field, queryset):
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.get_search_expression(field, queryset)

    def is_searchable(self, field):
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.is_searchable(field)

    def enhance_queryset(self, queryset, field, name, **kwargs):
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.enhance_queryset(queryset, field, name)

    def get_order(self, field, field_name, order_direction, table_name=None):
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.get_order(
            field, field_name, order_direction, table_model=table_name
        )

    def serialize_to_input_value(self, field, value):
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.serialize_to_input_value(field, value)

    def valid_for_bulk_update(self, field):
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.valid_for_bulk_update(field)

    def prepare_value_for_db(self, instance, value):
        fwego_field_type = self.get_fwego_field_type(instance)
        return fwego_field_type.prepare_value_for_db(instance, value)

    def prepare_value_for_db_in_bulk(
        self, instance, values_by_row, continue_on_error=False
    ):
        fwego_field_type = self.get_fwego_field_type(instance)
        return fwego_field_type.prepare_value_for_db_in_bulk(
            instance, values_by_row, continue_on_error
        )

    def get_group_by_serializer_field(self, field, **kwargs):
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.get_group_by_serializer_field(field, **kwargs)

    def get_group_by_field_unique_value(self, field, field_name, value):
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.get_group_by_field_unique_value(
    ...
    def get_group_by_field_filters_and_annotations(
        ...
        fwego_field_type = self.get_fwego_field_type(field)
        return fwego_field_type.get_group_by_field_filters_and_annotations(
    ...
    def get_export_serialized_value(
        ...
        fwego_field_type = self.get_fwego_field_type(field_object["field"])
        return fwego_field_type.get_export_serialized_value(
    ...
    def set_import_serialized_value(
        ...
        field_object = row.get_field_object(field_name)
        fwego_field_type = self.get_fwego_field_type(field_object["field"])
        return fwego_field_type.set_import_serialized_value(
            row, field_name, value, id_mapping, cache, files_zip, storage
        )

    def get_export_value(self, value, field_object, rich_value=False):
        fwego_field_type = self.get_fwego_field_type(field_object["field"])
        return fwego_field_type.get_export_value(value, field_object, rich_value)

    def get_human_readable_value(self, value, field_object):
        fwego_field_type = self.get_fwego_field_type(field_object["field"])
        return fwego_field_type.get_human_readable_value(value, field_object)
