...
from fwego_premium.license.exceptions import (
...
from fwego.core.utils import get_fwego_saas_base_url
from fwego.ws.signals import broadcast_to_users
...
class LicenseHandler:
    ...
    def instance_has_feature(cls, feature: str):
        """
        Checks if the Fwego instance has a particular feature granted by an active
    ...
    def workspace_has_feature(cls, feature: str, workspace: Workspace):
        """
        Checks if the Fwego workspace has a particular feature granted to the
        workspace itself.
    ...
    def _get_license_plugin(cls):
        from fwego_premium.plugins import PremiumPlugin
    ...
    def get_public_key(cls):
        ...
        key file is loaded when Fwego is in debug mode.
        """

        import fwego_premium

        file_name = "public_key_debug.pem" if settings.DEBUG else "public_key.pem"
        public_key_path = join(dirname(fwego_premium.__file__), file_name)
