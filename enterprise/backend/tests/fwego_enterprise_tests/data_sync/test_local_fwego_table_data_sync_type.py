...
from fwego_enterprise.data_sync.fwego_table_data_sync import (
...
def test_create_data_sync_table(enterprise_data_fixture):
    ...
    data_sync = handler.create_data_sync_table(
        ...
        database=database,
        table_name="Test",
        type_name="local_fwego_table",
        synced_properties=["id", f"field_{field_1.id}", f"field_{field_2.id}"],
        source_table_id=source_table.id,
    )

    assert isinstance(data_sync, LocalFwegoTableDataSync)
...
def test_sync_data_sync_table_is_equal(enterprise_data_fixture):
    ...
    # it means that the `is_equal` method of `FwegoFieldDataSyncProperty` is not
    # working as expected.
...
def test_sync_data_sync_table_single_select_get_metadata_create(
    ...
    data_sync_property = FwegoFieldDataSyncProperty(
