...
from fwego.api.exceptions import RequestBodyValidationException
from fwego.contrib.builder.domains.handler import DomainHandler
from fwego.contrib.database.fields.handler import FieldHandler
from fwego.contrib.database.fields.models import (
    CreatedByField,
    CreatedOnField,
    DateField,
    DurationField,
...
from fwego.core.user.exceptions import UserNotFound
from fwego.core.user_sources.exceptions import UserSourceImproperlyConfigured
from fwego.core.user_sources.handler import UserSourceHandler
from fwego.core.user_sources.registries import (
    DEFAULT_USER_ROLE_PREFIX,
    user_source_type_registry,
)
from fwego.core.user_sources.service import UserSourceService
from fwego.core.utils import MirrorDict, Progress
from fwego.test_utils.helpers import AnyStr
from fwego_enterprise.integrations.local_fwego.models import LocalFwegoUserSource
from fwego_enterprise.integrations.local_fwego.user_source_types import (
    LocalFwegoUserSourceType,
)

from .helpers import populate_local_fwego_test_data
...
def test_get_user_sources(api_client, data_fixture):
    ...
    integration = data_fixture.create_local_fwego_integration(application=application)
    ...
    assert response_json[0]["type"] == "local_fwego"
    ...
    assert response_json[1]["type"] == "local_fwego"
    ...
    assert response_json[2]["type"] == "local_fwego"
...
def user_source_fixture(data_fixture):
    """A fixture to help test LocalFwegoUserSourceType."""
...
@pytest.mark.django_db
def test_create_local_fwego_user_source(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    workspace = data_fixture.create_workspace(user=user)
    application = data_fixture.create_builder_application(workspace=workspace)
    database = data_fixture.create_database_application(workspace=workspace)
    ...
    response_json = response.json()
    assert response.status_code == HTTP_200_OK
    assert response_json["type"] == "local_fwego"
...
@pytest.mark.django_db
def test_create_local_fwego_user_source_wrong_field_type(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    workspace = data_fixture.create_workspace(user=user)
    application = data_fixture.create_builder_application(workspace=workspace)
    database = data_fixture.create_database_application(workspace=workspace)
...
@pytest.mark.django_db
def test_create_local_fwego_user_source_w_a_password_auth_source(
...
@pytest.mark.django_db
def test_create_local_fwego_user_source_w_two_password_auth_source(
    api_client, data_fixture
):
    user, token = data_fixture.create_user_and_token()
    workspace = data_fixture.create_workspace(user=user)
...
@pytest.mark.django_db
def test_create_local_fwego_user_source_w_wrong_field_ids(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    workspace = data_fixture.create_workspace(user=user)
    application = data_fixture.create_builder_application(workspace=workspace)
    database = data_fixture.create_database_application(workspace=workspace)
...
@pytest.mark.django_db
def test_create_local_fwego_user_source_w_a_missing_table(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    workspace = data_fixture.create_workspace(user=user)
    application = data_fixture.create_builder_application(workspace=workspace)
    database = data_fixture.create_database_application(workspace=workspace)
...
def test_user_source_create_user(data_fixture):
    ...
    created_user = user_source.get_type().create_user(
        user_source, "test2@fwego.io", "Test2"
    )

    model = table_from_same_workspace1.get_model()
    assert created_user.email == "test2@fwego.io"
    assert model.objects.count() == 2
    assert getattr(model.objects.last(), email_field.db_column) == "test2@fwego.io"
...
def test_user_source_create_user_w_role(data_fixture):
    ...
    created_user = user_source.get_type().create_user(
        user_source, "test2@fwego.io", "Test2", "role2"
    )
...
@pytest.mark.django_db
def test_import_local_fwego_user_source(data_fixture):
    user = data_fixture.create_user()
    workspace = data_fixture.create_workspace(user=user)
    application = data_fixture.create_builder_application(workspace=workspace)
    database = data_fixture.create_database_application(workspace=workspace)
...
@pytest.mark.django_db
def test_create_local_fwego_user_source_w_auth_providers(api_client, data_fixture):
    user, token = data_fixture.create_user_and_token()
    workspace = data_fixture.create_workspace(user=user)
    application = data_fixture.create_builder_application(workspace=workspace)
    database = data_fixture.create_database_application(workspace=workspace)
...
@pytest.mark.django_db
def test_local_fwego_user_source_token_generation(
    data_fixture,
):
    user = data_fixture.create_user()
    workspace = data_fixture.create_workspace(user=user)
    ...
    table, fields, rows = data_fixture.build_table(
        ...
        rows=[
            ["test@fwego.io", "Test", "super not secret", ""],
        ],
    )
    ...
    user = user_source_type.get_user(user_source, email="test@fwego.io")

    assert user.email == "test@fwego.io"
...
def test_public_dispatch_data_source_with_ab_user_using_user_source(
    ...
    data_source = data_fixture.create_builder_local_fwego_get_row_data_source(
        user=user,
        page=page,
        integration=integration,
        table=table,
    ...
    user_source = data_fixture.create_user_source(
        user_source_type_registry.get("local_fwego").model_class,
        application=builder,
        integration=integration,
        table=user_table,
        email_field=email_field,
    ...
    user_source_user = user_source_type.get_user(user_source, email="test@fwego.io")
...
@pytest.mark.django_db
def test_local_fwego_user_source_get_user(
    data_fixture,
):
    data = populate_local_fwego_test_data(data_fixture)
...
@pytest.mark.django_db
def test_local_fwego_user_source_get_user_misconfigured(
    data_fixture,
):
    data = populate_local_fwego_test_data(data_fixture)
...
@pytest.mark.django_db
def test_local_fwego_user_source_get_user_with_duplicate(
    data_fixture,
):
    data = populate_local_fwego_test_data(data_fixture)
...
@pytest.mark.django_db
def test_local_fwego_user_source_get_user_fails(
    data_fixture,
):
    data = populate_local_fwego_test_data(data_fixture)
...
@pytest.mark.django_db
def test_local_fwego_user_source_authentication(
    data_fixture,
):
    data = populate_local_fwego_test_data(data_fixture)
    ...
    user = user_source_type.authenticate(
        user_source, email="test@fwego.io", password="super not secret"
    )
    ...
    user = user_source_type.authenticate(
        user_source, email="test2@fwego.io", password="super not secret"
    )

    assert user.email == "test2@fwego.io"
    assert user.username == "Test2"


@pytest.mark.django_db
def test_local_fwego_user_source_authentication_improperly_configured(
    data_fixture,
):
    data = populate_local_fwego_test_data(data_fixture)
...
@pytest.mark.django_db
def test_local_fwego_user_source_authentication_missing_auth_provider(
    data_fixture,
):
    data = populate_local_fwego_test_data(data_fixture)
...
@pytest.mark.django_db
def test_local_fwego_user_source_authentication_bad_creds(
    data_fixture,
):
    data = populate_local_fwego_test_data(data_fixture)
    ...
    with pytest.raises(AuthenticationFailed):
        user_source_type.authenticate(
            user_source, email="test@fwego.io", password="bad password"
        )

    with pytest.raises(UserNotFound):
        user_source_type.authenticate(
            user_source, email="testt@fwego.io", password="super not secret"
        )

    with pytest.raises(AuthenticationFailed):
        user_source_type.authenticate(
            user_source, email="test4@fwego.io", password="super not secret"
        )


@pytest.mark.django_db
def test_local_fwego_user_source_authentication_list_users(
...
@pytest.mark.django_db
def test_local_fwego_user_source_authentication_list_users_not_configured(
    data_fixture,
):
    data = populate_local_fwego_test_data(data_fixture)
...
@pytest.mark.django_db
def test_local_fwego_user_source_authentication_list_users_get_user_model_misconfiguration(
    data_fixture,
):
    data = populate_local_fwego_test_data(data_fixture)
...
def test_local_fwego_user_source_get_user_model_with_trashed_table(data_fixture):
    user = data_fixture.create_user()
    integration = data_fixture.create_local_fwego_integration(user=user)
    ...
    with pytest.raises(UserSourceImproperlyConfigured) as exc:
        LocalFwegoUserSourceType().get_user_model(
            LocalFwegoUserSource(table=trashed_table, integration=integration)
        )
    assert exc.value.args[0] == "The table doesn't exist."


def test_local_fwego_user_source_authentication_is_configured(
    data_fixture,
):
    user_source_type = LocalFwegoUserSourceType()
    # All configured fields.
    # fmt: off
    assert user_source_type.is_configured(LocalFwegoUserSource(
        email_field_id=1,
        name_field_id=2,
        table_id=3,
        role_field_id=4,
...
def test_prepare_values_role_field_id_raises_if_field_doesnt_exist(
    ...
    with pytest.raises(RequestBodyValidationException) as e:
        LocalFwegoUserSourceType().prepare_values(
...
def test_prepare_values_role_field_id_returns_values(
    ...
    values = LocalFwegoUserSourceType().prepare_values(
        {"role_field_id": role_field.id},
        user,
        user_source,
    )
...
def test_get_user_role_returns_role_from_text_field(data_fixture):
    ...
    role_name = "foo_role"
    data = populate_local_fwego_test_data(data_fixture, role_name=role_name)
    user_source = data["user_source"]

    UserModel = data["user_table"].get_model()
    first_user = UserModel.objects.first()

    user_role = LocalFwegoUserSourceType().get_user_role(
        first_user,
        user_source,
    )
...
def test_get_roles_returns_default_role_if_role_field_is_null(data_fixture):
    ...
    roles = LocalFwegoUserSourceType().get_roles(user_source)
...
def test_role_type_is_allowed(data_fixture, field_type, expected_is_allowed):
    ...
    is_allowed = LocalFwegoUserSourceType().role_type_is_allowed(role_field)
...
def test_get_user_role_returns_role_from_formula_field_date_field(data_fixture):
    ...
    data = populate_local_fwego_test_data(
...
def test_get_user_role_handles_empty_role_field(data_fixture, role, expected_role):
    ...
    data = populate_local_fwego_test_data(data_fixture, role_name=role)
    user_source = data["user_source"]
...
def test_local_fwego_user_source_get_user_is_case_insensitive(
    data_fixture,
    user_provided_email,
):
    """
...
def test__generate_update_user_count_cache_key():
    ...
    assert (
        LocalFwegoUserSourceType()._generate_update_user_count_cache_key(user_source)
        == "local_fwego_user_source_123_user_count"
    )
...
def test__generate_update_user_count_cache_value():
    with freeze_time("2024-11-29T12:00:00.00Z"):
        assert (
            LocalFwegoUserSourceType()._generate_update_user_count_cache_value(500)
            == "500-1732881600.0"
        )
...
def test_update_user_count_with_configured_user_sources(
    ...
    integration1 = data_fixture.create_local_fwego_integration(
        application=application1
    )
    application2 = data_fixture.create_builder_application(workspace=workspace)
    integration2 = data_fixture.create_local_fwego_integration(
    ...
    table, fields, rows = data_fixture.build_table(
        ...
        rows=[
            ["jrmi@fwego.io", "Jérémie", ""],
            ["peter@fwego.io", "Peter", ""],
            ["afonso@fwego.io", "Afonso", ""],
            ["tsering@fwego.io", "Tsering", ""],
            ["evren@fwego.io", "Evren", ""],
        ],
    )
    email_field, name_field, role_field = fields
    ...
    with freeze_time("2030-11-29T12:00:00.00Z"), django_assert_num_queries(3):
        local_fwego_user_source_type.update_user_count()

    with django_assert_num_queries(0):
        user_count = local_fwego_user_source_type.get_user_count(user_source1)
        assert user_count.count == 5
        assert user_count.last_updated == datetime(2030, 11, 29, 12, 0, 0)

    with django_assert_num_queries(0):
        user_count = local_fwego_user_source_type.get_user_count(user_source2)
        assert user_count.count == 5
        assert user_count.last_updated == datetime(2030, 11, 29, 12, 0, 0)
...
def test_update_user_count_with_misconfigured_user_sources(
    ...
    with django_assert_num_queries(0):
        assert local_fwego_user_source_type.get_user_count(user_source1) is None
    with django_assert_num_queries(0):
        assert local_fwego_user_source_type.get_user_count(user_source2) is None
...
def test_local_fwego_after_user_source_update_requires_user_recount():
    assert (
        LocalFwegoUserSourceType().after_user_source_update_requires_user_recount(
            Mock(), {}
        )
        is True
    )
