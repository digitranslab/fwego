...
from fwego.core.context import clear_current_workspace_id, set_current_workspace_id
...
def test_files_are_served_by_base_file_storage_by_default(
    ...
    with patch("fwego.core.storage.get_default_storage", new=dummy_storage(tmpdir)):
...
@override_settings(
    DEBUG=True,
    FWEGO_SERVE_FILES_THROUGH_BACKEND=True,
    FWEGO_SERVE_FILES_THROUGH_BACKEND_PERMISSION="DISABLED",
    STORAGES={
        "default": {
            "BACKEND": "fwego_enterprise.secure_file_serve.storage.EnterpriseFileStorage"
        }
    },
)
...
@override_settings(
    DEBUG=True,
    FWEGO_SERVE_FILES_THROUGH_BACKEND=True,
    FWEGO_SERVE_FILES_THROUGH_BACKEND_PERMISSION=SecureFileServePermission.DISABLED,
    STORAGES={
        "default": {
            "BACKEND": "fwego_enterprise.secure_file_serve.storage.EnterpriseFileStorage"
        }
...
def test_files_can_be_downloaded_by_the_backend_with_valid_license(
    ...
    with patch("fwego.core.storage.get_default_storage", new=storage):
        file = SimpleUploadedFile("test.txt", b"Hello World")
        response = api_client.post(
            reverse("api:user_files:upload_file"),
            data={"file": file},
...
@override_settings(
    ...
    FWEGO_SERVE_FILES_THROUGH_BACKEND=True,
    FWEGO_SERVE_FILES_THROUGH_BACKEND_PERMISSION=SecureFileServePermission.DISABLED,
    FWEGO_SERVE_FILES_THROUGH_BACKEND_EXPIRE_SECONDS=59,
    STORAGES={
        "default": {
            "BACKEND": "fwego_enterprise.secure_file_serve.storage.EnterpriseFileStorage"
        }
...
def test_files_urls_can_expire(
    ...
    with (
        patch("fwego.core.storage.get_default_storage", new=storage),
...
@override_settings(
    DEBUG=True,
    FWEGO_SERVE_FILES_THROUGH_BACKEND=True,
    FWEGO_SERVE_FILES_THROUGH_BACKEND_PERMISSION=SecureFileServePermission.SIGNED_IN,
    STORAGES={
        "default": {
            "BACKEND": "fwego_enterprise.secure_file_serve.storage.EnterpriseFileStorage"
        }
...
@override_settings(
    DEBUG=True,
    FWEGO_SERVE_FILES_THROUGH_BACKEND=True,
    FWEGO_SERVE_FILES_THROUGH_BACKEND_PERMISSION=SecureFileServePermission.WORKSPACE_ACCESS,
    STORAGES={
        "default": {
            "BACKEND": "fwego_enterprise.secure_file_serve.storage.EnterpriseFileStorage"
        }
...
def test_user_must_have_access_to_workspace_to_download_file(
    ...
    with (
        patch("fwego.core.storage.get_default_storage", new=storage),
        patch(
            "fwego_enterprise.api.secure_file_serve.views.SecureFileServeAuthentication.authenticate"
