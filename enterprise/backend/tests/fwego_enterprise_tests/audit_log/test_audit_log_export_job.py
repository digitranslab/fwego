...
from fwego.core.actions import CreateApplicationActionType, CreateWorkspaceActionType
...
def test_deleting_audit_log_export_job_also_delete_exported_file(
    ...
    with patch(
        "fwego_enterprise.audit_log.job_types.get_default_storage"
    ) as get_remove_mock:
...
def test_audit_log_export_workspace_csv_correctly_if_feature_is_enable_for_the_user(
    ...
    # exception.
    with patch(
        "fwego_enterprise.audit_log.utils.LicenseHandler.raise_if_user_doesnt_have_feature"
    ):
        with pytest.raises(FeaturesNotAvailableError):
            csv_export_job = JobHandler().create_and_start_job(
                user, AuditLogExportJobType.type, sync=True, **csv_settings
