...
from fwego.core.encoders import JSONEncoderSupportingDataClasses
from fwego.core.jobs.models import Job
from fwego.core.mixins import CreatedAndUpdatedOnMixin
...
class AuditLogEntry(CreatedAndUpdatedOnMixin, models.Model):
    ...
    class Meta:
        ...
        # Note: the index name will be `fwego_ent_action__8db5d6_idx`
        # (when `workspace_id` used to be called `group_id`), but its true name
        # is `fwego_ent_action__ca13aa_idx`. See enterprise migration 0016.
        indexes = [
            models.Index(
