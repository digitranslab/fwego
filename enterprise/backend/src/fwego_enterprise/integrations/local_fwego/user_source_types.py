...
from fwego.contrib.database.fields.exceptions import FieldDoesNotExist
...
from fwego.contrib.database.rows.actions import CreateRowsActionType
...
from fwego.core.app_auth_providers.handler import AppAuthProviderHandler
...
from fwego.core.user_sources.registries import UserSourceCount, UserSourceType
from fwego.core.user_sources.types import UserSourceDict
...
class LocalFwegoUserSourceType(UserSourceType):
    type = "local_fwego"
    model_class = LocalFwegoUserSource
    ...
    # `userSourceTypes.js::LocalFwegoUserSourceType::allowedRoleFieldTypes`
    ...
    # that the `LocalFwegoUserSource` is considered "configured".
    fields_to_configure = [
        "table_id",
        "name_field_id",
        "email_field_id",
    ...
    def prepare_values(
        ...
        values: Dict[str, Any],
        user: AbstractUser,
        instance: Optional[LocalFwegoUserSource] = None,
    ) -> Dict[str, Any]:
    ...
    def after_update(
        self,
        user: AbstractUser,
        user_source: LocalFwegoUserSource,
        values: Dict[str, Any],
    ...
    def get_user_model(self, user_source: LocalFwegoUserSource):
        try:
    ...
    def is_configured(self, user_source: LocalFwegoUserSource) -> bool:
        """
    ...
    def gen_uid(self, user_source: LocalFwegoUserSource):
        """
    ...
    def get_user_role(self, user, user_source: LocalFwegoUserSource) -> str:
        """
    ...
    def list_users(
        self, user_source: LocalFwegoUserSource, count: int = 5, search: str = ""
    ):
        """
    ...
    def get_roles(self, user_source: LocalFwegoUserSource) -> List[str]:
        """
    ...
    def get_user(self, user_source: LocalFwegoUserSource, **kwargs):
        """
    ...
    def create_user(self, user_source: LocalFwegoUserSource, email, name, role=None):
        """
    ...
    def authenticate(self, user_source: LocalFwegoUserSource, **kwargs):
        """
        ...
        try:
            ...
            auth_provider = LocalFwegoPasswordAppAuthProvider.objects.get(
                user_source=user_source.id
            )
        except LocalFwegoPasswordAppAuthProvider.DoesNotExist as exc:
    ...
    def _get_cached_user_count(
        self, user_source: LocalFwegoUserSource
    ) -> Optional[UserSourceCount]:
        """
        Given a `user_source`, return the cached user count if it exists.

        :param user_source: The `LocalFwegoUserSource` instance.
    ...
    def _generate_update_user_count_cache_key(
        ...
        return f"local_fwego_user_source_{user_source.id}_user_count"
    ...
    def after_user_source_update_requires_user_recount(
        ...
        """
        By default, the Local Fwego user source type will re-count
    ...
    def update_user_count(
        self,
        user_sources: QuerySet[LocalFwegoUserSource] = None,
    ) -> Optional[UserSourceCount]:
        """
        ...
        for all configured `LocalFwegoUserSource`.
        ...
        for table in tables:
            ...
            for user_source_using_table in user_sources_using_table:
                ...
                cache.set(
                    ...
                    timeout=settings.FWEGO_ENTERPRISE_USER_SOURCE_COUNTING_CACHE_TTL_SECONDS,
                )
    ...
    def get_user_count(
        self, user_source: LocalFwegoUserSource, force_recount: bool = False
    ) -> Optional[UserSourceCount]:
        """
