...
from fwego_premium.fields.field_types import AIFieldType
from fwego_premium.fields.registries import ai_field_output_registry
from fwego_premium.license.handler import LicenseHandler
from rest_framework import serializers

from fwego.contrib.database.data_sync.exceptions import SyncError
from fwego.contrib.database.data_sync.models import DataSyncSyncedProperty
from fwego.contrib.database.data_sync.registries import DataSyncProperty, DataSyncType
from fwego.contrib.database.data_sync.utils import (
    compare_date,
    update_fwego_field_select_options,
)
...
from fwego.core.db import specific_iterator, specific_queryset
from fwego.core.handler import CoreHandler
from fwego.core.utils import ChildProgressBuilder
from fwego_enterprise.features import DATA_SYNC

from .models import LocalFwegoTableDataSync
...
# List of Fwego supported field types that can be included in the data sync.
...
class RowIDDataSyncProperty(DataSyncProperty):
    ...
    def to_fwego_field(self) -> NumberField:
        return NumberField(
            name=self.name, number_decimal_places=0, number_negative=False
        )


class FwegoFieldDataSyncProperty(DataSyncProperty):
    ...
    def get_metadata(self, fwego_field, existing_metadata=None):
        new_metadata = super().get_metadata(fwego_field, existing_metadata)
        field_type = field_type_registry.get_by_model(fwego_field)
        ...
        select_options_mapping = update_fwego_field_select_options(
    ...
    def is_equal(self, fwego_row_value: Any, data_sync_row_value: Any) -> bool:
        ...
        # compare loosely.
        if isinstance(fwego_row_value, date) and isinstance(
            data_sync_row_value, datetime
        ):
            return compare_date(fwego_row_value, data_sync_row_value)
        # The fwego row value is converted to a string, so we would need to convert
        ...
        return super().is_equal(fwego_row_value, data_sync_row_value)


class LocalFwegoTableDataSyncType(DataSyncType):
    type = "local_fwego_table"
    model_class = LocalFwegoTableDataSync
