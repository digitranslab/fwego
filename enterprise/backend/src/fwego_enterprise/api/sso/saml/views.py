...
from fwego.core.auth_provider.exceptions import DifferentAuthProvider
from fwego.core.exceptions import WorkspaceInvitationEmailMismatch
from fwego.core.user.exceptions import DeactivatedUserException, DisabledSignupError
from fwego_enterprise.api.sso.saml.errors import ERROR_SAML_INVALID_LOGIN_REQUEST
from fwego_enterprise.api.sso.saml.serializers import SAMLResponseSerializer
from fwego_enterprise.api.sso.serializers import SsoLoginRequestSerializer
from fwego_enterprise.api.sso.utils import (
    SsoErrorCode,
    map_sso_exceptions,
...
class AssertionConsumerServiceView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            ...
            "Sign in the user if already exists in Fwego or create a new one otherwise. "
...
class FwegoInitiatedSingleSignOn(APIView):
    permission_classes = (AllowAny,)
    ...
    @extend_schema(
        ...
        description=(
            ...
            "SSO SAML login from Fwego (the service provider). The user will be "
...
class AdminAuthProvidersLoginUrlView(APIView):
    ...
    def get(self, request: Request, query_params: Dict[str, Any]) -> Response:
        """Return the correct URL to initiate a SSO SAML login from fwego."""
