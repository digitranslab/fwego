...
help:
	@echo "fwego make. available targets:"
...
.docker-build-standalone-images:
	docker build -f backend/Dockerfile . -t fwego_backend
	docker build -f web-frontend/Dockerfile . -t fwego_web-frontend
...
docker-allinone-shell:
	$(DOCKERC) $(DOCKER_ALL_IN_ONE_CONF) exec fwego_all_in_one bash

docker-allinone-attach:
	$(DOCKERC) $(DOCKER_ALL_IN_ONE_CONF) attach fwego_all_in_one

docker-allinone-logs:
	$(DOCKERC) $(DOCKER_ALL_IN_ONE_CONF) logs -tf fwego_all_in_one
