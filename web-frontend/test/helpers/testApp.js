import setupCore from '@fwego/modules/core/plugin'
import Papa from 'papaparse'
import axios from 'axios'
import setupClient, {
  ClientErrorMap,
} from '@fwego/modules/core/plugins/clientHandler'

import setupDatabasePlugin from '@fwego/modules/database/plugin'
import setupBuilderPlugin from '@fwego/modules/builder/plugin'
import setupIntegrationPlugin from '@fwego/modules/integrations/plugin'
...
import setupHasFeaturePlugin from '@fwego/modules/core/plugins/hasFeature'

/**
 * Uses the real fwego plugins to setup a Vuex store and fwego registry
 * correctly.
 */
function _createFwegoStoreAndRegistry(app, vueContext, extraPluginSetupFunc) {
...
/**
 * An acceptance testing framework for testing Fwego components and surrounding logic
 * like stores.
 * TestApp sets up fwego components, registries and stores so they work out of the
 ...
 *                      fwego server with consistent test data.
...
export class TestApp {
  constructor(extraPluginSetupFunc = null) {
    ...
    this._app = {
      ...
      $config: {
        ...
        FWEGO_USE_PG_FULLTEXT_SEARCH: 'true',
        FWEGO_FRONTEND_SAME_SITE_COOKIE: 'lax',
      },
      i18n: {
        t: (key) => key,
        tc: (key) => key,
    ...
    this.store = _createFwegoStoreAndRegistry(
      this._app,
      this._vueContext,
  ...
  async mount(Component, { asyncDataParams = {}, ...kwargs }) {
    // Sometimes fwego directly appends to the documents body, ensure that we
...
/**
 * Various helper functions which interact with fwego components. Lean towards
 * putting and sharing any test code which relies on specific details of how fwego
