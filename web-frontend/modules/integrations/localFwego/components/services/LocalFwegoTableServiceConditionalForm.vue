<template>
  <div v-if="value">
    <div v-if="value.length === 0">
      <div class="filters__none">
        <div class="filters__none-title">
          {{ $t('localFwegoTableServiceConditionalForm.noFilterTitle') }}
        </div>
        <div class="filters__none-description">
          {{ $t('localFwegoTableServiceConditionalForm.noFilterText') }}
        </div>
      </div>
    </div>
    ...
    >
      ...
      >
        <InjectedFormulaInput
          ...
          :placeholder="
            $t(
              'localFwegoTableServiceConditionalForm.formulaFilterInputPlaceholder'
            )
          "
        />
      </template>
        ...
        <a
          ...
          :title="
            !filter.value_is_formula
              ? $t('localFwegoTableServiceConditionalForm.useFormulaForValue')
              : $t('localFwegoTableServiceConditionalForm.useDefaultForValue')
          "
    ...
    <div class="filters_footer">
      ...
      >
        {{ $t('localFwegoTableServiceConditionalForm.addFilter') }}
      </ButtonText>
    </div>
  </div>
</template>
...
<script>
import ViewFieldConditionsForm from '@fwego/modules/database/components/view/ViewFieldConditionsForm.vue'
...
export default {
  name: 'LocalFwegoTableServiceConditionalForm',
  components: {
    InjectedFormulaInput,
    ViewFieldConditionsForm,
  },
      ...
      try {
        ...
        if (field === undefined) {
          await this.$store.dispatch('toast/error', {
            title: this.$t(
              'localFwegoTableServiceConditionalForm.noCompatibleFilterTypesErrorTitle'
            ),
            message: this.$t(
              'localFwegoTableServiceConditionalForm.noCompatibleFilterTypesErrorMessage'
            ),
          })
        } else {
