  ...
  <div v-if="value">
    <div v-if="value.length === 0" class="sortings__none">
      <div class="sortings__none-title">
        {{ $t('localFwegoTableServiceSortForm.noSortTitle') }}
      </div>
      <div class="sortings__none-description">
        {{ $t('localFwegoTableServiceSortForm.noSortText') }}
      </div>
    </div>
      ...
      >
        ...
        <div class="sortings__description">
          <template v-if="index === 0">{{
            $t('localFwegoTableServiceSortForm.sortBy')
          }}</template>
          <template v-if="index > 0">{{
            $t('localFwegoTableServiceSortForm.thenBy')
          }}</template>
        </div>
    ...
    <template v-if="value.length < availableFieldsLength && !disableSort">
      <div ref="addContextToggle">
        <ButtonText
          ...
          "
        >
          {{ $t('localFwegoTableServiceSortForm.addSort') }}
        </ButtonText>
      </div>
      <Context
        ref="addContext"
...
export default {
  name: 'LocalFwegoTableServiceSortForm',
  mixins: [context],
  props: {
    value: {
      type: Array,
