export default {
  ...
  computed: {
    /**
     * Used by `LocalFwegoTableServiceConditionalForm` so that when read,
    ...
    /**
     * Used by `LocalFwegoTableServiceSortForm` so that when read, we
    ...
    /**
     * Used by `LocalFwegoTableSelector` so that when read, we return the
  ...
  methods: {
    ...
    excludeTrashedFields(value) {
      const localFwegoFieldIds = this.tableFields.map(({ id }) => id)
      return value.filter(({ field }) => localFwegoFieldIds.includes(field))
    },
  },
}
