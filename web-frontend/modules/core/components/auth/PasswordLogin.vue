...
export default {
  ...
  async mounted() {
    if (!this.$config.FWEGO_DISABLE_PUBLIC_URL_CHECK) {
        ...
        try {
          ...
          this.showError(
            ...
            `Cannot connect to the backend at ${publicBackendUrlWithProto}.` +
              ` You visited Fwego at ${browserWindowUrl} ` +
              ' which indicates you have mis-configured the Fwego ' +
              ' FWEGO_PUBLIC_URL or PUBLIC_BACKEND_URL environment variables. ' +
              ' Please visit https://fwego.io/docs/tutorials/debugging-connection-issues ' +
