/**
 * Formula autocompleting in Fwego works in two steps:
...
import { getTokenStreamForFormula } from '@fwego/modules/core/formula/parser/parser'
...
function _countRemainingOpenBrackets(i, stop, stream, numOpenBrackets) {
  for (let k = i; k < stop; k++) {
    ...
    if (afterToken.type === FwegoFormula.OPEN_PAREN) {
      numOpenBrackets++
    }
    if (afterToken.type === FwegoFormula.CLOSE_PAREN) {
      numOpenBrackets--
    }
  }
...
export function _calculateAutocompleteRangeAndType(formula, cursorPosition) {
  ...
  for (let i = 0; i < stream.tokens.length; i++) {
    ...
    if (isNormalToken) {
      if (token.type === FwegoFormula.OPEN_PAREN) {
        numOpenBrackets++
      }
      if (token.type === FwegoFormula.CLOSE_PAREN) {
        numOpenBrackets--
      }
      if (insideFieldRef) {
        if (token.type === FwegoFormula.CLOSE_PAREN) {
      ...
      if (token.type === FwegoFormula.FIELD) {
    ...
    if (output.length >= cursorPosition) {
      ...
      const insideFunctionRef = [
        FwegoFormulaLexer.IDENTIFIER,
        FwegoFormulaLexer.FIELD,
        FwegoFormulaLexer.LOOKUP,
        FwegoFormulaLexer.IDENTIFIER_UNICODE,
      ].includes(token.type)
