...
FWEGO_WEBFRONTEND_PORT="${FWEGO_WEBFRONTEND_PORT:-3000}"

show_help() {
    echo """
The available Fwego web-frontend related commands and services are shown below:
...
build-local  : Triggers a nuxt re-build of Fwego's web-frontend.
...
setup_additional_modules(){
  ...
  for plugin_dir in "$FWEGO_PLUGIN_DIR"/*; do
...
case "$1" in
    ...
    nuxt)
      startup_plugin_setup
      setup_additional_modules
      exec ./node_modules/.bin/nuxt start --hostname "${FWEGO_WEBFRONTEND_BIND_ADDRESS:-0.0.0.0}" --port "$FWEGO_WEBFRONTEND_PORT" "${@:2}"
    ;;
    nuxt-local)
      startup_plugin_setup
      setup_additional_modules
      exec ./node_modules/.bin/nuxt start --hostname "${FWEGO_WEBFRONTEND_BIND_ADDRESS:-0.0.0.0}" --port "$FWEGO_WEBFRONTEND_PORT" --config-file ./config/nuxt.config.local.js "${@:2}"
    ;;
    storybook-dev)
      startup_plugin_setup
      setup_additional_modules
    ...
    ;;
    install-plugin)
      exec /fwego/plugins/install_plugin.sh "${@:2}"
    ;;
    uninstall-plugin)
