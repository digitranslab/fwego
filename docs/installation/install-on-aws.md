...
This guide will walk you through picking the best way to deploy Fwego to AWS, what
...
Fwego can be deployed to AWS in the following ways:
...
2. Using
   this [docker-compose](https://github.com/digitranslab/fwego/-/blob/develop/docker-compose.no-caddy.yml)
...
3. Using the [Fwego community maintained helm chart](./install-with-helm.md) and EKS.
...
All of these deployment methods will store Fwego's data and state in RDS and S3 making
switching between them (for example migrating to using EKS later on)
straightforward.

## Prerequisites for deploying Fwego to AWS

We will go through these in more detail later in the guide but to give you a quick
overview this is what any AWS deployment of Fwego will need:
...
* An AWS IAM account with privileges to upload to the S3 bucket.
    * Specifically their AWS Access Key ID and Secret Access Key will be needed to
      configure Fwego to upload into the bucket.
    * A VPC
    * A non-clustered Redis
      server (Elasticache with TLS on and cluster mode off)
    *
        * A SMTP email server for
          Fwego to send emails with.

## Option 1) Deploying the all-in-one image to Fargate/ECS

The `digitranslab/fwego:1.30.1` image runs all of Fwego’s various services inside the
...
* Simpler to use and setup compared to the one container per-service model below
  because:
    * You don't need to worry about configuring and linking together the different
      services that make up a Fwego deployment.
    * Configuring load balancers is easier as you can just directly route through all
      requests to any horizontally scaled container running `digitranslab/fwego:1.30.1`.
...
* Potentially higher resource usage overall as each of the all-in-one containers will
  ...
  specific services.
    * For example if you deploy 10 `digitranslab/fwego:1.30.1` containers horizontally you
...
* Harder to isolate failures specific to certain services as the logs per container will
  include multiple services.
    * However, Fwego fully
      supports [Open Telemetry](./monitoring.md) which lets you hook Fwego up to many
...
First Fwego needs an S3 bucket. Fwego will use this bucket to store files uploaded
by users into tables and view/table exports for users to then download.

Fwego will then generate pre-signed S3 URLs for the user to view and download files
...
We recommend setting up a separate IAM user who Fwego will be configured with
...
This bucket will also in the current version of Fwego need CORS rules set for the
...
allowed_headers = ["*"]
allowed_methods = ["GET"]
allowed_origins = ["REPLACE_WITH_YOUR_FWEGO_PUBLIC)URL"]
expose_headers  = ["ETag"]
...
Fwego stores all of its non-file data in a PostgreSQL database. In AWS we recommend
using an RDS Postgres cluster. You will later need to configure Fwego to be able to
access this cluster.

Fwego uses PostgreSQL heavily so scaling the RDS cluster up will be needed for larger
deployments.
...
> completes. Fwego is therefore incompatible because it makes schema changes, and if
...
Fwego uses Redis as a cache, for real-time collaboration over WebSockets and async
background task processing.

We recommend setting up an Elasticache Redis in non-cluster mode with TLS enabled and in
AUTH mode where you can specify an AUTH token password for Fwego to later connect to
the Redis server.

Generally, the Redis server is not the bottleneck in Fwego deployments as they scale.

#### 4) Setting up an ALB and target group

Now create a target group on port 80 and ALB ready to route traffic to the Fwego
containers.

When setting up the health check for the ALB the `digitranslab/fwego:1.30.1` container
...
#### 5) Launching Fwego on ECS/Fargate

Now we are ready to spin up our `digitranslab/fwego:1.30.1` containers. See below for a
...
1. Select the `digitranslab/fwego:1.30.1` image.
...
| `FWEGO_PUBLIC_URL`          | The public URL or IP that will be used to access fwego in your user's browser. Always should start with http:// https:// even if accessing via an IP address.                                                                                                                                                                                                                                                                           |
| `DATABASE_HOST`               | The hostname of the Postgres database Fwego will use to store its data in.                                                                                                                                                                                                                                                                                                                                                              |
| `DATABASE_USER`               | The username of the database user Fwego will use to connect to the database at `DATABASE_HOST`.                                                                                                                                                                                                                                                                                                                                         |
| `DATABASE_PORT`               | The port Fwego will use when trying to connect to the Postgres database at `DATABASE_HOST`.                                                                                                                                                                                                                                                                                                                                             |
| `DATABASE_NAME`               | The database name Fwego will use to store data.                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `AWS_ACCESS_KEY_ID`           | The access key for your S3 IAM AWS account. When set to anything other than empty will switch Fwego to use a S3 compatible bucket for storing user file uploads.                                                                                                                                                                                                                                                                        |
...
| `FWEGO_EXTRA_ALLOWED_HOSTS` | An optional comma-separated list of hostnames which will be added to Fwego’s Django backend ALLOWED_HOSTS setting. Add your ALB IP address here so the health checks it sends are allowed through, or alternatively configure the less secure value `*` to get things running and restrict hosts later once everything is working.                                                                                                      |
| `FWEGO_JWT_SIGNING_KEY`     | **Must be set so all the containers share the same signing key.** The signing key is used to sign the content of generated tokens. For HMAC signing, this should be a random string with at least as many bits of data as is required by the signing protocol. See [here](https://django-rest-framework-simplejwt.readthedocs.io/en/latest/settings.html#signing-key) for more details. `FWEGO_JWT_SIGNING_KEY_FILE` is also supported. |
...
| `EMAIL_SMTP_*`                | There are a number of SMTP related environment variables documented in our environment variable guide [here](./configuration.md) which will also need to be set so Fwego can send invitation and password reset emails.                                                                                                                                                                                                                 |
  ...
  [
    {
      ...
      "logConfiguration": {                     #logs are not mandatory
                ...
                "options": {
                    ...
                    "awslogs-group" : "/ecs/fwego_log",
                    "awslogs-stream-prefix" : "fwego",
      ...
      {
        "name": "FWEGO_EXTRA_ALLOWED_HOSTS",
        "value": "<YOUR_ALLOWED_HOSTS>"
      },
      {
        "name": "FWEGO_JWT_SIGNING_KEY",
...
Other than launching more `digitranslab/fwego` tasks and scaling up the RDS postgres
server, the `digitranslab/fwego` image has the following scaling environment variables
...
1. `FWEGO_AMOUNT_OF_GUNICORN_WORKERS` controls the number of REST API workers (
...
2. `FWEGO_AMOUNT_OF_WORKERS` controls the number of background task celery runners,
...
3. You can make the image launch fewer internal processes and hence reduce memory usage
   by setting `FWEGO_RUN_MINIMAL=yes` AND `FWEGO_AMOUNT_OF_WORKERS=1`.
    ...
    2. The consequence of this is that there is only one process handling tasks per
       container and so a slow task such as a snapshot of a large Fwego database might
...
You should now have a fully running Fwego cluster. The first user to sign-up becomes
the first "staff" instance-wide admin user. This user can then configure Fwego's
in-tool settings, active enterprise licenses, promote other users to being staff etc.

## Option 2) Deploying Fwego as separate services to Fargate/ECS

The `fwego/backend:1.30.1` and `fwego/web-frontend:1.30.1` images allow you to run
Fwego's various services as separate containers.
...
Later on the Fwego `web-frontend` service will need to be able to communicate with
...
#### 5) Deploying Fwego's individual services

Let's now deploy each of Fwego's individual services to Fargate/ECS. Make a
new cluster for Fwego and then proceed to make the following task definitions.

> If you are familiar with K8S then [this sample config](./install-with-k8s.md) gives an
> overview of the services.
>
Alternatively [this docker-compose](https://github.com/digitranslab/fwego/-/blob/develop/docker-compose.no-caddy.yml)
...
1. In the task defintion use the `fwego/backend:1.30.1` image
...
1. Use the `fwego/backend:1.30.1`
...
1. Use the `fwego/backend:1.30.1` image with `celery-worker` as the image command.
...
1. Use the `fwego/backend:1.30.1` image.
...
1. Use the `fwego/web-frontend:1.30.1` image with no arguments needed.
...
* `FWEGO_PUBLIC_URL`:
  The public URL or IP that will be used to access fwego in your user's browser.
...
* `PRIVATE_BACKEND_URL`: The web-frontend containers need to be able to send HTTP
  requests to the `backend-wsgi` containers running the rest API.
    ...
    * Ensure the security groups are set up correctly to allow these internal HTTP
      requests.
    * If you've not set `FWEGO_EXTRA_ALLOWED_HOSTS=*` on the `backend-wsgi`
      and `backend-asgi` containers make sure to add the value you've set for this env
      var to `FWEGO_EXTRA_ALLOWED_HOSTS` for those services otherwise they will not
...
> single ECS service for all of Fwego's tasks, however you will then need to use
...
> to route properly to each individually. Set the `FWEGO_BACKEND_PORT` env var
...
Fwego UI stuck at 0%) then you can add more `celery-exportworker` services.
...
You should now have a fully running Fwego cluster. This deployment method is more
complex to get working so if you need any help please post in
our [community forums](https://community.fwego.io).

The first user to sign-up becomes the first "staff" instance-wide admin user. This user
can then configure Fwego's in-tool settings, active enterprise licenses, promote other
users to being staff etc.

## Upgrading Fwego

Upgrading an ECS/Fargate deployment of Fwego can be done by.
...
4. The first new `digitranslab/fwego` or `fwego/backend-wsgi/asgi` container to startup
   will
   automatically apply any required database migrations and upgrades.
5. Once these are complete, all the new Fwego containers will start accepting requests
...
If you see this error in your logs it means you have launched Fwego with a Redis which
is in cluster mode. Fwego uses libraries that do not support Redis in cluster mode, so
you will need to provision a new Redis in the non-cluster mode for things to work.
Non-cluster mode Redis can still be scaled and multi-zone, additionally Fwego does not
...
### I get `CORS errors` when downloading a file from a Fwego file field
