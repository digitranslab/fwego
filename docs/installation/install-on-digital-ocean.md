...
step-by-step guide that helps you install Fwego from scratch in a scalable way.
...
the server specs you would like to have. Fwego is compatible with 1vCPU and 1GB
RAM.
...
server specs you would like to have. Fwego is compatible with 1vCPU and 1GB RAM.
...
Repository: `digitranslab/fwego`
Image tag or digest: `1.30.1`

Click on `Next`, then on the `Edit` button of the `fwego-fwego` web service. Here
you must change the HTTP Port to 80, and then click on `Back`. Click on the `Next`
button, and then on `Edit` next to the `fwego-fwego` environment variables. Add the
following environment variables. Everything between brackets must be replaced.

Generate a unique secret string for `SECRET_KEY` and `FWEGO_JWT_SIGNING_KEY`. This
can for example be done via https://djecrety.ir/.

```
SECRET_KEY=(generate a unique random string)
FWEGO_JWT_SIGNING_KEY=(generate a unique random string)
FWEGO_PUBLIC_URL=http://localhost
FWEGO_AMOUNT_OF_GUNICORN_WORKERS=2
FWEGO_AMOUNT_OF_WORKERS=2
DATABASE_URL=(YOUR_POSTGRESQL_CONNECTION_STRING)
REDIS_URL=(YOUR_REDIS_CONNECTION_STRING)
DISABLE_VOLUME_CHECK=yes
FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION=false
...
visible in the settings. Copy this URL, click on the fwego-fwego component settings,
edit the environment variables and set `FWEGO_PUBLIC_URL` to `(the copied URL)`, then
hit the save button. Note that the URL does not have a trailing slask, so it should
look like `https://fwego.io` and not `https://fwego.io/`.
...
environment variable `FWEGO_TRIGGER_SYNC_TEMPLATES_AFTER_MIGRATION` to `true`.

### Spaces

In order for the download button to work in Fwego you would need to configure the CORS
...
Wait until the build restarts, and then visit the copied URL to make use of your Fwego
environment.

## Update version

In order to update the Fwego version, you simply need to replace the image tag.
Navigate to the `Settings` tag of your created app, click on the `fwego-fwego`
...
If you're going to use Fwego with more concurrent users, have big database schemas,
...
- 4GB of RAM:
    ...
    - `FWEGO_AMOUNT_OF_WORKERS=4`
- 8GB of RAM:
    - `FWEGO_AMOUNT_OF_GUNICORN_WORKERS=10`
    - `FWEGO_AMOUNT_OF_WORKERS=8`
- 16GB of RAM:
    - `FWEGO_AMOUNT_OF_GUNICORN_WORKERS=20`
    - `FWEGO_AMOUNT_OF_WORKERS=16`
