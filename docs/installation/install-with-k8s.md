...
It assumes you want the most performant version of Fwego possible and
...
environment variables below. See [Configuring Fwego](./configuration.md) for more
details on these variables.

You should also set up and configure Fwego to use an S3 compatible storage service
for uploading and serving user uploaded files.

## Example fwego.yml

```yaml
# A secret containing all the required Fwego settings.
apiVersion: v1
kind: Secret
metadata:
...
stringData:
  ...
  # See the docs for more info https://fwego.io/docs/installation%2Fconfiguration#user-file-upload-configuration
          ...
          readinessProbe:
            exec:
              command:
                - /bin/bash
                - -c
                - /fwego/backend/docker/docker-entrypoint.sh celery-exportworker-healthcheck
            initialDelaySeconds: 10
            timeoutSeconds: 10
            periodSeconds: 10
          envFrom:
