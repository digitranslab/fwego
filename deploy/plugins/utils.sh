...
FWEGO_PLUGIN_DIR=${FWEGO_PLUGIN_DIR:-/fwego/plugins}
...
startup_plugin_setup(){
  if [[ -z "${FWEGO_PLUGIN_SETUP_ALREADY_RUN:-}" ]]; then
    if [[ -z "${FWEGO_DISABLE_PLUGIN_INSTALL_ON_STARTUP:-}" ]]; then
      ...
      for plugin_dir in "$FWEGO_PLUGIN_DIR"/*/; do
        ...
        if [[ -d "$plugin_dir" ]]; then
          /fwego/plugins/install_plugin.sh --runtime --folder "$plugin_dir"
        fi
      done

      # Make sure any plugins configured via the environment variable are installed.
      for url in $(echo "${FWEGO_PLUGIN_URLS:-}" | tr "," "\n")
      do
        log "Downloading and installing the plugin found at $url"
        /fwego/plugins/install_plugin.sh --runtime --url "$url"
      done

      for repo in $(echo "${FWEGO_PLUGIN_GIT_REPOS:-}" | tr "," "\n")
      do
        log "Downloading and installing the plugin found at $repo"
        /fwego/plugins/install_plugin.sh --runtime --git "$repo"
      done

      # Ensure we don't run this function multiple times in the same shell.
      export FWEGO_PLUGIN_SETUP_ALREADY_RUN="yes"
    else
      log "Not installing any plugins found in $FWEGO_PLUGIN_DIR or set in the
      FWEGO_PLUGIN_URLS or FWEGO_PLUGIN_GIT_REPOS env variables as
      FWEGO_DISABLE_PLUGIN_INSTALL_ON_STARTUP is set."
    fi
  fi
}
