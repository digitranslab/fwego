...
show_help() {
    echo """
Usage: docker run --rm -it [-v fwego:$DATA_DIR] fwego COMMAND_FROM_BELOW
Commands
start         : Launches fwego with all services running internally in a single
                container.
start-only-db : Starts up only the embedded postgres server and makes it available
                ...
                need to manually inspect Fwegos database etc.
...
if [[ -z "${1:-}" ]]; then
  echo "Must provide arguments to fwego"
  show_help
  exit 1
fi
...
shopt -s nullglob
for f in /fwego/supervisor/env/*.sh; do
...
source /fwego/supervisor/default_fwego_env.sh
...
if [[ -z "${DISABLE_VOLUME_CHECK:-}" ]]; then
  ...
  if [ -z "$mountVar" ]
  then
  echo -e "\e[33mPlease run fwego with a mounted data folder " \
          "'docker run -v " \
          "fwego_data:/fwego/data ...', otherwise your data will be lost between " \
...
if [[ "$DATABASE_HOST" == *"localhost"* && -z "${DISABLE_LOCALHOST_CHECK:-}" ]]; then
  echo -e "\e[31mERROR: You have set DATABASE_HOST to localhost but this will mean "\
          "localhost in the Fwego container and not on your host machine. "\
          "To connect Fwego to a db running on your host instead then you need to"\
...
if [[ "$DATABASE_HOST" == "embed" && -z "${DATABASE_URL:-}" ]]; then
  ...
  export FWEGO_EMBEDDED_PSQL=true
else
  ...
  startup_echo "Using provided external postgres at ${DATABASE_HOST:-} or the " \
               "DATABASE_URL"
  export FWEGO_EMBEDDED_PSQL=false
fi
...
if [[ "$REDIS_HOST" == *"localhost"* && -z "${DISABLE_LOCALHOST_CHECK:-}" ]]; then
  echo -e "\e[31mERROR: You have set REDIS_HOST to localhost but this will mean "\
          "localhost in the Fwego container and not on your host machine. "\
          "To connect Fwego to a redis running on your host instead then you need to"\
          " run docker with the following flags: \n\n"\
          "--add-host host.docker.internal:host-gateway "\
...
if [[ "$REDIS_HOST" == "embed" && -z "${REDIS_URL:-}" ]]; then
  ...
  startup_echo "Using embedded fwego redis as no REDIS_HOST or REDIS_URL provided. "
else
...
file_env FWEGO_JWT_SIGNING_KEY
create_secret_env_if_missing .jwt_signing_key FWEGO_JWT_SIGNING_KEY
...
run_cmd_with_db(){
    if [[ $(pgrep -f "redis") && $(pgrep -f "bin/postgres") ]]; then
      ...
      export SUPERVISOR_CONF=/fwego/supervisor/supervisor_include_only.conf
      /fwego/supervisor/start.sh "${@:2}" &
      SUPERVISORD_PID=$!
      ...
      trap finish EXIT
      /fwego/backend/docker/docker-entrypoint.sh wait_for_db
      startup_echo "======== RUNNING COMMAND ========="
...
case "$1" in
    start)
      exec /fwego/supervisor/start.sh "${@:2}"
    ;;
    backend-cmd-with-db)
      run_cmd_with_db gosu "$DOCKER_USER" /fwego/backend/docker/docker-entrypoint.sh "${@:2}"
    ;;
    start-only-db)
      check_can_start_embedded_services
      ...
      export SUPERVISOR_CONF=/fwego/supervisor/supervisor_include_only.conf
      startup_echo "INFO: You can find the fwego database user's password by running"
      startup_echo "docker exec -it fwego cat $DATA_DIR/.pgpass"
      exec /fwego/supervisor/start.sh "${@:2}"
    ;;
    backend-cmd)
      cd /fwego/backend
      docker_safe_exec /fwego/backend/docker/docker-entrypoint.sh "${@:2}"
    ;;
    web-frontend-cmd)
      cd /fwego/web-frontend
      docker_safe_exec /fwego/web-frontend/docker/docker-entrypoint.sh "${@:2}"
    ;;
    install-plugin)
      exec /fwego/plugins/install_plugin.sh --runtime "${@:2}"
    ;;
    uninstall-plugin)
      export FWEGO_DISABLE_PLUGIN_INSTALL_ON_STARTUP="on"
      ...
      run_cmd_with_db /fwego/plugins/uninstall_plugin.sh "${@:2}"
    ;;
    list-plugins)
