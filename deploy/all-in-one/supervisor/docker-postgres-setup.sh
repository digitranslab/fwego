...
_main() {
    ...
    ALREADY_SETUP_INDICATOR_FILE="$PGDATA/fwego_db_setup"
    ...
    if [ "$(id -u)" = '0' ]; then
      ...
      if [ "$1" == "upgrade" ]; then
        if [ $EXIT_STATUS = 0 ]; then
          ...
          echo
          echo 'You can now run the official `digitranslab/fwego:1.30.1` image to start Fwego.'
          echo
    ...
    # this part is reserved for the postgres user
      ...
      if [ "$PGDATA_VERSION" = $POSTGRES_OLD_VERSION ]; then
        ...
        echo "Please look into Fwego official documentation for more information on how to upgrade your database."
        echo
        exit 1
      fi
      ...
      if [ -f "$ALREADY_SETUP_INDICATOR_FILE" ]; then
        ...
        if [ "$PGDATA_VERSION" != "$POSTGRES_VERSION" ]; then
          echo
          echo "Your PostgreSQL data directory was initialized with version $PGDATA_VERSION, but this image is running version $POSTGRES_VERSION."
          echo "Please look into official Fwego documentation at https://fwego.io/docs/installation%2Finstall-with-docker#upgrading-postgresql-database-from-a-previous-version for more information on how to upgrade your database using a different Fwego image ('digitranslab/fwego-pgautoupgrade:1.30.1') or how to run Fwego using legacy PostgreSQL 11 image ('digitranslab/fwego-pg11:1.30.1')."
          echo
          exit 1
        fi
      else
