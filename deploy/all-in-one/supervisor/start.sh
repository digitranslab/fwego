...
# Sets up and starts Fwego and all of its required services using supervisord.
...
=========================================================================================
EOF
cat /fwego/supervisor/STARTUP_README.md
...
SUPERVISOR_DISABLED_CONF_DIR=/fwego/supervisor/includes/disabled
SUPERVISOR_ENABLED_CONF_DIR=/fwego/supervisor/includes/enabled

if [[ "$DATABASE_HOST" == "localhost" && -z "${DATABASE_URL:-}" ]]; then
  startup_echo "Running setup of embedded fwego database."
  ...
  # Setup an empty fwego database with the provided user and password.
  POSTGRES_SETUP_SCRIPT_COMMAND=${POSTGRES_SETUP_SCRIPT_COMMAND:-setup}
  ./fwego/supervisor/wrapper.sh GREEN POSTGRES_INIT ./fwego/supervisor/docker-postgres-setup.sh ${POSTGRES_SETUP_SCRIPT_COMMAND}
...
# = LOG ABOUT URL
# ========================
if [[ "$FWEGO_PUBLIC_URL" == "http://localhost"* ]]; then
  startup_echo "No FWEGO_PUBLIC_URL environment variable provided. Starting fwego locally at http://localhost without automatic https."
else
  startup_echo "Starting Fwego using addresses $FWEGO_PUBLIC_URL, if any are https automatically Caddy will attempt to setup HTTPS automatically."
fi
...
# = STARTUP SUPERVISOR
# ========================
startup_echo "Starting all Fwego processes:"
exec /usr/bin/supervisord --configuration "${SUPERVISOR_CONF:-/fwego/supervisor/supervisor.conf}"
