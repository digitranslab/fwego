...
# Keep in sync with src/fwego/config/settings/base.py:594
...
for _ in $(seq 1 "${FWEGO_E2E_STARTUP_MAX_WAIT_TIME_SECONDS:-60}")
do
  echo 'Waiting for backend, web-frontend and synced templates to be ready'
  if fwego_ready; then
    echo 'Fwego is ready! Exiting with success code.'
    exit 0
  fi
  sleep 1
done
