...
# the grammar files mounted into to, then generates the appropriate parsers for Fwego
...
if [ ! -f FwegoFormula.g4 ] || [ ! -f FwegoFormulaLexer.g4 ] || [ ! -d ../backend ] ; then
    echo "${RED}Grammar files or fwego code not found in build scripts directory, cannot continue...${NC}"
    exit 1
fi
...
docker run -it --rm \
    ...
    "$(docker build -q --build-arg UID="$USER_ID" --build-arg GID="$GROUP_ID" . )" sh -c "
        cd src
        java -jar ../antlr.jar -Dlanguage=JavaScript -o out/frontend_parser -visitor -listener FwegoFormulaLexer.g4 FwegoFormula.g4
        java -jar ../antlr.jar -Dlanguage=Python3 -o out/backend_parser -visitor -listener FwegoFormulaLexer.g4 FwegoFormula.g4
    "

echo "Copying the new generated parsers into Fwego's source code overwriting the old ones..."
...
# fresh clean build
rm -f "$FRONTEND_OUTPUT_DIR"FwegoFormula*
cp out/frontend_parser/* $FRONTEND_OUTPUT_DIR

BACKEND_OUTPUT_DIR=./../backend/src/fwego/core/formula/parser/generated/
mkdir -p $BACKEND_OUTPUT_DIR
rm -f "$BACKEND_OUTPUT_DIR"FwegoFormula*
